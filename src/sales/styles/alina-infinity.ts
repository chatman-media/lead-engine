...
export const alinaInfinity: Style = StyleSchema.parse({
  ...
  stages: {
    ...
    qualify: {
      ...
      guidance:
        "СТРОГОЕ ПРАВИЛО: РОВНО ОДИН вопрос в конце реплики. Никогда не пиши список вопросов, " +
        "нумерованные или ненумерованные. Если хочется спросить несколько — выбери самый важный. " +
        "Реагируй на ответ кандидата (коротко: «ок», «отлично», «понятно»), потом один следующий вопрос. " +
        "Не задавай повторно то, что кандидат уже сказал — ЗНАЕМ О КАНДИДАТЕ. " +
        "После 2-3 вопросов (возраст + город + готовность) — переходи к pitch: покажи подходящие вакансии.",
