...
export const SKILL_CATALOGUE: readonly Skill[] = [
  ...
  // ─── Phase 1 recruitment-specific (SPIN / NEPQ / Cialdini) ─────────
  {
    slug: "qualify-budget-via-spin",
    family: "sales",
    displayName: "SPIN qualify — situation → pain → implication → payoff",
    description:
      "SPIN-методология: 4 типа вопросов (Situation → Problem → Implication → Need-payoff) " +
      "для квалификации кандидата. Выявляет реальный мотив и финансовые ожидания без лобовых вопросов 'сколько хочешь?'.",
    promptFragment:
      "Квалифицируй кандидата через SPIN: " +
      "(S) Ситуационный: 'Ты сейчас работаешь? Где?' / 'Как сейчас с деньгами в целом?' — " +
      "получи контекст БЕЗ давления. " +
      "(P) Проблемный: 'Что не устраивает в текущей ситуации?' / 'Что мешает зарабатывать больше?' — " +
      "выяви боль. " +
      "(I) Импликационный: 'Если так продолжится ещё год — что изменится?' / " +
      "'Как это влияет на твои планы?' — углуби боль через последствия. " +
      "(N) Need-payoff: 'А если бы за 3 месяца закрыть [её боль из P] — стоило бы попробовать?' — " +
      "кандидат сам формулирует ценность. Не спрашивай про цифры первым — жди пока она не " +
      "озвучит ожидания сама; тогда можешь предложить конкретную вакансию из АКТУАЛЬНЫЕ ВАКАНСИИ.",
    applicableStages: stages("qualify"),
    intent: "deepen_engagement",
  },
  {
    slug: "objection-visa-cost",
    family: "sales",
    displayName: "NEPQ objection — visa/cost pain reframe",
    description:
      "NEPQ-методология для работы с возражениями про стоимость визы / авансов / расходов на выезд. " +
      "Вместо спора — pain-discovery: что именно стоит дорого и что значит для неё 'дорого'.",
    promptFragment:
      "Когда кандидат возражает на тему стоимости ('дорого', 'нет денег на визу', 'не потяну расходы') — " +
      "НЕ защищай цену сразу. Применяй NEPQ: " +
      "1) Уточни боль: 'А когда ты говоришь дорого — что именно имеешь в виду: сумму или " +
      "момент оплаты?' " +
      "2) Изучи контекст: 'Если бы расходы покрывались постепенно из первой зарплаты — " +
      "это изменило бы что-то?' " +
      "3) Переведи в решение: 'У нас есть варианты с авансом ДО вылета — хочешь расскажу ' +  " +
      "'как это устроено?' " +
      "Если реально нет схемы — не выдумывай; скажи честно что это нужно уточнить у менеджера. " +
      "Главное — сначала понять, что ИМЕННО стоит дорого, прежде чем объяснять условия.",
    applicableStages: stages("objection"),
    intent: "reframe_objection",
  },
  {
    slug: "close-soft-deposit",
    family: "cialdini",
    displayName: "Commitment — soft reservation / slot hold",
    description:
      "Cialdini Commitment: вместо 'подпишешь договор?' — 'забронирую для тебя место на поток'. " +
      "Психологически мягче; кандидат чувствует себя выбранной, а не загнанной в угол.",
    promptFragment:
      "Когда кандидат близок к решению, но тянет — предложи не 'купить', а 'зарезервировать': " +
      "'Слушай, я могу поставить тебя в список на ближайший поток — это ни к чему не обязывает, " +
      "просто держим место. Тебе ок?' " +
      "Если соглашается — это сильный commitment anchor: будет тяжелее отказаться позже " +
      "(принцип последовательности Чалдини). После бронирования дай конкретный микро-шаг " +
      "('нужно только анкету заполнить, пока место за тобой'). " +
      "НЕ говори что место 'уйдёт через час' если это не правда — только реальные ограничения.",
    applicableStages: stages("close"),
    intent: "elicit_commitment",
  },
];
