...
export const recruiterEmpatheticV1: Style = StyleSchema.parse({
  ...
  hooks: [
    {
      kind: "reciprocity",
      text: "дай ценную информацию о вакансии / рынке труда ДО того как попросить данные кандидата",
    },
    {
      kind: "commitment",
      text: "используй micro-yes: маленькие согласия (имя, город, опыт) перед большим шагом (анкета, встреча)",
    },
    {
      ...
      kind: "liking",
      text: "говори 'наши кандидаты', 'мы поможем', 'наша команда' — кандидат чувствует причастность",
    },
  ],
  stages: {
    opener: {
