...
describe("objection detection", () => {
  ...
  test("'боюсь' → objection", () => expect(s("немного боюсь ехать")).toBe("objection"));
  test("'развод' → objection", () => expect(s("это развод какой-то")).toBe("objection"));
  test("'не могу' → objection", () => expect(s("не могу сейчас")).toBe("objection"));
...
describe("pitch trigger (info-seeking)", () => {
  ...
  test("'какие вакансии' → pitch", () => expect(s("какие у вас вакансии?")).toBe("pitch"));
  test("'условия' → pitch", () => expect(s("расскажи об условиях")).toBe("pitch"));
  test("'виза' → pitch", () => expect(s("как оформляется виза?")).toBe("pitch"));
  test("'ktv' → pitch", () => expect(s("что за ktv работа?")).toBe("pitch"));
});

describe("close from agreement", () => {
  test("'давай' from pitch → close", () => expect(s("ок, давай", "pitch")).toBe("close"));
...
describe("turn-based defaults", () => {
  ...
  test("from opener → qualify on turn 2", () => expect(s("расскажи", "opener", 2)).toBe("qualify"));
  test("from close → stays close", () => expect(s("окей, спасибо", "close", 5)).toBe("close"));
});

describe("qualifier stays in qualify", () => {
  test("age mention keeps qualify", () => expect(s("мне 25 лет", "qualify")).toBe("qualify"));
