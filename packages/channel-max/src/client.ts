import type { MaxApiErrorBody, MaxBotStartedUpdate, MaxMessage } from "./types.ts";
...
export type MaxRecipient = { kind: "user"; id: number } | { kind: "chat"; id: number };
...
export class MaxApiError extends Error {
  constructor(
    ...
    super(`MAX ${method} failed (${statusCode}${code ? `/${code}` : ""}): ${description}`);
...
export class MaxClient {
  ...
  constructor(opts: MaxClientOptions) {
    ...
    this.baseUrl = (opts.baseUrl ?? "https://platform-api.max.ru").replace(/\/{1,512}$/, "");
  ...
  private async call<T>(
    ...
    try {
      ...
      throw new MaxApiError(method, res.status, text || "invalid json response");
    }
  ...
  async getBotInfo(): Promise<MaxBotInfo> {
    ...
    if (!info.user_id || !info.is_bot) {
      throw new MaxApiError("getBotInfo", 200, "token does not belong to a MAX bot");
    }
    return info;
  }
  ...
  async sendText(input: MaxSendTextInput): Promise<{ messageId: string }> {
    ...
    const query = recipient.kind === "user" ? { user_id: recipient.id } : { chat_id: recipient.id };
    ...
    if (!id) throw new MaxApiError("sendText", 200, "no message.body.mid in response");
    return { messageId: id };
  }

  async subscribeWebhook(input: MaxSubscribeWebhookInput): Promise<{ success: boolean }> {
    return this.call<{ success: boolean }>("subscribeWebhook", "/subscriptions", {
      method: "POST",
      body: JSON.stringify({
        url: input.url,
  ...
  async downloadMedia(url: string): Promise<Response> {
    ...
    if (!res.ok) {
      throw new MaxApiError("downloadMedia", res.status, await res.text().catch(() => "no body"));
    }
    return res;
  }

  async sendAction(input: { chatId: number; action: "typing_on" | "mark_seen" }): Promise<void> {
    await this.call<MaxBotStartedUpdate>("sendAction", `/chats/${input.chatId}/actions`, {
      method: "POST",
