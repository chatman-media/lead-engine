...
describe("MaxAdapter", () => {
  it("send text -> POST /messages", async () => {
    ...
    expect(calls[0]?.url).toBe("https://platform-api.max.ru/messages?user_id=555");
  ...
  it("send with unsupported part throws", async () => {
    ...
    await expect(
      a.send({
        channelId: "max1",
        externalUserId: "user:555",
        parts: [{ kind: "photo", mediaRef: { channelId: "max1", externalRef: "u" } }],
      }),
  ...
  it("receive respects already-aborted and pending abort signals", async () => {
    ...
    already.abort();
    await expect(a.receive(already.signal)[Symbol.asyncIterator]().next()).resolves.toMatchObject({
      done: true,
    });
