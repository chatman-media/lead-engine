...
import { type FetchLike, MaxApiError, MaxClient, parseMaxRecipient } from "./client.ts";
...
describe("MaxClient", () => {
  it("requires accessToken", () => {
    expect(() => new MaxClient({ accessToken: "" })).toThrow(/accessToken required/);
  });
  ...
  it("sendText to user calls POST /messages?user_id=...", async () => {
    ...
    await expect(c.sendText({ externalUserId: "user:42", text: "hi" })).resolves.toEqual({
      messageId: "m1",
    });
    expect(calls[0]?.url).toBe("https://platform-api.max.ru/messages?user_id=42");
  ...
  it("sendText to chat calls POST /messages?chat_id=...", async () => {
    ...
    expect(calls[0]?.url).toBe("https://platform-api.max.ru/messages?chat_id=55");
  ...
  it("sendText throws when MAX response has no message id", async () => {
    ...
    await expect(c.sendText({ externalUserId: "user:42", text: "hi" })).rejects.toThrow(
      /message\.body\.mid/,
    );
  });
  ...
  it("downloadMedia returns successful responses and throws on failures", async () => {
    ...
    await expect(c.downloadMedia("https://cdn.example.test/missing.jpg")).rejects.toThrow(
      /downloadMedia failed \(404\)/,
    );
  ...
  it("throws MaxApiError on invalid JSON and success=false body", async () => {
    ...
    await expect(failedBody.sendText({ externalUserId: "user:1", text: "x" })).rejects.toThrow(
      /bad\.request/,
    );
  });
});
