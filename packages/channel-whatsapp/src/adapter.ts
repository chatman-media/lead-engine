...
export class WhatsAppCloudAdapter implements ChannelAdapter {
	...
	async send(envelope: OutboundEnvelope): Promise<Sent> {
		...
		const whatsappMeta = envelope.channelMeta?.whatsapp;
		const template = whatsappMeta?.template;
		if (template) {
			const r = await this.client.sendTemplate({ to, template });
			return {
				channelId: this.id,
		...
		if (
			whatsappMeta?.freeFormAllowedUntil === undefined ||
			whatsappMeta.freeFormAllowedUntil < now
		) {
			throw new Error(
				"WhatsApp free-form outbound requires approved template outside active service window",
			);
		}
