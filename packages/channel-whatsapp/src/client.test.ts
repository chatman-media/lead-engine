...
describe("WhatsAppClient constructor", () => {
	it("требует phoneNumberId и accessToken", () => {
		expect(
			() => new WhatsAppClient({ phoneNumberId: "", accessToken: "t" }),
		).toThrow(/phoneNumberId required/);
		expect(
			() => new WhatsAppClient({ phoneNumberId: "p", accessToken: "" }),
		).toThrow(/accessToken required/);
	});
});
...
describe("sendText", () => {
	...
	it("non-ok → WhatsAppApiError с кодом", async () => {
		const c = new WhatsAppClient(
			opts(mockFetch(() => ({ status: 401, text: "bad token" }))),
		);
		await expect(c.sendText({ to: "1", text: "x" })).rejects.toThrow(
			WhatsAppApiError,
		);
		await expect(c.sendText({ to: "1", text: "x" })).rejects.toThrow(
			/sendText failed \(401\)/,
...
describe("sendTemplate", () => {
	it("POST messages type=template с language/components → messageId", async () => {
		const calls: MockCall[] = [];
		const c = new WhatsAppClient(
			opts(mockFetch(() => ({ body: okSend }), calls)),
		);
	...
	it("template non-ok → WhatsAppApiError", async () => {
		const c = new WhatsAppClient(
			opts(mockFetch(() => ({ status: 400, text: "bad template" }))),
		);
		await expect(
			c.sendTemplate({
				to: "1",
				template: { name: "missing_tpl", languageCode: "en_US" },
			}),
...
describe("getPhoneInfo", () => {
	...
	it("403 → WhatsAppApiError", async () => {
		const c = new WhatsAppClient(
			opts(mockFetch(() => ({ status: 403, text: "forbidden" }))),
		);
		await expect(c.getPhoneInfo()).rejects.toThrow(
			/getPhoneInfo failed \(403\)/,
		);
	});
});
...
describe("sendMedia", () => {
	...
	it("non-ok → WhatsAppApiError", async () => {
		const c = new WhatsAppClient(
			opts(mockFetch(() => ({ status: 500, text: "err" }))),
		);
		await expect(
			c.sendMedia({ to: "1", kind: "video", mediaId: "v" }),
		).rejects.toThrow(/sendMedia failed \(500\)/);
	});
	...
	it("нет message id → throw", async () => {
		...
		await expect(
			c.sendMedia({ to: "1", kind: "document", mediaId: "d" }),
...
describe("downloadMedia (two-step)", () => {
	it("meta → url → bytes", async () => {
		...
		const c = new WhatsAppClient(
			opts(
				mockFetch((url) => {
					if (url.includes("/m99"))
						return { body: { url: "https://cdn.local/file.bin" } };
					return { body: { ok: true } };
				}, calls),
			),
		);
	...
	it("meta non-ok → WhatsAppApiError", async () => {
		const c = new WhatsAppClient(
			opts(mockFetch(() => ({ status: 404, text: "no media" }))),
		);
		await expect(c.downloadMedia("m1")).rejects.toThrow(
			/downloadMedia.meta failed \(404\)/,
		);
	});
	...
	it("bytes non-ok → WhatsAppApiError", async () => {
		const c = new WhatsAppClient(
			opts(
				mockFetch((url) => {
					if (url.includes("/m1"))
						return { body: { url: "https://cdn.local/x" } };
					return { status: 500, text: "cdn down" };
				}),
			),
		);
		await expect(c.downloadMedia("m1")).rejects.toThrow(
			/downloadMedia.bytes failed \(500\)/,
		);
	});
});
