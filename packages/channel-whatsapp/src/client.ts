...
import type { WhatsAppTemplatePayload } from "@chatman-media/channel-core";
...
export class WhatsAppClient {
	...
	constructor(opts: WhatsAppClientOptions) {
		if (!opts.phoneNumberId)
			throw new Error("WhatsAppClient: phoneNumberId required");
		if (!opts.accessToken)
			throw new Error("WhatsAppClient: accessToken required");
	...
	/**
	 * Отправить approved WhatsApp template. Используется для proactive/provider
	 * outreach вне free-form service window.
	 */
	async sendTemplate(input: {
		...
		const id = body.messages?.[0]?.id;
		if (!id)
			throw new WhatsAppApiError("sendTemplate", res.status, "no message id");
		return { messageId: id };
	}
	...
	async sendMedia(input: {
		...
		const id = body.messages?.[0]?.id;
		if (!id)
			throw new WhatsAppApiError("sendMedia", res.status, "no message id");
		return { messageId: id };
	}
	...
	async downloadMedia(mediaId: string): Promise<Response> {
		...
		const bytesRes = await this.fetchImpl(meta.url, {
			headers: this.authHeaders(),
		});
		if (!bytesRes.ok) {
			throw new WhatsAppApiError(
