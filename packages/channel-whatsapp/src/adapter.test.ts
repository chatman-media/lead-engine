...
describe("WhatsAppCloudAdapter", () => {
	it("send text envelope → POST /messages с правильным payload", async () => {
		...
		const sent = await a.send({
			...
			channelMeta: { whatsapp: { freeFormAllowedUntil: freeFormWindow() } },
		});
		expect(sent.externalMessageId).toBe("wa.OUT.1");
		expect(calls).toHaveLength(1);
		const call = calls[0];
		if (!call) throw new Error("expected recorded WhatsApp call");
	...
	it("send template metadata → POST /messages type=template", async () => {
		const { fetch, calls } = fakeFetch([
			{
				status: 200,
				body: {
		...
		const sent = await a.send({
			channelId: "wa1",
			externalUserId: "79161234567",
			parts: [{ kind: "text", text: "fallback body" }],
			channelMeta: {
				whatsapp: {
					template: {
						...
						components: [
							{
								type: "body",
								parameters: [{ type: "text", text: "massage" }],
							},
						],
					},
				},
	...
	it("rejects free-form send outside active window when no approved template is present", async () => {
		...
		await expect(
			a.send({
				channelId: "wa1",
				externalUserId: "79161234567",
				parts: [{ kind: "text", text: "cold free-form" }],
			}),
		).rejects.toThrow(/requires approved template/);
	});
