...
function handoffMeta(overrides: Partial<OperatorHandoffMeta> = {}): OperatorHandoffMeta {
  return {
    reason: "kyc_review",
    title: "KYC review",
...
describe("operatorMediaNotificationData", () => {
  it("без медиа → {}", () => {
    ...
    expect(operatorMediaNotificationData({ parts: [{ kind: "text", text: "hi" }] })).toEqual({});
  });
  ...
  it("с медиа → count + summary + сериализованные refs", () => {
    ...
    const parsed = JSON.parse(String(data.mediaRefsJson)) as Array<Record<string, unknown>>;
...
describe("emitOperatorHandoffNotifications", () => {
  ...
  it("ошибка notify логируется warn'ом и не валит остальные handoff'ы", async () => {
    ...
    await emitOperatorHandoffNotifications({
      ...
      conversationId: 2,
      contactId: 3,
      envelopes: [envelope(handoffMeta()), envelope(handoffMeta({ reason: "payout_review" }))],
      notifications: service,
      sink,
    });
    expect(calls).toHaveLength(2);
    const warns = logs.filter(
      (l) => l.level === "warn" && l.msg === "operator handoff notification failed",
    );
