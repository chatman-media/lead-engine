...
import type { PipelineSink, ProcessInboundResult, TenantContext } from "./types.ts";
...
beforeAll(async () => {
  ...
  const fid = f!.id;
  const mkStage = (slug: string, kind: string, phase: string | null, position: number) =>
...
async function freshContact(tenantId: number, attributesJson?: string | null): Promise<number> {
  const [c] = await db
...
async function contactAttrs(contactId: number): Promise<Record<string, unknown>> {
  const [row] = await db
...
describe("runDeferredInboundPostProcessing: stage classification", () => {
  it("классифицированная стадия пишется в conversation + debug-лог", async () => {
    if (!enabled) return;
    const contactId = await freshContact(tenantWithFunnel);
    const conversationId = await freshConversation(tenantWithFunnel, contactId, "opener");
  ...
  it("той же стадии → без UPDATE и без лога", async () => {
    if (!enabled) return;
    const contactId = await freshContact(tenantWithFunnel);
    const conversationId = await freshConversation(tenantWithFunnel, contactId, "qualify");
    const { logs, sink } = makeSink();
...
describe("runDeferredInboundPostProcessing: memory extraction", () => {
  ...
  it("существующие string-факты и история передаются extractor'у", async () => {
    ...
    await insertMessage(tenantWithFunnel, conversationId, "assistant", "здравствуйте", n + 1);
  ...
  it("извлечённые факты merge'атся в attributes_json + debug-лог", async () => {
    if (!enabled) return;
    const contactId = await freshContact(tenantWithFunnel, JSON.stringify({ city: "Bangkok" }));
    const conversationId = await freshConversation(tenantWithFunnel, contactId);
    const { logs, sink } = makeSink();
