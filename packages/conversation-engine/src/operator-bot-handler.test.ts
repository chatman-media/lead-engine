...
function makeSendDraftDb(
	...
	let durableDraftStatus = "pending";
				...
				return {
					where: () => ({
						returning: async () => {
							...
							if (durableDraftStatus !== "pending") return [];
							durableDraftStatus = "sent";
							return [{ id: 700 }];
						},
					}),
				};
...
describe("operator action callbacks", () => {
	...
	it("callback op:take/op:ai no-op does not duplicate writes or audit", async () => {
		for (const testCase of [
			{
				callback: "op:take:109",
				initialMode: "human",
				expectedText: "уже в работе оператора",
			},
			{
				callback: "op:ai:109",
				initialMode: "ai",
				expectedText: "уже под управлением AI",
			},
		] as const) {
			const settings = makeSettings({
				adminId: 10,
			...
			const tx = {
				...
				select: () => ({
					from: () => ({
						where: () => ({
							limit: async () => [{ mode: testCase.initialMode }],
						}),
					}),
				}),
				update: () => ({
			...
			await handler.handleUpdate({
				update_id: 15,
				callback_query: {
					id: `cb-${testCase.callback}`,
					from: { id: 5 },
					data: testCase.callback,
					message: {
						message_id: 9,
						date: 0,
						chat: { id: 777, type: "private" },
			...
			expect(client.sent[0]?.text).toContain(testCase.expectedText);
		}
	});
	...
	it("preview confirmation rejects wrong chat/admin without sending to client", async () => {
		for (const testCase of [
			{
				name: "wrong chat",
				callbackChatId: 888,
			...
			const settings = makeSettings({
				adminId: testCase.settingsAdminId,
				tenantId: 3,
				telegramChatId: String(testCase.callbackChatId),
			});
			const repo = new FakeRepo(settings);
			const inserts: Array<Record<string, unknown>> = [];
			const updates: Array<Record<string, unknown>> = [];
			...
			const tx = {
				...
				select: () => ({
					from: (table: unknown) => ({
						where: () => ({
							limit: async () =>
								table === operatorActionDrafts ? [draftRow] : [],
						}),
					}),
				}),
				insert: () => ({
			...
			await handler.handleUpdate({
				...
				callback_query: {
					...
					message: {
						message_id: 21,
						date: 0,
						chat: { id: testCase.callbackChatId, type: "private" },
					},
				},
			} as unknown as TgUpdate);
	...
	it("durable preview confirmation is idempotent after first claim", async () => {
		const settings = makeSettings({
			adminId: 10,
			tenantId: 3,
			telegramChatId: "777",
		...
		const repo = new FakeRepo(settings);
		const { audits, db, inserts } = makeSendDraftDb();
		const handler = new OperatorBotHandler(
			repo as unknown as NotificationsRepo,
			"token",
			{
		...
		const draft = {
			...
			chatId: "777",
			conversationId: 109,
			text: "One durable client reply",
			metadata: { source: "operator_bot_reply" },
			createdAt: 123,
			expiresAt: 723,
		};
	...
	it("confirmed payment review previews persist order review metadata without marking paid", async () => {
		for (const [action, paymentReviewStatus] of [
			["payment_under_review", "under_review"],
			["payment_problem", "problem"],
		] as const) {
			const settings = makeSettings({
				adminId: 10,
				tenantId: 3,
			...
			const repo = new FakeRepo(settings);
			const { audits, db, inserts, order } = makeSendDraftDb({
				order: {
					id: 77,
					status: "awaiting_payment",
					verificationId: "operator-bot-109-99",
					payoutCode: null,
					payoutCodeExpiresAt: null,
					proofJson: JSON.stringify({ receiptType: "fiat_receipt" }),
				},
			});
			const handler = new OperatorBotHandler(
				repo as unknown as NotificationsRepo,
			...
			const proof = JSON.parse(String(order?.proofJson)) as Record<
				string,
				unknown
			>;
			expect(proof.receiptType).toBe("fiat_receipt");
			expect(proof.operatorPaymentReview).toMatchObject({
				status: paymentReviewStatus,
				action,
				reviewedByAdminId: 10,
				reviewedAt: 123,
				source: "operator_bot",
				previousOrderStatus: "awaiting_payment",
			});

			const messageInsert = inserts.find((row) => row.table === "messages");
			const messageMeta = JSON.parse(
				String(messageInsert?.values.metaJson),
			) as Record<string, unknown>;
			expect(messageMeta.exchangeSideEffects).toMatchObject({
				action,
				orderId: 77,
	...
	it("confirmed KYC OK preview persists verified contact state", async () => {
		...
		const repo = new FakeRepo(settings);
		const { audits, db, inserts, order, contactAttributes } = makeSendDraftDb({
	...
	it("confirmed KYC non-approval previews keep contact unverified", async () => {
		for (const [action, expectedStatus] of [
			...
			const repo = new FakeRepo(settings);
			const { db, inserts, order, contactAttributes } = makeSendDraftDb({
				contactAttributesJson: JSON.stringify({
					city: "Phuket",
					isVerified: true,
					exchangeKyc: {
