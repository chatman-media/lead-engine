...
export async function resolveConversation(opts: {
	...
	const channelId =
		opts.channelId && opts.channelId > 0 ? opts.channelId : null;

	if (channelId) {
		const byChannel = await opts.conversations.findByContactAndChannel(
			opts.contactId,
			channelId,
		);
		if (byChannel) {
		...
		if (canReuseLegacySource(opts.channelKind)) {
			const legacy = await opts.conversations.findByContactAndSource(
				opts.contactId,
				source,
			);
			if (legacy) {
