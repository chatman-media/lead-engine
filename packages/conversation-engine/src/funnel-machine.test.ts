...
describe("funnel-machine", () => {
	...
	it("validateTransition allow'ает переход из next[]", () => {
		expect(() =>
			validateTransition(TEMPLATE, "intake", "qualified"),
		).not.toThrow();
		expect(() =>
			validateTransition(TEMPLATE, "qualified", "won"),
		).not.toThrow();
	});

	it("validateTransition allow'ает same-state (idempotent retry)", () => {
		expect(() =>
			validateTransition(TEMPLATE, "qualified", "qualified"),
		).not.toThrow();
	});

	it("validateTransition блокирует переход НЕ из next[]", () => {
		expect(() => validateTransition(TEMPLATE, "intake", "won")).toThrow(
			FunnelTransitionError,
		);
	});

	it("validateTransition блокирует переход из terminal", () => {
		expect(() => validateTransition(TEMPLATE, "won", "qualified")).toThrow(
			/terminal/,
		);
		expect(() => validateTransition(TEMPLATE, "rejected", "intake")).toThrow(
			/terminal/,
		);
	});

	it("validateTransition блокирует переход в несуществующий stage", () => {
		expect(() => validateTransition(TEMPLATE, "intake", "zzz")).toThrow(
			/to-state/,
		);
		expect(() => validateTransition(TEMPLATE, "yyy", "intake")).toThrow(
			/from-state/,
		);
	});

	it("allowedTransitions возвращает next[]", () => {
		expect(allowedTransitions(TEMPLATE, "intake")).toEqual([
			"qualified",
			"rejected",
		]);
