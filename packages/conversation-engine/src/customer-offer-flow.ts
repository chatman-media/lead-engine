import type { ChannelKind, OutboundEnvelope } from "@chatman-media/channel-core";
import {
  channelIdentities,
  channels,
  orderEvents,
...
export class CustomerOfferFlow {
  ...
  async draftCustomerOffer(opts: {
    ...
    const quoteEventData = await this.latestEventData(providerRequest.id, "provider_quoted");
    ...
    const availabilityText =
      typeof quoteEventData.availabilityText === "string" && quoteEventData.availabilityText.trim()
  ...
  async sendCustomerOffer(input: SendCustomerOfferInput): Promise<SendCustomerOfferResult> {
  ...
  async acceptCustomerOffer(input: AcceptCustomerOfferInput): Promise<CustomerOfferAccepted> {
    const order = await this.requireOrder(input.orderId);
    if (order.status !== "offer_ready" && order.status !== "awaiting_customer_payment") {
      throw new Error(`service order is not offer-ready: ${order.status}`);
    }
    const providerRequest = await this.requireProviderRequest(order.id, ["quoted", "accepted"]);
  ...
  async recordPaymentSuccess(
    ...
    if (!["awaiting_customer_payment", "paid", "confirmed"].includes(initialOrder.status)) {
      return {
        ok: false,
    ...
    const providerRequest = await this.latestProviderRequestForOrder(initialOrder.id, [
      "accepted",
      "quoted",
    ]);
    if (!providerRequest) {
    ...
    if (order.status === "paid") {
      order = await this.relay.transitionOrderStatus(order.id, "confirmed", input.nowEpoch);
  ...
  private async updateOrderPaymentState(opts: {
    ...
    const [row] = await this.ctx.db
      ...
      .where(and(eq(serviceOrders.id, opts.orderId), eq(serviceOrders.tenantId, this.ctx.tenantId)))
      .returning();
...
export function formatCustomerOrderContext(orders: ServiceOrderRow[]): string | null {
...
function readStringFromObject(value: Record<string, unknown>, keys: string[]): string | null {
