...
type SummaryConversationsRepo = {
	...
	): Promise<{ summaryJson: string | null } | null>;
...
export async function loadRollingConversationContext(input: {
	...
	const recentLimit =
		recentWindow +
		(input.currentMessageId !== undefined || input.currentMessageText ? 1 : 0);
	...
	try {
		...
		input.onWarn?.(
			"[conversation-summary] failed to load summary snapshot",
			err,
		);
		return { history };
	}
	...
	const maxMessagesToSummarize =
		input.options.maxMessagesToSummarize !== undefined
			...
			: 10_000;
	const oldRows = await input.messages.forConversationSummary(
		input.conversationId,
		{
	...
	try {
		await conversations.setSummaryJson(
			input.conversationId,
			stringifySummaryPayload(payload),
		);
	} catch (err) {
