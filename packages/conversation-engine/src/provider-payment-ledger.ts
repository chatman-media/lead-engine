...
export type ServiceOrderCommissionStatus =
	| "pending"
	| "earned"
	| "void"
	| "refunded"
...
export class ProviderPaymentLedger {
	...
	async refundPayment(input: {
		...
		const commission = await this.refundCommissionForPayment(
			payment.id,
			input.nowEpoch,
		);
		await this.appendEventOnce({
	...
	private async updateOrderPaymentSnapshot(opts: {
		...
		const [row] = await this.ctx.db
			...
			.where(
				and(
					eq(serviceOrders.id, opts.order.id),
	...
	private async appendEventOnce(opts: {
		...
		const paymentId =
			typeof opts.data?.paymentId === "number" ? opts.data.paymentId : null;
...
function resolveOrderPaymentAmounts(opts: {
	...
	const providerAmount =
		opts.order.quotedAmount ?? opts.order.customerAmount ?? opts.amount ?? 0;
	if (!(providerAmount > 0)) {
	...
	const customerAmount =
		opts.amount ??
		opts.order.customerAmount ??
		roundMoney(providerAmount + commissionAmount);
	const commissionPct =
