...
import { processInbound, type ReplyStrategy, transcribeInboundVoice } from "./process-inbound.ts";
import {
...
function makeDeps(reply: ReplyStrategy | null = null, channel: ChannelContext = CHANNEL) {
...
describe("processInbound", () => {
  ...
  it("same contact can keep separate conversations per real channel", async () => {
    ...
    await processInbound(textInbound({ extUserId: "u1", extMessageId: "100", text: "tg" }), deps);
    const whatsappDeps = {
      ...deps,
      channel: {
        channelId: 20,
    ...
    expect(deps._fakes.conversations.all().map((row) => row.channelId)).toEqual([10, 20]);
    expect(deps._fakes.conversations.all().map((row) => row.source)).toEqual(["bot", "bot"]);
  });
  ...
  it("правка (edited) с тем же текстом — no-op (идемпотентность ретрая edited_message)", async () => {
    ...
    await processInbound(textInbound({ extUserId: "u1", extMessageId: "100", text: "hi" }), deps);
    expect(replyCalls).toBe(1);
  ...
  it("human_takeover в exchange с медиа помечается reason=kyc_review (KYC-кнопки в боте)", async () => {
    ...
    const photoInbound = {
      ...
      externalUserId: "u-kyc",
      parts: [{ kind: "photo", mediaRef: { channelId: "10", externalRef: "passport_2" } }],
      receivedAt: 1700000000,
      raw: {},
    } as unknown as Inbound;
  ...
  it("exchange media-only inbound creates actionable operator handoff with media refs", async () => {
    ...
    const handoff = events.find((e) => e.eventType === "operator_handoff_required");
    ...
    expect(events.some((e) => e.eventType === "verification_requested")).toBe(false);
  });
  ...
  it("exchange: фото с подписью про оплату → payment_review handoff (не KYC)", async () => {
    ...
    const reply: ReplyStrategy = {
      generate: async () => {
        replyCalls += 1;
        return [
          { channelId: "10", externalUserId: "u-pay", parts: [{ kind: "text", text: "AI" }] },
        ];
      },
    };
    const d = {
