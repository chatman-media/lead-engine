...
describe("OPERATOR_BOT_ACTIONS", () => {
	it("перечисляет все поддерживаемые действия", () => {
		expect(OPERATOR_BOT_ACTIONS).toEqual([
			"open_chat",
			"takeover",
			"return_ai",
		]);
...
describe("isOperatorHandoffEvent", () => {
	...
	it("false для чужих типов событий", () => {
		expect(
			isOperatorHandoffEvent({ eventType: "lead_created", conversationId: 5 }),
		).toBe(false);
	});

	it("false без валидного conversationId", () => {
		expect(
			isOperatorHandoffEvent({ eventType: "operator_handoff_required" }),
		).toBe(false);
		expect(
			isOperatorHandoffEvent({
				eventType: "operator_handoff_required",
...
describe("operatorPreviewCallbackData", () => {
	it("кодирует send/cancel и round-trip'ится", () => {
		...
		expect(operatorPreviewCallbackData("cancel", "abc123")).toBe(
			"opm:c:abc123",
		);
...
describe("operatorExchangeActionCallbackData", () => {
	it("включает orderId когда он валиден", () => {
		expect(
			operatorExchangeActionCallbackData("payment_confirmed", 109, 77),
		).toBe("opx:payok:109:77");
	});

	it("опускает orderId когда он отсутствует или невалиден", () => {
		expect(operatorExchangeActionCallbackData("payout_ready", 109)).toBe(
			"opx:payout:109",
		);
		expect(
			operatorExchangeActionCallbackData("office_details", 109, null),
		).toBe("opx:office:109");
		expect(operatorExchangeActionCallbackData("kyc_approved", 109, 0)).toBe(
			"opx:kycok:109",
		);
	});
	...
	it("round-trip через parseOperatorExchangeActionCallback", () => {
		...
		expect(
			parseOperatorExchangeActionCallback(
				operatorExchangeActionCallbackData("kyc_rejected", 11),
			),
