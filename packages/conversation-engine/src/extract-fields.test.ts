...
function textInbound(externalUserId: string, externalMessageId: string, text: string): Inbound {
  return {
    channelId: "tg-1",
    externalMessageId,
    externalUserId,
...
function makeTemplate(extractor: (text: string) => Record<string, unknown>): VerticalTemplate {
  return {
    slug: "test_v1",
    displayName: "Test",
    version: 1,
...
describe("processInbound + extractFields hook", () => {
  ...
  it("merge'нет несколько inbound'ов — атрибуты накапливаются", async () => {
    ...
    const contact = deps._fakes.contacts.all()[0]!;
    expect(JSON.parse(contact.attributesJson!)).toEqual({ name: "Алина", age: 25 });
  });
  ...
  it("hook НЕ вызывается на media-only inbound (text пустой)", async () => {
    ...
    const inbound: Inbound = {
      ...
      externalUserId: "u1",
      parts: [{ kind: "photo", mediaRef: { channelId: "tg-1", externalRef: "f" } }],
      receivedAt: 1700000000,
      raw: {},
    };
    await processInbound(inbound, deps);
  ...
  it("hook-исключение не ломает pipeline — inbound persist'нет, reply pipeline не break'нет", async () => {
    ...
    expect(sinkLogs.some((l) => l.msg === "extractFields hook failed")).toBe(true);
  });
