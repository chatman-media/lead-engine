...
describe("loadExperimentVariants", () => {
	...
	test("возвращает null когда configJson невалидный", async () => {
		const result = await loadExperimentVariants(
			...
			{
				findActiveBySlug: async () => ({ configJson: "{invalid json}" }),
			} as never,
		);
		expect(result).toBeNull();
	});
	...
	test("возвращает variants для валидного стиля", async () => {
		const result = await loadExperimentVariants(
			makeExperiment(
				JSON.stringify([{ styleSlug: "test-style", weight: 0.6 }]),
			),
			{
				findActiveBySlug: async () => ({ configJson: VALID_CONFIG_JSON }),
			} as never,
		);
	...
	test("пропускает missing стиль, возвращает валидные", async () => {
		...
		const mixedRepo = {
			findActiveBySlug: async () =>
				++call === 1 ? null : { configJson: VALID_CONFIG_JSON },
		};
		const result = await loadExperimentVariants(
			makeExperiment(
				JSON.stringify([
