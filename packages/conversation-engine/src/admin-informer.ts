...
const SEV_RANK: Record<InformerSeverity, number> = {
	info: 1,
	important: 2,
	critical: 3,
};
...
export function notificationEventToInformer(
	...
	if (event.data.title) parts.push(String(event.data.title));
	if (event.data.action) parts.push(String(event.data.action));
	if (event.data.contractId) {
		parts.push(`Contract: ${String(event.data.contractId)}`);
	}
	...
	return {
		...
		dedupKey:
			event.eventType === "operator_handoff_required"
				? `${event.eventType}:${ref}:${String(event.data.reason ?? "general")}`
				: `${event.eventType}:${ref}`,
		url,
	};
}
...
export class AdminInformer {
	...
	async emit(event: InformerEvent): Promise<void> {
		...
		const plan = await withTenant(this.deps.db, event.tenantId, async (tx) => {
			...
			const level = (owner.settings?.informerLevel ??
				"important") as InformerLevel;
			const realtime =
		...
		if (event.severity === "critical" && this.deps.email && plan.email) {
			try {
				...
				this.deps.log?.warn?.("[informer] email send failed", {
					to: plan.email,
					err: String(err),
				});
			}
