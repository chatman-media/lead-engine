...
import { parseProviderResponse, ProviderResponseHandler } from "./provider-response-handler.ts";
...
describe("parseProviderResponse", () => {
  it("extracts quotes, availability, and decline intent without LLM calls", () => {
    expect(
      parseProviderResponse([{ kind: "text", text: "Available today 18:00, price 1,200 THB" }]),
    ).toEqual({
      kind: "quote",
      quotedAmount: 1200,
    ...
    expect(parseProviderResponse([{ kind: "text", text: "Sorry, no slots today" }])).toMatchObject({
      kind: "decline",
      availabilityText: "today",
    });
    expect(parseProviderResponse([{ kind: "text", text: "Let me check" }])).toMatchObject({
      kind: "ambiguous",
    });
  });
});
...
describe("ProviderResponseHandler", () => {
  it("associates provider quote with the open request and does not notify customer", async () => {
    ...
    const quoteEvent = events.find((event) => event.eventType === "provider_quoted");
  ...
  it("records provider decline on the linked request", async () => {
    ...
    const result = await handler().handleProviderResponse({
      inbound: inbound([{ kind: "text", text: "Sorry, no slots today" }], "wa-decline-1", now + 40),
    });
    ...
    const events = await relayRepo().eventsForOrder(outreach.order.id);
    const declineEvent = events.find((event) => event.eventType === "provider_declined");
  ...
  it("attaches provider media metadata to order events", async () => {
    ...
    const events = await relayRepo().eventsForOrder(outreach.order.id);
    const mediaEvent = events.find((event) => event.eventType === "provider_response_ambiguous");
