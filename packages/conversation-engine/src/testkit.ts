...
export class FakeChannelIdentitiesRepo {
  ...
  async find(channelId: number, externalUserId: string): Promise<ChannelIdentityRow | null> {
    return (
      this.rows.find((r) => r.channelId === channelId && r.externalUserId === externalUserId) ??
      null
    );
  }
  async create(opts: { contactId: number; channelId: number; externalUserId: string }) {
...
export class FakeConversationsRepo {
  ...
  async autoAssignLeastBusyOperator(_conversationId: number): Promise<number | null> {
  ...
  async applyAutoHandoff(input: {
    ...
    if (!row) return null;
    const changed = row.mode !== "human" || row.status !== "pending" || row.escalatedAt == null;
    row.mode = "human";
    row.status = "pending";
...
export class FakeMessagesRepo {
  ...
  async findUserByExternalId(
    ...
    return (
      this.rows.find(
        (r) => r.conversationId === conversationId && r.role === "user" && r.tgMessageId === num,
      ) ?? null
    );
  }
...
export class FakeOutboundQueueRepo {
  ...
  async enqueue(opts: {
    ...
    if (opts.envelope.idempotencyKey) {
      const existing = this.rows.find((r) => r.idempotencyKey === opts.envelope.idempotencyKey);
