...
export interface ProcessInboundDeps {
	...
	downloadVoice?:
		| ((
				mediaRef: import("@chatman-media/channel-core").MediaRef,
...
export async function processInbound(
	...
	if (deps.transcriber && deps.downloadVoice) {
		for (const part of inbound.parts) {
			if (part.kind === "voice") {
				try {
					const res = await deps.downloadVoice(
						part.mediaRef,
	...
	const hasMedia = inbound.parts.some(
		(p) => p.kind !== "text" && p.kind !== "callback_query",
	);
			...
			if (newStage && newStage !== "opener" && deps.leads && deps.db) {
				try {
					...
					if (res && (res.created || res.advanced)) {
						deps.sink?.log?.(
							"info",
							res.created ? "lead auto-created" : "lead advanced",
							{
								tenantId: deps.tenant.tenantId,
								conversationId: conversation.id,
								contactId: contact.id,
