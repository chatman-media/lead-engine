...
describe("NotificationService.notify", () => {
	...
	it("operator handoff notification includes reactive operator buttons", async () => {
		...
		expect(JSON.stringify(markup)).toContain("https://app.example/conversations/109");
		expect(JSON.stringify(markup)).toContain("op:take:109");
		expect(JSON.stringify(markup)).toContain("op:ai:109");
