...
export class OpsAlertRouter implements OpsAlertSink {
		...
		if (deps.telegram !== undefined) {
			...
			this.telegram = {
				send: (chatId, htmlText) =>
					client
						.sendMessage({ chatId, text: htmlText, parseMode: "HTML" })
						.then(() => undefined),
			};
		} else {
			this.telegram = null;
	...
	async emit(alert: OpsAlert): Promise<void> {
		...
		const owner = await resolveOwnerContacts(
			this.deps.db,
			alert.tenantId,
		).catch((err) => {
		...
		if (this.telegram && owner.telegramChatIds.length > 0) {
			...
			for (const chatId of owner.telegramChatIds) {
				try {
					...
					this.deps.log?.warn?.("[ops-alerts] telegram send failed", {
						chatId,
						err: String(err),
					});
				}
		...
		const emailNeeded =
			...
			(!telegramDelivered && alert.severity !== "info");
...
export class ResendEmailSender implements OpsEmailSender {
	...
	async send(opts: {
		...
		if (!this.apiKey) {
			console.log(
				`[resend] dry-run (no key) → to=${opts.to} subject="${opts.subject}"`,
			);
			return;
		}
