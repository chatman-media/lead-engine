...
export class ProviderResponseHandler {
  ...
  async handleProviderResponse(
    ...
    const channelDbId = input.channelDbId ?? parsePositiveInt(input.inbound.channelId);
...
export function parseProviderResponse(parts: InboundPart[]): ProviderResponseParse {
...
export function extractProviderResponseText(parts: InboundPart[]): string {
  ...
  for (const part of parts) {
    ...
    if ((part.kind === "photo" || part.kind === "video") && typeof part.caption === "string") {
...
function extractAvailabilityText(text: string): string | null {
  ...
  const day = normalized.match(/\b(today|tomorrow|tonight|сегодня|завтра|вечером)\b/i)?.[0];
