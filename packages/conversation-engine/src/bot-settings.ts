...
export interface BotSettings {
	...
	replyDelaySeconds: number | null; // 0..120
}
...
export function parseBotSettings(json: string | null | undefined): BotSettings {
	...
	return {
		temperature:
			raw.temperature == null ? null : clampFloat(raw.temperature, 0, 1.5),
		maxOutputTokens:
			raw.maxOutputTokens == null
				? null
				: clampInt(raw.maxOutputTokens, 100, 4000),
		compactAfterMessages:
			raw.compactAfterMessages == null
				? null
				: clampInt(raw.compactAfterMessages, 5, 100),
		fallbackText: str(raw.fallbackText, 1000),
		greetingText: str(raw.greetingText, 1000),
		hoursEnabled: bool(raw.hoursEnabled, false),
		hoursStartMin:
			raw.hoursStartMin == null ? null : clampInt(raw.hoursStartMin, 0, 1439),
		hoursEndMin:
			raw.hoursEndMin == null ? null : clampInt(raw.hoursEndMin, 0, 1439),
		...
		handoffAfterFallbacks:
			raw.handoffAfterFallbacks == null
				? null
				: clampInt(raw.handoffAfterFallbacks, 1, 20),
		splitReplies: bool(raw.splitReplies, false),
		typingIndicator: bool(raw.typingIndicator, false),
		autocloseHours:
			raw.autocloseHours == null ? null : clampInt(raw.autocloseHours, 1, 720),
		voiceStt: bool(raw.voiceStt, true),
		mediaAckText: str(raw.mediaAckText, 500),
		replyDelaySeconds:
			raw.replyDelaySeconds == null
				? null
				: clampInt(raw.replyDelaySeconds, 0, 120),
	};
}
 ...
 */
export function isWithinBotHours(
	settings: BotSettings,
	nowEpochSec: number,
): boolean {
	if (!settings.hoursEnabled) return true;
	if (settings.hoursStartMin == null || settings.hoursEndMin == null)
		return true;
	let minutes: number;
	try {
