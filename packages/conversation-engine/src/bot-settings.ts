...
export interface BotSettings {
  ...
  // #735 — тенант-дефолт языка ответа/перевода. null = берём DEFAULT_LANG.
  // Используется когда conversation.detected_lang ещё не определён. Набор:
  // ru/en/ko/zh — валидируется через asSupportedLang в parseBotSettings.
...
export function parseBotSettings(json: string | null | undefined): BotSettings {
  ...
  return {
    ...
    replyDelaySeconds:
      raw.replyDelaySeconds == null ? null : clampInt(raw.replyDelaySeconds, 0, 120),
    defaultLang: asSupportedLang(raw.defaultLang),
  };
}
...
export function isWithinBotHours(settings: BotSettings, nowEpochSec: number): boolean {
  ...
  return start < end ? minutes >= start && minutes < end : minutes >= start || minutes < end; // окно через полночь
}
