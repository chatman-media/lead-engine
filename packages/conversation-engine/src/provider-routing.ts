...
const activeStatusesSql = sql.join(
	ACTIVE_SERVICE_ORDER_STATUSES.map(
		(status: ServiceOrderStatus) => sql`${status}`,
	),
	sql`, `,
);
...
export class ProviderRouter {
	...
	private async selectOverride(opts: {
		...
		if (provider.status !== "active")
			return { ok: false, reason: "provider_paused" };
		...
		const hasService = await this.providerHasActiveService(
			opts.providerId,
			opts.serviceType,
		);
		return {
	...
	private async hasEligibleServiceIgnoringArea(
		serviceType: string,
	): Promise<boolean> {
