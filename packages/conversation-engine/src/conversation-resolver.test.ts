...
function repo(existing?: unknown | unknown[], assignAdminId?: number | null) {
  ...
  const conversations = {
    findByContactAndSource: async (contactId: number, source: string) =>
      rows.find(
        (row) => row.userId === contactId && row.source === source && row.channelId === null,
      ) ?? null,
    findByContactAndChannel: async (contactId: number, channelId: number) =>
      rows.find((row) => row.userId === contactId && row.channelId === channelId) ?? null,
...
describe("resolveConversation", () => {
  ...
  it("legacy-строка без channelId → reuse для telegram_bot (legacy source)", async () => {
    const { conversations } = repo({ id: 5, userId: 1, channelId: null, source: "bot" });
    const res = await resolveConversation({
      contactId: 1,
      channelId: 10,
      channelKind: "telegram_bot",
    ...
    expect(res.created).toBe(false);
    expect(res.conversation.id).toBe(5);
  });
  ...
  it("ошибка авто-назначения → диалог всё равно создаётся (best-effort)", async () => {
    const { conversations } = repo();
    const throwingConversations = {
      ...conversations,
      autoAssignLeastBusyOperator: async () => {
    ...
    const res = await resolveConversation({
      ...
      channelId: 20,
      channelKind: "telegram_bot",
      conversations: throwingConversations,
      nowEpoch: 0,
    });
    expect(res.created).toBe(true);
  });
