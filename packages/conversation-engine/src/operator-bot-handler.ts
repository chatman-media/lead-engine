...
export class OperatorBotHandler {
	...
	private async handleCallback(cq: TgCallbackQuery): Promise<void> {
		...
		const operatorAction = parseOperatorActionCallback(cq.data);
		if (operatorAction) {
			await this.handleOperatorAction(cq, s, operatorAction);
			return;
		}
	...
	private extractConversationIdFromReply(
		...
		for (const row of rows) {
			for (const button of row) {
				const action = parseOperatorActionCallback(button.callback_data);
				if (action) return action.conversationId;
	...
	private async applyExchangeDraftSideEffects(
		...
		if (action === "payment_under_review" || action === "payment_problem") {
			return this.applyPaymentReviewSideEffect(tx, draft, now, action);
		}
	...
	private async applyPaymentReviewSideEffect(
		tx: Db,
		draft: PendingOperatorDraft,
		now: number,
		action: "payment_under_review" | "payment_problem",
	): Promise<Record<string, unknown>> {
		const orderId = this.metadataOrderId(draft.metadata);
		const order = await this.findExchangeOrderForDraft(tx, draft);
		if (!order) {
		...
		const paymentReviewStatus =
			action === "payment_problem" ? "problem" : "under_review";
		const proof = parseJsonObject(order.proofJson);
		const currentReview = objectValue(proof.operatorPaymentReview);
		const nextProof = {
			...proof,
			operatorPaymentReview: {
				...currentReview,
		...
		await tx
			.update(exchangeOrders)
			.set({
				proofJson: JSON.stringify(nextProof),
				updatedAt: now,
			})
			.where(
				and(
	...
	private async handleOperatorAction(
		cq: TgCallbackQuery,
		settings: { adminId: number; tenantId: number },
		input: { action: OperatorBotActionKind; conversationId: number },
	): Promise<void> {
		if (!this.client) return;
		if (!this.actions.db) {
			await this.client.answerCallbackQuery({
