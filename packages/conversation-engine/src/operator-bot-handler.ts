...
const EXCHANGE_QUICK_REPLIES: Record<OperatorBotExchangeAction, ExchangeQuickReplyDraft> = {
  kyc_approved: {
...
export class OperatorBotHandler {
  ...
  async handleUpdate(update: TgUpdate): Promise<void> {
    ...
    if (text && !text.startsWith("/") && message.reply_to_message) {
      const handled = await this.handlePotentialDraftReply(message, chatId, text);
      if (handled) return;
    }
    ...
    if (
      ...
      const handled = await this.handleTopicReply(chatId, message.message_thread_id, text);
      if (handled) return;
    }
    ...
    if (text === "/setup" || text.startsWith("/setup@")) {
      await this.linking.handleSetupGroup(message.chat.id, message.chat.title || "эту группу");
      return;
    }
    ...
    if (text === "/mute" || text.startsWith("/mute ")) return this.informer.cmdMute(chatId, text);
    if (text === "/last" || text.startsWith("/last ")) return this.informer.cmdLast(chatId, text);
  ...
  private async handleCallback(cq: TgCallbackQuery): Promise<void> {
    ...
    if (kind === "lvl" && (LEVELS as readonly string[]).includes(val ?? "")) {
      ...
      await this.editKeyboard(cq, "Насколько громко информировать?", levelKeyboard(val as string));
      await this.client.answerCallbackQuery({
        callbackQueryId: cq.id,
    ...
    if (kind === "dig" && (DIGESTS as readonly string[]).includes(val ?? "")) {
      ...
      await this.editKeyboard(cq, "Как часто слать сводку?", digestKeyboard(val as string));
      await this.client.answerCallbackQuery({
        callbackQueryId: cq.id,
    ...
    if (kind === "tpc" && (TOPICS as readonly string[]).includes(val ?? "")) {
      ...
      await this.editKeyboard(cq, "Темы (нажми, чтобы вкл/выкл):", topicsKeyboard(map));
      await this.client.answerCallbackQuery({
        callbackQueryId: cq.id,
  ...
  private async handlePotentialDraftReply(
    ...
    const conversationId = extractConversationIdFromReply(message.reply_to_message);
   ...
   */
  private async handleTopicReply(chatId: string, threadId: number, text: string): Promise<boolean> {
    if (!this.client) return false;
    ...
    const conversation = await conversations.findConversationByOperatorThread(threadId);
    ...
    const adminId = conversation.assignedAdminId ?? (await this.resolveOwnerAdminId(rule.tenantId));
    if (adminId == null) {
      await this.client.sendMessage({
        chatId,
        messageThreadId: threadId,
  ...
  private async applyInstantKycApproved(
    ...
    const outcome = await withTenant(db, settings.tenantId, async (tx) => {
      ...
      await applyKycDecisionSideEffect(tx, draft, now, conv.contactId, "kyc_approved");
      ...
      replyMarkup: {
        inline_keyboard: [
          ...
          [
            {
              text: "Отмена",
              callback_data: operatorPreviewCallbackData("cancel", draft.draftId),
            },
          ],
        ],
      },
