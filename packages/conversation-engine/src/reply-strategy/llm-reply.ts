...
const EXCHANGE_QUOTE_INTENT_RE = /курс|rate|сколько|получ(?:у|ится|ить)|итого|посчитай|рассчитай/i;
...
async function maybeForceExchangeQuoteReply(
  ...
  const freshAmount =
    ...
    (EXCHANGE_QUOTE_INTENT_RE.test(userMessageText) && !hasRecentExchangeQuote(history));
  ...
  const dirQ4 =
    typeof resultRow4.direction === "string" ? resultRow4.direction.split("->")[1] : null;
...
function exchangeOrderArgsFromCollected(
  ...
  return {
    ...
    ...(collected.paymentMethod ? { paymentMethod: collected.paymentMethod } : {}),
  };
}
...
async function maybeForceExchangeOrderReply(
  ...
  if (!text) return null;
  const toolCalls: ToolCallRecord[] = [{ name: orderTool.name, args, result, cycle: 0 }];
  ...
  if (orderRow && typeof orderRow.orderId === "number" && orderRow.needsVerification !== true) {
    const fetchTool = tools.find((tool) => tool.name === "fetch_exchange_requisites");
    if (fetchTool) {
      try {
...
export class LlmReplyStrategy implements ReplyStrategy {
  constructor(
    ...
    private readonly conversationsRepoFor?: (tenantId: number) => ConversationsRepo,
  ) {}
  ...
  async generate(input: {
    ...
    const { history, conversationSummary } = await loadRollingConversationContext({
    ...
    const exchangeCollected =
      isExchange && this.opts.resolveExchangeCollected
        ? await Promise.resolve(
            ...
            console.warn("[llm-reply] failed to resolve exchange collected fields:", err);
            return null;
          })
        : null;
    ...
    const exchangeGuardEnabled =
      isExchange && this.opts.resolveExchangeResponseGuardEnabled
        ? await Promise.resolve(
            ...
            console.warn("[llm-reply] failed to resolve exchange response guard flag:", err);
            return true;
          })
        : true;
    const exchangeCustomerNoticeEnabled =
      isExchange && this.opts.resolveExchangeCustomerNoticeEnabled
        ? await Promise.resolve(
            ...
            console.warn("[llm-reply] failed to resolve exchange handoff notice setting:", err);
            return true;
          })
        : true;
    ...
    if (forcedExchangeReply) {
      ...
      const guardFinding = exchangeGuardEnabled ? exchangeGuardFindingFromResult(guarded) : null;
      if (this.opts.recordToolCalls && (forcedExchangeReply.toolCalls.length > 0 || guardFinding)) {
        try {
    ...
    const systemPrompt = [
      ...
      isExchange ? EXCHANGE_TOOLS_MANDATE : "",
    ...
    const msgs: ChatMessage[] = [{ role: "system", content: systemPrompt }, ...historyMessages];
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      const badAnswer = !guarded.ok || guarded.text.trim() === EXCHANGE_SAFE_FALLBACK;
      if (!badAnswer) break;
