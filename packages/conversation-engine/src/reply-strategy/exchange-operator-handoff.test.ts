...
describe("buildExchangeOperatorHandoff", () => {
  ...
  it("без состояния и сигналов → null", async () => {
    expect(buildExchangeOperatorHandoff({ text: "Спасибо, хорошего дня!" })).toBeNull();
  });
});

describe("parsePickupWindow", () => {
  it("accepts common destination keys", () => {
    expect(parsePickupWindow('{"pickupWindow":"15:00-16:00"}')).toBe("15:00-16:00");
    expect(parsePickupWindow('{"time_window":"after 18:00"}')).toBe("after 18:00");
