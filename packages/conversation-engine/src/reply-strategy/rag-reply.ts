...
export interface RagReplyStrategyOpts {
	...
	/**
	 * Optional current KB scope resolver. When set, retrieval searches stage
	 * docs first, then funnel docs, then global docs.
	 */
	 ...
	 * into requestContext so answerWithRag can ground offer/payment replies.
	 */
	...
	/**
	 * Optional post-generation recorder for agentic tool-loop traces. Called only
	 * when answerWithRag executed at least one tool. The hook should persist via
	 * its own short withTenant transaction; reply generation already completed.
	 */
...
export class RagReplyStrategy implements ReplyStrategy {
	...
	async generate(input: {
		...
		// Load persuasion skills, director hooks, agentic tools, reranker and policy state in parallel.
		...
		const [
			...
			this.opts.resolveSkills
				? this.opts.resolveSkills({ tenantId })
				: Promise.resolve([]),
		...
		const logExchangeGuardBlock = (
			...
			console.warn(
				`[exchange-policy-guard] tenant=${tenantId} conversation=${input.conversationId} reason=${guarded.reason ?? "unknown"} fallback=${guarded.text}`,
			);
		};
		...
		const groundingBlock =
			[
				serviceOrderContext,
				exchangeQuality && exchangePolicyState
		...
		const result = await answerWithRag({
			...
			rewriteQueryBeforeRetrieval:
				this.opts.rewriteQueryBeforeRetrieval ?? true,
			...
			...(groundingBlock
				? { vacanciesBlock: groundingBlock, vacancyGuard: false }
				: {}),
		});
		...
		if (hasToolCalls && this.opts.recordToolCalls) {
			await Promise.resolve(
				this.opts.recordToolCalls({
					...
					telemetry: {
						...(result.telemetry.toolCall
							? { toolCall: result.telemetry.toolCall }
							: {}),
