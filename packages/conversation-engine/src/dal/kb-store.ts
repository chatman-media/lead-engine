...
export class ScopedKbStore implements IKbStore {
  ...
  async search(embedding: number[], k: number, topic?: string | null): Promise<KbSearchHit[]> {
    return this.firstNonEmpty((scope) => this.inner.search(embedding, k, topic, scope));
  }
  ...
  async hybridSearch(input: {
    ...
    return this.firstNonEmpty((scope) => this.inner.hybridSearch({ ...input, scope }));
  }
  ...
  insertChunkWithEmbedding(input: Parameters<IKbStore["insertChunkWithEmbedding"]>[0]) {
