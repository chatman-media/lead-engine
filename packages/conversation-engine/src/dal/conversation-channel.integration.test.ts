...
describe("ConversationsRepo channel identity", () => {
  it("allows separate source=bot conversations for different real channels", async () => {
    ...
    expect((await repo().findByContactAndChannel(contactId, whatsappChannelId))?.id).toBe(
      whatsapp.id,
    );
    expect((await repo().findByContactAndChannel(contactId, webChannelId))?.id).toBe(web.id);
  ...
  it("keeps legacy source uniqueness only for channel_id=NULL rows", async () => {
    ...
    expect((await repo().findByContactAndSource(legacyContact.id, "bot"))?.id).toBe(legacy.id);
    await expect(
      repo().create({
        contactId: legacyContact.id,
        source: "bot",
