...
export interface ConversationRow {
  ...
  /** #735 — true = язык залип, менять только confident-сигналом другого скрипта. */
  langLocked: boolean;
}
...
export class ConversationsRepo {
  ...
  async bumpFallbackStreak(
    ...
    const rows = await this.ctx.db
      .update(conversationsTable)
      .set({
        fallbackStreak: isFallback ? sql`${conversationsTable.fallbackStreak} + 1` : sql`0`,
      })
      .where(
        and(
          eq(conversationsTable.id, conversationId),
   ...
   */
  async autoAssignLeastBusyOperator(conversationId: number): Promise<number | null> {
    const operator = await pickLeastBusyOperator(this.ctx.db, this.ctx.tenantId);
  ...
  /**
   * #735 — сохранить определённый язык диалога. lang=null — сбросить (например,
   * админ-операция); locked=true означает «менять только confident-сигналом
   * другого скрипта» (см. `resolveConversationLang`). Атомарный UPDATE без
   * предварительного SELECT — гонки между параллельными inbound'ами не критичны
   * (последний confident-детект побеждает; для НЕ confident detect-слой не
   * вызывает setDetectedLang вовсе).
   */
  async setDetectedLang(
    ...
    await this.ctx.db
      .update(conversationsTable)
      .set({ detectedLang: lang, langLocked: locked })
      .where(
        and(
          eq(conversationsTable.id, conversationId),
          eq(conversationsTable.tenantId, this.ctx.tenantId),
   ...
   */
  async findConversationByOperatorThread(threadId: number): Promise<ConversationRow | null> {
    const [row] = await this.ctx.db
      .select()
      .from(conversationsTable)
      .where(
  ...
  async applyAutoHandoff(input: {
    ...
    const changed =
      existing.mode !== "human" || existing.status !== "pending" || existing.escalatedAt == null;
    if (changed) {
      await this.ctx.db
        .update(conversationsTable)
        .set({
