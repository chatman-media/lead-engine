import { orderEvents, providerRequests, serviceOrders } from "@chatman-media/storage";
...
export type OrderEventActorType = "system" | "customer" | "provider" | "operator" | "payment";
...
const serviceOrderTransitions: Record<ServiceOrderStatus, readonly ServiceOrderStatus[]> = {
  intake: ["matching", "awaiting_provider", "cancelled", "failed"],
  matching: ["awaiting_provider", "provider_declined", "offer_ready", "cancelled", "failed"],
  awaiting_provider: ["provider_declined", "offer_ready", "cancelled", "failed"],
...
const providerRequestTransitions: Record<ProviderRequestStatus, readonly ProviderRequestStatus[]> =
  {
...
export class ProviderRelayRepo {
  ...
  async orderById(orderId: number): Promise<ServiceOrderRow | null> {
    const [row] = await this.ctx.db
      .select()
      .from(serviceOrders)
      .where(and(eq(serviceOrders.id, orderId), eq(serviceOrders.tenantId, this.ctx.tenantId)));
    return (row as ServiceOrderRow) ?? null;
  }

  async providerRequestById(requestId: number): Promise<ProviderRequestRow | null> {
    const [row] = await this.ctx.db
      .select()
      .from(providerRequests)
      .where(
        and(eq(providerRequests.id, requestId), eq(providerRequests.tenantId, this.ctx.tenantId)),
      );
  ...
  async createServiceOrder(opts: {
    ...
    const [row] = await this.ctx.db
      ...
      .values({
        ...
        ...(opts.quotedAmount !== undefined ? { quotedAmount: opts.quotedAmount } : {}),
        ...(opts.customerAmount !== undefined ? { customerAmount: opts.customerAmount } : {}),
        ...(opts.commissionPct !== undefined ? { commissionPct: opts.commissionPct } : {}),
        ...(opts.commissionAmount !== undefined ? { commissionAmount: opts.commissionAmount } : {}),
  ...
  async transitionOrderStatus(
    ...
    const [row] = await this.ctx.db
      ...
      .where(and(eq(serviceOrders.id, orderId), eq(serviceOrders.tenantId, this.ctx.tenantId)))
      .returning();
    if (!row) throw new Error("service_orders.transition: update returned no row");
    return row as ServiceOrderRow;
  }
  ...
  async addProviderRequest(opts: {
    ...
    const [row] = await this.ctx.db
      ...
      .values({
        ...
        ...(opts.outboundQueueId !== undefined ? { outboundQueueId: opts.outboundQueueId } : {}),
        ...(opts.currency ? { currency: opts.currency } : {}),
        ...(opts.quoteExpiresAt !== undefined ? { quoteExpiresAt: opts.quoteExpiresAt } : {}),
        ...(opts.idempotencyKey ? { idempotencyKey: opts.idempotencyKey } : {}),
        ...(opts.metadata ? { metadataJson: JSON.stringify(opts.metadata) } : {}),
      })
      .returning();
    if (!row) throw new Error("provider_requests.create: insert returned no row");

    await this.transitionOrderStatus(opts.orderId, "awaiting_provider", opts.nowEpoch);
    if (
      ...
      await this.ctx.db
        ...
        .where(
          and(eq(serviceOrders.id, opts.orderId), eq(serviceOrders.tenantId, this.ctx.tenantId)),
        );
    }
  ...
  async recordProviderQuote(opts: {
    ...
    const commissionAmount =
      ...
      (commissionPct !== null ? roundMoney((opts.quotedAmount * commissionPct) / 100) : null);
    ...
    const [updatedOrder] = await this.ctx.db
      ...
      .where(
        and(eq(serviceOrders.id, current.orderId), eq(serviceOrders.tenantId, this.ctx.tenantId)),
      )
      .returning();
    if (!updatedOrder) {
  ...
  async recordProviderDecline(opts: {
    ...
    await this.transitionOrderStatus(current.orderId, "provider_declined", opts.nowEpoch);
    await this.appendEvent({
      orderId: current.orderId,
      providerRequestId: opts.providerRequestId,
      actorType: "provider",
  ...
  async eventsForOrder(orderId: number, limit = 50): Promise<OrderEventRow[]> {
    const rows = await this.ctx.db
      .select()
      .from(orderEvents)
      .where(and(eq(orderEvents.tenantId, this.ctx.tenantId), eq(orderEvents.orderId, orderId)))
