...
describe("ContactsRepo", () => {
  it("create/byId/mergeAttributes respect tenant scope and merge shallow JSON", async () => {
    ...
    expect(await new ContactsRepo({ db, tenantId: otherTenantId }).byId(contact.id)).toBeNull();
    ...
    expect(await contactsRepo().mergeAttributes(999_999, { city: "Nowhere" }, now + 3)).toBeNull();
  });
});
...
describe("LeadsRepo", () => {
  it("create/find/update covers contact and request_type selectors", async () => {
    ...
    expect((await leadsRepo().byId(transfer.id))?.state).toBe("transfer_request");
    expect(await new LeadsRepo({ db, tenantId: otherTenantId }).byId(transfer.id)).toBeNull();
    expect((await leadsRepo().findByContactId(contact.id))?.id).toBe(food.id);
    expect((await leadsRepo().findAllByContact(contact.id)).map((l) => l.id)).toEqual([
      food.id,
      transfer.id,
    ]);
    expect((await leadsRepo().findByContactAndType(contact.id, "transfer"))?.id).toBe(transfer.id);
    expect(await leadsRepo().findByContactAndType(contact.id, "tour")).toBeNull();
...
describe("MessagesRepo", () => {
  it("insert/findUserByExternalId/recent/count handle dedupe fields and filtering", async () => {
    ...
    expect((await messagesRepo().findUserByExternalId(conversationId, "101"))?.id).toBe(user.id);
    expect(await messagesRepo().findUserByExternalId(conversationId, "not-a-number")).toBeNull();
    ...
    const summaryWindow = await messagesRepo().forConversationSummary(conversationId, {
  ...
  it("origLang on insert + setTranslation/getTranslation cache, tenant-scoped (#736)", async () => {
    if (!enabled) return;
    const contact = await contactsRepo().create({ displayName: "i18n contact" });
    const [conversation] = await db
      .insert(schema.conversations)
      .values({
        tenantId,
    ...
    // origLang пишется при insert; без него — null
    const withLang = await messagesRepo().insert({
      conversationId,
      role: "user",
      text: "안녕하세요",
      origLang: "ko",
    ...
    expect(withLang.origLang).toBe("ko");
    expect(withLang.translatedText).toBeNull();
    const noLang = await messagesRepo().insert({
      conversationId,
      role: "assistant",
      text: "reply",
      nowEpoch: now + 1,
    });
    expect(noLang.origLang).toBeNull();

    // нет перевода → getTranslation === null (кэш-семантика для #731)
    expect(await messagesRepo().getTranslation(withLang.id)).toBeNull();

    // setTranslation → getTranslation round-trip
    await messagesRepo().setTranslation(withLang.id, {
      text: "Здравствуйте",
      lang: "ru",
    });
    expect(await messagesRepo().getTranslation(withLang.id)).toEqual({
      text: "Здравствуйте",
      lang: "ru",
    });

    // tenant-isolation: чужой тенант не видит и не перезаписывает перевод
    const otherRepo = new MessagesRepo({ db, tenantId: otherTenantId });
    expect(await otherRepo.getTranslation(withLang.id)).toBeNull();
    await otherRepo.setTranslation(withLang.id, { text: "hijack", lang: "en" });
    expect(await messagesRepo().getTranslation(withLang.id)).toEqual({
      text: "Здравствуйте",
      lang: "ru",
    });
...
describe("SkillOutcomesRepo", () => {
  it("record is idempotent, byLeadId returns rows and aggregates map counts", async () => {
    ...
    expect((await outcomesRepo().byLeadId(lead.id)).map((row) => row.skillSlug).sort()).toEqual([
      "clarity",
      "empathy",
    ]);
...
describe("AgentToolCallsRepo", () => {
  it("recordMany persists tool traces and feedback stays tenant-scoped", async () => {
    ...
    expect((await agentToolCallsRepo().byId(rows[1]!.id))?.toolName).toBe("create_exchange_order");

    const byConversation = await agentToolCallsRepo().byConversation(conversation.id);
    ...
    expect(
      (await agentToolCallsRepo().feedbackForToolCall(rows[1]!.id)).map((row) => row.label),
...
describe("StylesRepo", () => {
  it("create/byId/findActiveBySlug/listActive/listAll/update/softDelete", async () => {
    ...
    expect(await new StylesRepo({ db, tenantId: otherTenantId }).byId(created.id)).toBeNull();
    ...
    expect(all.map((s) => s.slug)).toEqual(expect.arrayContaining(["warm", "cold"]));
    ...
    expect((await stylesRepo().listAll()).map((s) => s.slug)).not.toContain("warm");
    expect((await stylesRepo().byId(created.id))?.deletedAt).not.toBeNull();
    // soft-deleted строка больше не апдейтится.
    expect(await stylesRepo().update(created.id, { displayName: "zombie" })).toBeNull();
  });
});
