...
const env = (idem?: string): OutboundEnvelope =>
  ({ kind: "text", text: "hi", ...(idem ? { idempotencyKey: idem } : {}) }) as unknown as OutboundEnvelope;
...
beforeAll(async () => {
  ...
  const [t] = await db.insert(schema.tenants).values({ slug: `outq-${now}` }).returning({ id: schema.tenants.id });
  tenantId = t!.id;
  const [c] = await db
...
describe("OutboundQueueRepo", () => {
  it("enqueue → pending row", async () => {
    if (!enabled) return;
    const row = await repo().enqueue({ channelId, envelope: env(), nowEpoch: now });
  ...
  it("enqueue идемпотентен по idempotencyKey (без дубля)", async () => {
    if (!enabled) return;
    const a = await repo().enqueue({ channelId, envelope: env("idem-1"), nowEpoch: now });
    const b = await repo().enqueue({ channelId, envelope: env("idem-1"), nowEpoch: now });
    expect(b.id).toBe(a.id);
    const all = await db.select().from(schema.outboundQueue).where(eq(schema.outboundQueue.idempotencyKey, "idem-1"));
  ...
  it("claimPending не берёт строки из будущего (scheduled_at > now)", async () => {
    if (!enabled) return;
    await repo().enqueue({ channelId, envelope: env("future"), scheduledAt: now + 10_000, nowEpoch: now });
    const claimed = await repo().claimPending({ limit: 50, nowEpoch: now });
    expect(claimed.find((r) => r.payloadJson.includes("future"))).toBeUndefined();
  });

  it("claimPending с kinds-фильтром пропускает чужой kind", async () => {
    if (!enabled) return;
    await repo().enqueue({ channelId, envelope: env("kf"), nowEpoch: now });
    const claimed = await repo().claimPending({ limit: 50, nowEpoch: now, kinds: ["whatsapp"] });
    expect(claimed.find((r) => r.payloadJson.includes('"kf"'))).toBeUndefined();
  });

  it("markSent → status sent + externalMessageId + attempt++", async () => {
    if (!enabled) return;
    const row = await repo().enqueue({ channelId, envelope: env("sent"), nowEpoch: now });
    await repo().markSent(row.id, "ext-123", now);
    const [after] = await db.select().from(schema.outboundQueue).where(eq(schema.outboundQueue.id, row.id));
  ...
  it("markFailed → status failed + lastError + attempt++", async () => {
    if (!enabled) return;
    const row = await repo().enqueue({ channelId, envelope: env("fail"), nowEpoch: now });
  ...
  it("releaseStuckProcessing возвращает зависшие processing в pending", async () => {
    ...
    await repo().enqueue({ channelId, envelope: env("stuck"), scheduledAt: now - 1000, nowEpoch: now - 1000 });
    await repo().claimPending({ limit: 50, nowEpoch: now });
    const released = await repo().releaseStuckProcessing({ nowEpoch: now, stuckSec: 60 });
