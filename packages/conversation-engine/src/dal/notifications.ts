...
export class NotificationsRepo {
	...
	async findOperatorSettingsByTenant(
		tenantId: number,
	...
	async partialUpdateSettings(
		...
		tenantId: number,
		fields: Partial<
			Pick<OperatorSettings, "telegramChatId" | "notifyOnAssignedOnly">
		>,
	): Promise<void> {
	...
	async upsertTemplate(
		tpl: Omit<NotificationTemplate, "id" | "updatedAt">,
	): Promise<void> {
		await this.db
	...
	async findGroupLinkToken(
		...
		{ tenantId: number; adminId: number; eventType: string } | undefined
	> {
		const now = Math.floor(Date.now() / 1000);
		const rows = await this.db
			.select()
	...
	async deleteGroupLinkToken(token: string): Promise<void> {
		await this.db
			.delete(notificationGroupTokens)
			.where(eq(notificationGroupTokens.token, token));
	}
	...
	async findOperatorSettingsByChatId(
		chatId: string,
	): Promise<OperatorSettings | undefined> {
		const rows = await this.db
			.select()
