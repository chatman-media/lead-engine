...
describe("NotificationsRepo: rules", () => {
	it("createRule + findRulesByEvent (только активные) + listRules + deleteRule", async () => {
		...
		expect(
			(await repo.listRules(tenantId)).find((r) => r.id === active.id),
		).toBeUndefined();
	});
});
...
describe("NotificationsRepo: templates", () => {
	it("upsert (insert→update) + find + list + delete", async () => {
		...
		expect((await repo.findTemplate(tenantId, "stage_changed"))?.body).toBe(
			"v1",
		);
...
describe("NotificationsRepo: operator settings", () => {
	...
	it("linkChat проставляет chatId и чистит токен; резолв по adminId/chatId/tenant", async () => {
		...
		expect((await repo.findOperatorSettingsByChatId("chat-777"))?.adminId).toBe(
			adminId,
		);
		expect(
			(await repo.findOperatorSettingsByTenant(tenantId)).some(
				(x) => x.adminId === adminId,
			),
		).toBe(true);
	});
	...
	it("partialUpdateSettings и updateInformerPrefs (plain + upsert)", async () => {
		if (!enabled) return;
		await repo.partialUpdateSettings(adminId, tenantId, {
			notifyOnAssignedOnly: false,
		});
		expect(
			(await repo.findOperatorSettings(adminId))?.notifyOnAssignedOnly,
		).toBe(false);
		await repo.updateInformerPrefs(adminId, { informerLevel: "all" });
		expect((await repo.findOperatorSettings(adminId))?.informerLevel).toBe(
			"all",
		);
		// upsert: у admin2 строки ещё нет — tenantId создаёт её
		await repo.updateInformerPrefs(admin2, { informerDigest: "off" }, tenantId);
		expect((await repo.findOperatorSettings(admin2))?.informerDigest).toBe(
			"off",
		);
	});
	...
	it("upsertOperatorSettings обновляет chatId/флаг по конфликту adminId", async () => {
		...
		expect((await repo.findOperatorSettings(admin2))?.telegramChatId).toBe(
			"chat-upsert",
		);
	});
});
...
describe("NotificationsRepo: group tokens", () => {
	it("generate → find → delete", async () => {
		if (!enabled) return;
		const token = await repo.generateGroupLinkToken(
			tenantId,
			adminId,
			"high_value_deal",
		);
...
describe("NotificationsRepo: ledger + resolveOwnerSettings", () => {
	it("insert → dedup → list → pending → delivered/digested", async () => {
		...
		expect((await repo.findRecentByDedup(tenantId, "led-1", since))?.id).toBe(
			id,
		);
		// окно в будущем → не находим
		expect(
			await repo.findRecentByDedup(
				tenantId,
				"led-1",
		...
		expect(
			(await repo.listRecentNotifications(tenantId, adminId, 10)).some(
				(r) => r.id === id,
			),
		).toBe(true);
		expect(
			(await repo.listPendingDigest(tenantId, adminId)).some(
				(r) => r.id === id,
			),
		).toBe(true);
