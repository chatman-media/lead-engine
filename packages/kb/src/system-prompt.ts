...
import type { ReplyLang } from "./styles.ts";

/** Имя языка ответа (#730) для директивы промпта — на самом этом языке, чтобы
 *  инструкция была однозначной для многоязычной модели. Один источник правды
 *  для composeSystemPrompt (prompt.ts) и legacy buildSystemPrompt. */
export const REPLY_LANG_NAME: Record<ReplyLang, string> = {
  ru: "русском",
  en: "English",
  ko: "한국어",
  zh: "中文",
...
export function buildSystemPrompt(
  ...
  const langName = REPLY_LANG_NAME[lang];
  ...
  const personaLine =
    persona.role === "human"
      ...
      : `Тебя зовут ${persona.name}, ты ИИ-ассистент${company ? ` агентства ${company}` : ""}. ` +
        `Отвечай вежливо и по делу. О себе пиши в нейтральном грамматическом роде ` +
        `или в настоящем времени («готово», «рассчитываю», «уточняю»): не используй ` +
        `гендерно-окрашенные формы — ни глаголы прошедшего времени ` +
        `(«посчитал/посчитала», «получил/получила»), ни краткие прилагательные ` +
        `(«готов/готова», «рад/рада», «уверен/уверена»). Например, вместо ` +
        `«всегда готов помочь» пиши «всегда на связи» или «обращайтесь». ` +
        `Держи единый стиль во всех сообщениях.`;

  const conversational =
    persona.role === "human"
      ? `\nЖИВАЯ РЕЧЬ (Telegram):\n` +
        `- Пиши так, чтобы это выглядело как переписка с реальным менеджером: естественные ` +
        `опорные слова, можно «ок/принято», «если вкратце», «по контрактам у нас…» — ` +
  ...
  const rules =
    ...
    `по-человечески. Если в CONTEXT есть конкретика по вопросу — названия, ` +
    `адреса, города, часы работы, перечни вариантов — приводи её дословно и ` +
    `полностью, не подменяй обобщением («один из наших офисов» вместо списка ` +
    `офисов). Не используй общие знания о мире, не сочиняй цифры, цены, ` +
    `сроки, города, названия стран, которых нет в CONTEXT — приводи только то, ` +
    `что реально есть в CONTEXT. ` +
    `CONTEXT может быть на ДРУГОМ ЯЗЫКЕ (например, по-английски) — это нормальный ` +
    `источник: если факты по теме вопроса есть в CONTEXT на любом языке, переведи ` +
    `их на язык ответа (${langName}) и ответь по сути; НЕ сваливайся на маркер только ` +
    `из-за языка источника. Имена собственные (города, офисы, банки) передавай как ` +
    `принято на языке ответа, не искажая. ` +
    ...
    `3. Пиши ТОЛЬКО на языке ответа (${langName}), даже если вопрос задан на другом. ` +
