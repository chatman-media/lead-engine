...
export const conversations = pgTable(
  "conversations",
  {
    ...
    channelId: integer("channel_id").references(
      (): import("drizzle-orm/pg-core").AnyPgColumn => channels.id,
      { onDelete: "set null" },
    ),
  ...
  (t) => [
    ...
    uniqueIndex("uniq_conversations_user_source")
      .on(t.userId, t.source)
      .where(sql`${t.channelId} IS NULL`),
    uniqueIndex("uniq_conversations_user_channel")
...
export const tenants = pgTable(
  ...
  (t) => [
    ...
    check(
      "tenants_reply_history_limit_check",
      sql`${t.replyHistoryLimit} IS NULL OR (${t.replyHistoryLimit} >= 2 AND ${t.replyHistoryLimit} <= 100)`,
    ),
    check(
      "tenants_reply_delay_seconds_check",
      sql`${t.replyDelaySeconds} IS NULL OR (${t.replyDelaySeconds} >= 0 AND ${t.replyDelaySeconds} <= 120)`,
    ),
  ],
);
...
export const funnelVersions = pgTable(
  ...
  (t) => [
    check(
      ...
      sql`${t.source} IN ('ai_apply','template_apply','manual_edit','rollback')`,
    ),
    index("idx_funnel_versions_funnel_created").on(
      t.tenantId,
      t.funnelId,
      sql`${t.createdAt} DESC`,
    ),
...
export const agentToolCallImprovementProposals = pgTable(
  ...
  (t) => [
    ...
    index("idx_agent_tool_call_improvement_status").on(
      t.tenantId,
      t.status,
      sql`${t.updatedAt} DESC`,
    ),
    index("idx_agent_tool_call_improvement_tool").on(
      t.tenantId,
      t.toolName,
      sql`${t.updatedAt} DESC`,
    ),
    index("idx_agent_tool_call_improvement_resolution").on(
      t.tenantId,
      t.resolutionKind,
      sql`${t.updatedAt} DESC`,
    ),
...
export const agentToolCallRegressionCases = pgTable(
  ...
  (t) => [
    ...
    index("idx_agent_tool_call_regression_status").on(
      t.tenantId,
      t.status,
      sql`${t.updatedAt} DESC`,
    ),
    index("idx_agent_tool_call_regression_tool").on(
      t.tenantId,
      t.toolName,
      sql`${t.updatedAt} DESC`,
    ),
...
export const simPersonas = pgTable(
  "sim_personas",
  {
    id: serial("id").primaryKey(),
    tenantId: integer("tenant_id")
...
export const exchangePayoutPoints = pgTable(
  "exchange_payout_points",
  {
    id: serial("id").primaryKey(),
    tenantId: integer("tenant_id")
      .notNull()
  ...
  (t) => [
    ...
    uniqueIndex("uniq_exchange_payout_points_external")
...
export const operatorPayoutCoverage = pgTable(
  "operator_payout_coverage",
  {
    id: serial("id").primaryKey(),
    tenantId: integer("tenant_id")
      .notNull()
    ...
    adminId: integer("admin_id")
      .notNull()
      .references(() => admins.id, { onDelete: "cascade" }),
    payoutPointId: integer("payout_point_id")
      .notNull()
      .references(() => exchangePayoutPoints.id, { onDelete: "cascade" }),
    createdAt: integer("created_at").notNull().default(epochNow()),
  },
  (t) => [
...
export const exchangeRateProposals = pgTable(
  "exchange_rate_proposals",
  {
    id: serial("id").primaryKey(),
    tenantId: integer("tenant_id")
      .notNull()
  ...
  (t) => [
    ...
    check(
      "exchange_rate_proposals_status_check",
      sql`${t.status} IN ('pending','confirmed','rejected','superseded')`,
    ),
    ...
    uniqueIndex("uniq_exchange_rate_proposals_pending_dir")
      ...
      .where(sql`status = 'pending'`),
    index("idx_exchange_rate_proposals_tenant_status").on(
      t.tenantId,
      t.status,
      sql`${t.createdAt} DESC`,
    ),
...
export const exchangeOrders = pgTable(
  "exchange_orders",
  {
    ...
    payoutPointId: integer("payout_point_id").references(() => exchangePayoutPoints.id, {
      onDelete: "set null",
    }),
...
export const partnerDeals = pgTable(
  "partner_deals",
  {
    ...
    proofJson: text("proof_json"),
    notes: text("notes"),
    settlementId: integer("settlement_id").references(() => partnerSettlements.id, {
      onDelete: "set null",
    }),
    sentAt: integer("sent_at"),
...
export const serviceOrders = pgTable(
  "service_orders",
  {
    ...
    customerContactId: integer("customer_contact_id")
      .notNull()
      .references(() => contacts.id, { onDelete: "cascade" }),
    customerConversationId: integer("customer_conversation_id").references(() => conversations.id, {
      onDelete: "set null",
    }),
    leadId: integer("lead_id").references(() => leads.id, { onDelete: "set null" }),
    assignedProviderId: integer("assigned_provider_id").references(() => providerProfiles.id, {
      onDelete: "set null",
    }),
...
export const providerRequests = pgTable(
  "provider_requests",
  {
    ...
    providerConversationId: integer("provider_conversation_id").references(() => conversations.id, {
      onDelete: "set null",
    }),
...
export const orderEvents = pgTable(
  "order_events",
  {
    ...
    orderId: integer("order_id")
      .notNull()
      .references(() => serviceOrders.id, { onDelete: "cascade" }),
    providerRequestId: integer("provider_request_id").references(() => providerRequests.id, {
      onDelete: "set null",
    }),
    conversationId: integer("conversation_id").references(() => conversations.id, {
      onDelete: "set null",
