...
export class VkClient {
  ...
  constructor(opts: VkClientOptions) {
    ...
    this.baseUrl = (opts.baseUrl ?? "https://api.vk.com/method").replace(/\/{1,512}$/, "");
  ...
  private async call<T>(
    ...
    if (parsed.error) {
      throw new VkApiError(method, res.status, parsed.error.error_msg, parsed.error.error_code);
    }
  ...
  async getGroupInfo(groupId: string | number): Promise<VkGroupInfo> {
    const response = await this.call<VkRawGroup[] | { groups?: VkRawGroup[] }>("groups.getById", {
      group_id: groupId,
    });
    const groups = Array.isArray(response) ? response : (response.groups ?? []);
    const group = groups[0];
    if (!group?.id) throw new VkApiError("groups.getById", 200, "group not found");
    return {
      id: group.id,
  ...
  async downloadMedia(url: string): Promise<Response> {
    ...
    if (!res.ok) {
      throw new VkApiError("downloadMedia", res.status, await res.text().catch(() => "no body"));
    }
    return res;
  }
}
