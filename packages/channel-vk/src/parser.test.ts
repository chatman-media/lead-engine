...
describe("parseCallbackPayload (VK)", () => {
  ...
  it("skips confirmation and non-message events", () => {
    expect(parseCallbackPayload(CH, { type: "confirmation", group_id: 1 })).toEqual([]);
    expect(parseCallbackPayload(CH, { type: "message_reply", group_id: 1 })).toEqual([]);
  });
  ...
  it("invalid object returns empty", () => {
    expect(parseCallbackPayload(CH, { type: "message_new", object: {} })).toEqual([]);
  });
});
