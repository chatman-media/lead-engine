...
console.log(`[test] persona=${config.persona.name}/${config.persona.role}/${config.persona.company}`);
...
for (const h of res.hits) {
  ...
  console.log(`  - chunk=${h.chunk_id} dist=${h.distance.toFixed(4)} title="${h.title}" :: ${preview}…`);
}
console.log("\n[test] ANSWER:");
...
const banned = ["оператор", "operator", "коллег", "ассистент", "as an ai", "я бот", "я ии", "i am an ai"];
const found = banned.filter((w) => lower.includes(w));
console.log("\n[test] banned-word check:", found.length === 0 ? "OK" : `LEAKED: ${found.join(", ")}`);
