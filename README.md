...
Telegram, WhatsApp, Messenger, VK, MAX, and a web widget — it walks a person from "just curious"
...
| App | What it is |
|---|---|
| `apps/api` | HTTP server: webhook handlers (telegram / whatsapp / facebook / vk / max / stripe), `/ws/:slug` (web), the full admin API, `/metrics`, `/healthz` |
...
`storage` (Drizzle schema + migrations), `channel-{core,telegram,whatsapp,facebook,vk,max,web}`,
