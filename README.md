...
минимальная реализация. Текущее состояние: **185 unit + 14 e2e зелёных, 0 падающих.**

**Sales-style engine** (опционально): подключается флагом `BOT_SALES_STYLE` —
вместо одной env-персоны бот использует pluggable стили (флирт-рекрутер,
...
Подробности и список доступных стилей — [docs/SALES_STYLES.md](docs/SALES_STYLES.md).
