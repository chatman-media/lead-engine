...
Open `http://localhost:5173/signup` → create a tenant → walk through the
...
10. apps/worker (TG/WA/userbot) or apps/api (web) drain outbound_queue
    via SKIP LOCKED → adapter.send → mark sent.
```
