...
describe("scooter_v1 template", () => {
  ...
  it("funnelSeedKey === 'scooter'", () => {
    const t = defaultRegistry.load("scooter_v1");
    expect(t.funnelSeedKey).toBe("scooter");
  });

  it("воронка содержит active_rental, returned, cancelled", () => {
    const t = defaultRegistry.load("scooter_v1");
    const slugs = new Set(t.funnelStages.map((s) => s.slug));
    expect(slugs.has("active_rental")).toBe(true);
    expect(slugs.has("returned")).toBe(true);
    expect(slugs.has("cancelled")).toBe(true);
  });
});
