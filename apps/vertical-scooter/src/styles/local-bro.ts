...
export const localBro: Style = StyleSchema.parse({
  ...
  stages: {
    opener: {
      ...
      guidance: "«Привет! Что ищем — автомат или механику? На сколько дней?» Одна реплика, максимум 2 вопроса.",
      maxTurns: 1,
    },
    qualify: {
      goal: "узнать даты, тип байка, есть ли права",
      guidance: "NEPQ: «Куда планируешь ездить?», «Раньше катался?» По одному вопросу. Советуй по ходу.",
      maxTurns: 4,
    },
    pitch: {
      goal: "предложить конкретный байк и цену",
      guidance: "Конкретная модель + цена в день + условия. Упомяни плюсы выбора под запрос клиента.",
      maxTurns: 2,
    },
    close: {
