...
describe("visa_v1 template", () => {
  it("регистрируется в defaultRegistry при import", () => {
    const t = defaultRegistry.load("visa_v1");
    expect(t.displayName).toContain("Визовое");
  ...
  it("funnelSeedKey === 'visa'", () => {
    const t = defaultRegistry.load("visa_v1");
    expect(t.funnelSeedKey).toBe("visa");
  });

  it("воронка содержит processing, visa_issued, rejected", () => {
    const t = defaultRegistry.load("visa_v1");
    const slugs = new Set(t.funnelStages.map((s) => s.slug));
    expect(slugs.has("processing")).toBe(true);
    expect(slugs.has("visa_issued")).toBe(true);
    expect(slugs.has("rejected")).toBe(true);
  });
});
