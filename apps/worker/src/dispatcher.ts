...
export class OutboundDispatcher {
	...
	constructor(
		...
		private readonly opts: {
			...
			/** Operator/admin notifications for relay dispatch failures. */
	...
	private async deliverOne(row: OutboundQueueRow): Promise<void> {
		...
		if (!entry) {
			await this.markFailed(
				row,
				`no active adapter for channel_id=${row.channelId}`,
			);
	...
	private async markFailed(
		...
		const now = Math.floor(Date.now() / 1000);
		const providerRequest = await withTenant(
			this.db,
			row.tenantId,
			async (tx) => {
				const txRepo = new OutboundQueueRepo({
				...
				const relayRepo = new ProviderRelayRepo({
					db: tx,
					tenantId: row.tenantId,
				});
				return relayRepo.recordProviderRequestSendFailedForOutbound({
		...
		await this.notifyProviderRequestSendFailed(row, error, providerRequest);
	}

	private async notifyProviderRequestSendFailed(
		row: OutboundQueueRow,
		error: string,
		providerRequest: ProviderRequestRow | null,
	): Promise<void> {
		if (!providerRequest || !this.opts.notifications) return;
		await this.opts.notifications
			.notify({
				tenantId: row.tenantId,
			...
			.catch((err) => {
				console.error(
					"[dispatcher] provider request failure notification error",
					err,
				);
			});
	}
