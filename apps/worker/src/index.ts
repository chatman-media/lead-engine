...
async function main() {
	...
	if (cfg.staleSweepIntervalMs > 0) {
		...
		staleSweeperPromise = sweeper.run(abort.signal).catch((err) => {
			log.error("stale-sweep fatal", {
				err: err instanceof Error ? err : new Error(String(err)),
			});
		});
		log.info("stale-lead sweep enabled", {
			intervalMs: cfg.staleSweepIntervalMs,
		});
	}
	...
	if (cfg.checkinSweepIntervalMs > 0) {
		...
		log.info("check-in sweep enabled", {
			intervalMs: cfg.checkinSweepIntervalMs,
		});
	}
	...
	if (cfg.exchangePaymentSweepMs > 0) {
		...
		exchangePaymentSweeperPromise = exchangeSweeper
			.run(abort.signal)
			.catch((err) => {
		...
		log.info("exchange payment-TTL sweep enabled", {
			intervalMs: cfg.exchangePaymentSweepMs,
		});
	}
