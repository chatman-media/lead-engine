 ...
 * ветвится по типу запроса (обмен / трансфер / еда), каждая ветка — короткий
 * воркфлоу qualify → offer → fulfill, все ветки сходятся в общие терминалы
 * (completed / cancelled). Это одна валидная воронка костяка (ровно один
 * intake, монотонные фазы, один won + один lost) — не N отдельных воронок.
 *
 ...
 *   request_received → {exchange|transfer|food}_request → *_offer → *_fulfill
 *   → completed | cancelled.
 */
export const CONCIERGE_FUNNEL_STAGES: readonly FunnelStageDef[] = [
  {
    ...
    next: ["exchange_request", "transfer_request", "food_request", "cancelled"],
  },
