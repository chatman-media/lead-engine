...
export interface ServiceCatalogRuntime {
  extract(opts: ServiceCatalogRuntimeInput): Promise<ServiceCatalogRuntimeResult>;
}
...
const SERVICE_SYNONYMS: Record<string, string[]> = {
  ...
  massage: ["massage", "spa", "therapist", "thai", "deep tissue", "массаж", "спа", "мастер"],
  beauty: [
    "beauty",
    "salon",
    "hair",
...
export function makeServiceCatalogRuntime(
  ...
  return {
    async extract(input) {
      ...
      const heuristicRequestTypes = new Set(heuristicMatches.map((match) => match.requestType));
          ...
          if (existing) {
            if (!existing.requestType) {
              await tx
                ...
                .where(and(eq(leads.tenantId, input.tenantId), eq(leads.id, existing.id)));
            }
            skipped.push({
              serviceSlug: item.slug,
            ...
            const [deal] = await tx
              ...
              .values({
                ...
                commissionPct: Number(
                  item.partnerServiceCommissionPct ?? item.partnerDefaultCommissionPct ?? 0,
                ),
...
export function deterministicCatalogMatches(
  ...
  for (const item of catalog) {
    ...
    for (const token of importantTokens(item)) {
      ...
      if (haystack.includes(token) && normalizedText.includes(token)) score += 1;
    }
...
async function resolveMatches(opts: {
  ...
  const heuristic = opts.heuristic ?? deterministicCatalogMatches(opts.text, opts.catalog);
      ...
      return {
        ...
        fields:
          req.fields && typeof req.fields === "object" && !Array.isArray(req.fields)
            ...
            : {},
        ...(typeof req.note === "string" ? { note: req.note.slice(0, 300) } : {}),
        source: "llm",
      };
    })
...
function explicitTargetFunnelId(item: CatalogRow, target: StageTarget): number | null {
...
async function resolveStageTarget(
  ...
  if (item.partnerServiceFunnelId) {
    const stage = await firstStageInFunnel(tx, tenantId, item.partnerServiceFunnelId);
    if (stage) return stage;
  }
...
async function writeFieldValues(opts: {
  ...
  const defs = await opts.tx
    ...
    .from(stageFields)
    .where(and(eq(stageFields.tenantId, opts.tenantId), eq(stageFields.stageId, opts.stageId)));
...
function requestTypeForCatalogItem(item: CatalogMatchInput): string {
  return inferServiceKind(item) ?? (normalizeSlug(item.slug || item.name) || "service");
}
...
function importantTokens(item: CatalogMatchInput): string[] {
  const tokens = normalizeText(`${item.slug} ${item.name} ${item.category ?? ""}`)
    .split(/\s+/)
...
function dedupeMatches(matches: ServiceRequestMatch[]): ServiceRequestMatch[] {
  ...
  return [...byRequestType.values()].sort((a, b) => b.confidence - a.confidence).slice(0, 8);
}
  ...
  try {
    ...
    return {
      ...(Array.isArray(parsed.requiredFields)
        ? {
            requiredFields: parsed.requiredFields.filter((v): v is string => typeof v === "string"),
          }
        : {}),
      ...(typeof parsed.handoffMode === "string" ? { handoffMode: parsed.handoffMode } : {}),
      ...(typeof parsed.handoffUrl === "string" ? { handoffUrl: parsed.handoffUrl } : {}),
    };
  } catch {
    return {};
  }
...
function readHandoffMode(item: CatalogRow): "fire_and_forget" | "await_callback" {
