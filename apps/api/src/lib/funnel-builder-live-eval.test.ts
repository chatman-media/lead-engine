...
describe("funnel-builder live eval scoring", () => {
  ...
  it("fails invalid JSON with concrete parse error", () => {
    ...
    expect(result.errors.some((error) => error.startsWith("Parse:"))).toBe(true);
  });
    ...
    const result = scoreFunnelOutput(
      ...
      JSON.stringify({
        ...
        stages: [
          ...
          stage("transfer_request", "Трансфер: детали", "qualify", ["transfer_offer"]),
          stage("food_request", "Еда: детали", "qualify", ["food_offer"]),
          stage("transfer_offer", "Трансфер: оффер", "offer", ["completed", "cancelled"]),
          stage("food_offer", "Еда: оффер", "offer", ["completed", "cancelled"]),
  ...
  it("JSON без stages → parseError 'stages array missing', reply сохраняется", () => {
    const parsed = parseFunnelBuilderOutput('{"reply":"расскажи подробнее про бизнес"}');
    expect(parsed.parseError).toBe("stages array missing");
    expect(parsed.reply).toBe("расскажи подробнее про бизнес");
    expect(parsed.stages).toEqual([]);

    const result = scoreFunnelOutput(linearScenario, '{"reply":"мало данных"}');
    expect(result.passed).toBe(false);
...
function terminal(slug: string, displayName: string, kind: "terminal_won" | "terminal_lost") {
  return {
    slug,
    displayName,
    kind,
