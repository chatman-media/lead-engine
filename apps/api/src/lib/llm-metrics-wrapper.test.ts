...
function fakeChat(
	behaviour: { reply?: string; throwError?: Error } = {},
): ChatClient {
	return {
...
function fakeEmbed(
	behaviour: { vec?: number[]; throwError?: Error } = {},
): EmbeddingClient {
	return {
		dim: 4,
...
describe("wrapChatClient", () => {
	...
	it("прокидывает return value наверх", async () => {
		const metrics = makePlatformMetrics();
		const wrapped = wrapChatClient(
			fakeChat({ reply: "structured-reply" }),
			metrics,
			{
				provider: "openai",
				purpose: "chat",
	...
	it("exception → llmErrors с kind=err.name, и re-throws", async () => {
		...
		const wrapped = wrapChatClient(
			fakeChat({ throwError: new CustomError() }),
			metrics,
			{
				provider: "openai",
				purpose: "chat",
		...
		await expect(
			wrapped.complete([{ role: "user", content: "x" }]),
	...
	it("разные purpose labels — отдельные counter-series", async () => {
		const metrics = makePlatformMetrics();
		const chat = wrapChatClient(fakeChat(), metrics, {
			provider: "openai",
			purpose: "chat",
		});
	...
	it("non-Error throws → kind='unknown'", async () => {
		...
		const w = wrapChatClient(wrapped, metrics, {
			provider: "ollama",
			purpose: "chat",
		});
		await expect(w.complete([{ role: "user", content: "x" }])).rejects.toBe(
			"string error",
		);
...
describe("wrapChatClient — optional methods forwarded", () => {
	it("completeWithTools — инкрементит llmCalls, пробрасывает результат", async () => {
		...
		const w = wrapChatClient(innerWithTools, metrics, {
			provider: "openai",
			purpose: "chat",
		});
	...
	it("completeWithTools error → llmErrors + re-throws", async () => {
		...
		const w = wrapChatClient(innerErr, metrics, {
			provider: "openai",
			purpose: "chat",
		});
		await expect(w.completeWithTools!([], [], {})).rejects.toThrow(
			"tools-down",
		);
	...
	it("completeStructured — пробрасывает напрямую (без обёртки метрик)", async () => {
		...
		const w = wrapChatClient(innerStruct, metrics, {
			provider: "openai",
			purpose: "chat",
		});
	...
	it("stream — пробрасывается когда inner поддерживает", async () => {
		...
		const w = wrapChatClient(innerStream, metrics, {
			provider: "openai",
			purpose: "chat",
		});
		const chunks: string[] = [];
	...
	it("клиент без completeWithTools → wrapper не добавляет метод", () => {
		const metrics = makePlatformMetrics();
		const w = wrapChatClient(fakeChat(), metrics, {
			provider: "openai",
			purpose: "chat",
		});
	...
	it("onComplete callback вызывается с latencyMs и success=true", async () => {
		...
		const w = wrapChatClient(
			...
			(info) =>
				calls.push({ success: info.success, latencyMs: info.latencyMs }),
		);
	...
	it("onComplete callback вызывается с success=false при ошибке", async () => {
		...
		await expect(
			w.complete([{ role: "user", content: "x" }]),
		).rejects.toThrow();
		expect(calls).toHaveLength(1);
