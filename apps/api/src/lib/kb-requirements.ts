...
export type KbRequirementDraft = Omit<
	KbRequirement,
	"covered" | "matchedDocuments"
>;
...
function addRequirement(
	map: Map<string, KbRequirementDraft>,
	req: KbRequirementDraft,
) {
...
export function buildKbRequirementDrafts(input: {
	...
	const isExchange =
		funnel.slug.includes("exchange") ||
		funnel.verticalTemplateId === "exchange_v1";

	addRequirement(out, {
		key: "business_overview",
		title: "Что делает бизнес",
		description:
			"Краткое описание услуги, кому она подходит, что бот может обещать клиенту.",
	...
	addRequirement(out, {
		...
		description:
			"Как проходит заявка по шагам, типичные сроки, где нужен оператор.",
		topic: "process",
		required: true,
		scopeType: "funnel",
		funnelId: funnel.id,
	...
	addRequirement(out, {
		...
		description:
			"Что можно называть клиенту, где брать цену, какие лимиты/комиссии/исключения.",
		topic: "pricing",
		required: true,
		scopeType: "funnel",
		funnelId: funnel.id,
		...
		for (const req of [
			{
				...
				description:
					"Пошаговая инструкция по RUB/crypto оплате, что прислать как proof.",
				topic: "how_to_pay",
			},
			{
				...
				description:
					"Когда нужна верификация, какие документы, что делать при high-risk.",
				topic: "kyc_aml",
			},
			{
				key: "exchange_payout",
				...
				description:
					"Офис, cardless ATM, курьер, банковский перевод, зоны и ограничения.",
				topic: "payout",
			},
			{
				...
				description:
					"Адреса, часы работы, районы, как выбрать ближайшую точку.",
				topic: "locations",
			},
		]) {
			addRequirement(out, {
	...
	for (const stage of stages) {
		...
		if (
			...
			addRequirement(out, {
				...
				description:
					"Какие документы нужны, формат файлов, кто проверяет и что делать при ошибке.",
				topic: "documents",
				required: true,
				scopeType: "stage",
				funnelId: funnel.id,
		...
		if (
			...
			stage.stageType === "external_approval"
		) {
			addRequirement(out, {
		...
		if (stage.stageType === "rate_confirmation") {
			addRequirement(out, {
				...
				description:
					"Источник курса, срок фиксации, что делать при торге или устаревшем курсе.",
				topic: "rates",
				required: true,
				scopeType: "stage",
				funnelId: funnel.id,
