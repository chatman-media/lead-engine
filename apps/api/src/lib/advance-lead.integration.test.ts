...
beforeAll(async () => {
  ...
  intakeId = await makeStage(funnelId, "intake", "intake", "form_fill", 0, ["qualify"]);
  qualifyId = await makeStage(funnelId, "qualify", "active", "form_fill", 1, ["op"]);
...
async function makePartnerFixture(
  ...
  const sourceId = await makeStage(funnelId, sourceSlug, "active", "form_fill", 30, [partnerSlug]);
...
describe("advanceLead", () => {
  ...
  it("без note: стадию двигаем молча — клиенту не шлём внутренние названия этапов", async () => {
    ...
    const msgs = await db
      ...
      .from(messages)
      .where(and(eq(messages.tenantId, tenantId), eq(messages.conversationId, conversation.id)));
  ...
  it("вход в awaiting_operator → notify operator_confirm_needed", async () => {
    ...
    expect(
      events.some((e) => e.eventType === "operator_confirm_needed" && e.leadId === leadId),
    ).toBe(true);
  });
  ...
  it("partner stage → создаёт deal, отправляет ping и сохраняет callback token", async () => {
    ...
    const [deal] = await db
      ...
      .from(partnerDeals)
      .where(and(eq(partnerDeals.tenantId, tenantId), eq(partnerDeals.leadId, fixture.leadId)));
    expect(deal).toEqual({
    ...
    expect(body.actions.confirm).toBe(`https://app.test/api/partner/cb/${awaitingToken}?a=confirm`);
    expect(body.actions.cancel).toBe(`https://app.test/api/partner/cb/${awaitingToken}?a=cancel`);
  });
  ...
  it("ошибка partner ping не откатывает продвижение лида", async () => {
    ...
    expect(errors.some((args) => args[0] === "[partner-ping] failed")).toBe(true);
  });
});
