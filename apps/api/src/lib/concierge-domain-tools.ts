	...
	return {
		...
		parameters: z.object({
			...
			datetime: z
				.string()
				.optional()
				.describe("Дата и время поездки (строка от гостя)"),
			passengers: z
				.number()
				.int()
				.positive()
				.optional()
				.describe("Количество пассажиров"),
		}),
		execute: async (args) => {
			...
			const available = args.passengers
				? catalog.transfer.classes.filter(
						(c) => !c.capacity || c.capacity >= args.passengers!,
					)
	...
	return {
		...
		parameters: z.object({
			...
			notes: z
				.string()
				.optional()
				.describe("Пожелания гостя (детское кресло, багаж и т.п.)"),
		}),
		execute: async (args) => {
			const rt = await resolveCurrentRequestType(db, tenantId, conversationId);
			if (rt && rt !== "transfer") {
			...
			const catalog = await loadConciergeCatalog(db, tenantId, masterKeyHex);
			const cls = catalog?.transfer?.classes?.find(
				(c) => c.id === args.classId,
			);
			if (!cls) {
				return {
	...
	return {
		...
		parameters: z.object({
			category: z
				.string()
				.optional()
				.describe("Категория для фильтрации (пицца, суши, напитки…)"),
		}),
		execute: async (args) => {
			const rt = await resolveCurrentRequestType(db, tenantId, conversationId);
			if (rt && rt !== "food") {
			...
			if (args.category) {
				...
				items = items.filter(
					(i) =>
						...
						i.name.toLowerCase().includes(cat),
				);
			}
			return {
				items: items.map((i) => ({
				...
				...(catalog.food.deliveryFee !== undefined
					...
					: {}),
				...(catalog.food.etaMinutes
					? { etaMinutes: catalog.food.etaMinutes }
					: {}),
			};
		},
	};
	...
	return {
		...
		parameters: z.object({
			...
			deliveryLocation: z
				.string()
				.optional()
				.describe("Куда доставить: номер, корпус, адрес"),
			notes: z
				.string()
				.optional()
				.describe("Пожелания: без лука, аллергии и т.п."),
		}),
		execute: async (args) => {
			const rt = await resolveCurrentRequestType(db, tenantId, conversationId);
			if (rt && rt !== "food") {
...
function makeGetHousekeepingOptionsTool(
	deps: ConciergeDomainToolsDeps,
): AnyRagTool {
	const { db, tenantId, masterKeyHex, conversationId } = deps;
	return {
		...
		parameters: z.object({
			date: z
				.string()
				.optional()
				.describe("Желаемая дата уборки (строка от гостя)"),
			hours: z
				.number()
				.positive()
				.optional()
				.describe("Сколько часов планирует (если назвал)"),
		}),
		execute: async (args) => {
			const rt = await resolveCurrentRequestType(db, tenantId, conversationId);
			...
			return {
				...
				...(hk.availableSlots?.length
					? { availableSlots: hk.availableSlots }
					: {}),
...
function makeConfirmHousekeepingTool(
	deps: ConciergeDomainToolsDeps,
): AnyRagTool {
	const { db, tenantId, masterKeyHex, conversationId } = deps;
	return {
		...
		parameters: z.object({
			...
			timeSlot: z
				.string()
				.optional()
				.describe("Временной слот (из get_housekeeping_options)"),
	...
	return {
		...
		parameters: z.object({
			participants: z
				...
				.positive()
				.optional()
				.describe("Количество участников"),
		...
		execute: async (args) => {
			...
			return {
				options: options.map((o) => ({
					...
					...(args.participants
						? { totalPrice: o.pricePerPerson * args.participants }
						: {}),
				})),
				...(args.date ? { requestedDate: args.date } : {}),
...
function makeConfirmTourBookingTool(
	deps: ConciergeDomainToolsDeps,
): AnyRagTool {
	const { db, tenantId, masterKeyHex, conversationId } = deps;
	return {
 ...
 */
export function makeConciergeDomainTools(
	deps: ConciergeDomainToolsDeps,
): AnyRagTool[] {
	return [
