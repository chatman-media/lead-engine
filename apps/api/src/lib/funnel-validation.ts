...
import { type ActivePhase, deriveDefaultPhase, validateBackbone } from "@chatman-media/verticals";
...
export function multiRequestBranchErrors(stages: SeedStage[]): string[] {
  ...
  if (requestType.optionsJson) {
    try {
      values = (JSON.parse(requestType.optionsJson) as Array<{ value?: unknown }>)
...
export function workflowBehaviorWarnings(stages: SeedStage[]): string[] {
  ...
  for (const stage of stages) {
    ...
    if (stage.nextStages.length === 0) {
      warnings.push(`Стадия "${stage.slug}": нет nextStages — стадия не знает следующий шаг.`);
    }
    const hasExecutableExit =
      stageWorkflowTransitions(stage.configJson).length > 0 ||
      parseAutoAdvanceConditionType(stage.autoAdvanceCondition) === "all_required_fields_filled" ||
...
export function normalizeSeedStageConfigJson(opts: {
  ...
  if (
    ...
    parseAutoAdvanceConditionType(opts.autoAdvanceCondition) === "all_required_fields_filled" &&
