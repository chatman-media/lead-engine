...
async function seedFixture(): Promise<Fixture> {
  return withTenant(db as Db, tenantId, async (tx) => {
    ...
    const transferService = expectInserted(insertedTransferService, "transfer service");
...
describe("service catalog runtime", () => {
  ...
  it("matches multiple services deterministically from one message", () => {
    ...
    expect(matches.map((match) => match.requestType).sort()).toEqual(["cleaning", "transfer"]);
  });
  ...
  it("creates one lead per requested service and a partner deal for provider services", async () => {
    ...
    const leadRows = await withTenant(db as Db, tenantId, (tx) =>
      tx
        ...
        .from(leads)
        .where(and(eq(leads.tenantId, tenantId), eq(leads.userId, fixture.contactId)))
        .orderBy(asc(leads.id)),
    );
    expect(leadRows.map((lead) => lead.requestType).sort()).toEqual(["cleaning", "transfer"]);
    expect(leadRows.every((lead) => lead.stageDefinitionId === fixture.stageId)).toBe(true);
    expect(leadRows.every((lead) => lead.state === "awaiting_operator")).toBe(true);
    expect(
      leadRows.every(
        (lead) => JSON.parse(lead.intakeJson ?? "{}").source === "service_catalog_runtime",
      ),
    ).toBe(true);
    ...
    const transferLead = leadRows.find((lead) => lead.requestType === "transfer");
    ...
    const values = await withTenant(db as Db, tenantId, (tx) =>
      tx
        ...
        .where(
          and(eq(leadFieldValues.tenantId, tenantId), eq(leadFieldValues.leadId, transferLead.id)),
        ),
    );
    const bySlug = new Map(values.map((value) => [value.slug, JSON.parse(value.valueJson)]));
  ...
  it("dedupes explicit funnel routes against an existing untyped lead in the same target funnel", async () => {
    ...
    expect(result.skipped).toEqual([expect.objectContaining({ reason: "open_request_exists" })]);

    const rows = await withTenant(db as Db, tenantId, (tx) =>
      tx
        .select({ id: leads.id, requestType: leads.requestType })
        .from(leads)
        .where(and(eq(leads.tenantId, tenantId), eq(leads.userId, setup.contactId))),
    );
  ...
  it("does not create an LLM-only parallel service while another request is active", async () => {
    ...
    const setup = await withTenant(db as Db, tenantId, async (tx) => {
      ...
      const [transferFunnel] = await tx
        ...
        .returning({ id: funnels.id });
      const exchangeFunnelRow = expectInserted(exchangeFunnel, "active exchange funnel");
      const transferFunnelRow = expectInserted(transferFunnel, "active transfer funnel");
      ...
      await tx.insert(leads).values({
        ...
        userId: contactRow.id,
        state: "exchange_request",
        stageDefinitionId: expectInserted(exchangeStage, "active exchange stage").id,
    ...
    const runtime = makeServiceCatalogRuntime({
      ...
      resolveChat: () =>
        ({
          complete: async () =>
            JSON.stringify({
              requests: [{ slug: setup.transferSlug, confidence: 0.99, fields: {} }],
            }),
        }) as never,
    });
    const result = await runtime.extract({
  ...
  it("uses LLM JSON embedded in text, display-name field mapping, and webhook handoff", async () => {
    ...
    const values = await withTenant(db as Db, tenantId, (tx) =>
      tx
        ...
        .innerJoin(stageFields, eq(stageFields.id, leadFieldValues.fieldId))
        .where(
          and(eq(leadFieldValues.tenantId, tenantId), eq(leadFieldValues.leadId, webhookLead.id)),
        ),
    );
    const bySlug = new Map(values.map((value) => [value.slug, JSON.parse(value.valueJson)]));
  ...
  it("uses partner service stage target before partner funnel fallback", async () => {
    ...
    const setup = await withTenant(db as Db, tenantId, async (tx) => {
      ...
      const [fallbackFunnel] = await tx
        ...
        .returning({ id: funnels.id });
      const fallbackFunnelRow = expectInserted(fallbackFunnel, "fallback funnel");
    ...
    const bySlug = new Map(
      leadRows.map((lead) => [JSON.parse(lead.intakeJson ?? "{}").serviceSlug as string, lead]),
    );
    ...
    const stageLead = expectInserted(bySlug.get(setup.stageSlug), "stage target lead");
  ...
  it("falls back to heuristic when LLM fails and to synthetic state without stages", async () => {
    ...
    const [contact] = await withTenant(db as Db, syntheticTenantId, async (tx) => {
      const [insertedContact] = await tx
        .insert(contacts)
        .values({
  ...
  it("LLM вернул мусор с фигурными скобками → heuristic fallback", async () => {
    ...
    expect(result.created.map((item) => item.serviceSlug)).toEqual(["garbage_transfer"]);
