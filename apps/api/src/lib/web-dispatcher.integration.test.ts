...
async function seedWebEnvelope(
	channelDbId: number,
	userId: string,
	text: string,
): Promise<number> {
...
describe("WebOutboundDispatcher integration", () => {
	it("kind='web' row → adapter.send → markSent, frame на WS-клиенте", async () => {
		...
		expect(
			fakeWs.sent.some(
				(s) => s.includes('"type":"bot_text"') && s.includes("из web"),
			),
		).toBe(true);
	...
	it("offline user (no active WS) → markFailed reason=web_offline", async () => {
		...
		const { dispatcher, metrics } = makeDispatcher(registry);
		const queueId = await seedWebEnvelope(
			webChannelDbId,
			"offline-user",
			"ау?",
		);
	...
	it("web row with missing registry entry → markFailed reason=web_registry_miss", async () => {
		if (!sql) return;
		const registry = new TestWebRegistry();
		registry.add({
			channelDbId: 99_999,
			...
			tenantSlug: "other",
			externalId: "other",
			adapter: new WebChannelAdapter({ id: "other" }),
		});
		const { dispatcher, metrics } = makeDispatcher(registry);
		const queueId = await seedWebEnvelope(
			webChannelDbId,
		...
		const [row] = await db
			...
			.where(eq(outboundQueue.id, queueId));
		expect(row?.status).toBe("failed");
		expect(row?.lastError).toContain("web registry miss");
		expect(metrics.registry.format()).toContain(
			`lead_engine_outbound_failed_total{reason="web_registry_miss",tenant="${tenantId}"} 1`,
		);
	});

	it("adapter send throws generic error → markFailed reason=send_error", async () => {
		if (!sql) return;
		const registry = new TestWebRegistry();
		registry.add({
			channelDbId: webChannelDbId,
		...
		const { dispatcher, metrics } = makeDispatcher(registry);
		const queueId = await seedWebEnvelope(webChannelDbId, "u-1", "send error");
		...
		const [row] = await db
			...
			.where(eq(outboundQueue.id, queueId));
		expect(row?.status).toBe("failed");
		expect(row?.lastError).toBe("socket write failed");
		expect(metrics.registry.format()).toContain(
