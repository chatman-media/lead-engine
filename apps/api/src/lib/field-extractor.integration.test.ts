...
async function requestOfLead(leadId: number) {
...
describe("concierge branch-aware auto-advance (slice 3)", () => {
	it("request_type=transfer → лид уходит в transfer_request + пишется request_type", async () => {
		...
		const request = await requestOfLead(lead.id);
		expect(request?.requestType).toBe("transfer");
		expect(request?.status).toBe("open");
		expect(request?.stageDefinitionId).toBe(lead.stageDefinitionId);
	...
	it("request_type=exchange → лид уходит в exchange_request", async () => {
		...
		const contactId = await freshContact();
		const extractor = makeFieldExtractor(
			stubRef('{"request_type":"exchange"}'),
		);

		await extractor.extract({
			tenantId,
			contactId,
			text: "хочу поменять usdt на баты",
			db,
		});
	...
	it("multi-request (3b): после завершения первого запроса новый создаёт отдельный лид", async () => {
		...
		if (!completed) throw new Error("completed stage missing");
		if (!first) throw new Error("first lead missing");
		await db
			.update(leads)
	...
	it("multi-request (parallel): новый тип в треде с открытым запросом → отдельный лид в его ветке", async () => {
		...
		expect(all.find((l) => l.requestType === "food")?.state).toBe(
			"food_request",
		);
		// Старый transfer-лид не тронут.
		expect(all.find((l) => l.requestType === "transfer")?.state).toBe(
			"transfer_request",
		);
		...
		expect(requests.map((r) => r.requestType).sort()).toEqual([
			"food",
			"transfer",
		]);
	});
...
describe("awaiting_operator entry → пинг оператору (R5)", () => {
	it("advance в awaiting_operator-стадию шлёт notify; обычный advance — нет", async () => {
		...
		expect(
			events.some(
				(e) =>
					e.eventType === "stage_changed" && e.data.awaitingOperator === true,
			),
		).toBe(true);
	});
});
