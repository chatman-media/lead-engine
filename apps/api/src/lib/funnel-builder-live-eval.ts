...
export function buildFunnelEvalMessages(
...
export function scoreFunnelOutput(
	...
	const phaseCoverage =
		requiredPhases.length === 0 ? 1 : phaseHits / requiredPhases.length;
	const branchValid = branchContractValid(scenario, parsed.stages);
	const fieldQuality = scoreFieldKeywords(
		parsed.stages,
		scenario.expect.fieldKeywords ?? [],
	);
	...
	const behaviorCoverage =
		activeStages.length === 0
			? 0
			: activeStages.filter(
					(stage) => stage.goal?.trim() && stage.guidance?.trim(),
				).length / activeStages.length;
	const structuralValid =
		parsed.stages.length > 0 &&
		!parsed.parseError &&
		backbone.errors.length === 0;
	...
	const errors = [
		...
		...(behaviorCoverage >= 0.5
			? []
			: ["Goal/guidance coverage below threshold"]),
	];
	const passed =
		structuralValid &&
		branchValid &&
