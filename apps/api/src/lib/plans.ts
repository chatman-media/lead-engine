...
// Кастомная версия для обменки: SaaS-биллинга нет, поэтому базовый план
// фактически без лимитов (несколько каналов: бот + личный аккаунт + WhatsApp,
// неограниченные лиды/админы/вызовы). maxChannels/maxKbDocuments требуют
// конечных чисел (quota-чек не понимает -1), поэтому заданы с большим запасом.
 ...
 */
export function resolvePlan(planStr: string, onUnknown?: (plan: string) => void): PlanLimits {
