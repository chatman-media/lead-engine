...
import { judgeExchangeSelfPlay, parseExchangeSelfPlayVerdict } from "./self-play-judge.ts";
import { EXCHANGE_SELF_PLAY_PERSONAS, getExchangeSelfPlayPersona } from "./self-play-personas.ts";
...
describe("exchange self-play judge", () => {
  ...
  it("regex fallback without reason uses (no reason)", () => {
    expect(parseExchangeSelfPlayVerdict('broken { "outcome": "operator_needed" yes')).toEqual({
  ...
  it("JSON array and unknown outcome both degrade to dead_end", () => {
    ...
    const unknownOutcome = parseExchangeSelfPlayVerdict('{"outcome":"unknown_state","reason":"x"}');
