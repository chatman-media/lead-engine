...
type ToolMap = Record<string, { execute: (args: Record<string, unknown>) => Promise<unknown> }>;
...
interface ScenarioInput {
  ...
  paymentMethod: "crypto_transfer" | "sbp_qr" | "card_transfer" | "bank_transfer";
  paymentRail?: string;
  sourceBank?: string;
  payoutMethod: "courier_cash" | "cardless_atm" | "thai_bank_transfer" | "office_cash" | "atm";
...
async function createCryptoOrderForVerification(id: string): Promise<{
  ...
  await must(tools.fetch_exchange_requisites, "fetch_exchange_requisites").execute({});
...
describe("Exchange workflow fixtures", () => {
  ...
  for (const scenario of SCENARIOS) {
    it(`${scenario.fixtureId}: creates order/requisites/proof/payout through mocked tools`, async () => {
      ...
      const first = await makeConversation(fixture, !scenario.expectNeedsVerification);
      ...
      if (scenario.amountMode === "target_thb") expect(quote.amountToThb).toBe(scenario.amount);

      let activeConversationId = first.conversationId;
      let create = (await must(tools.create_exchange_order, "create_exchange_order").execute({
        asset: scenario.asset,
        amount: scenario.amount,
        amountMode: scenario.amountMode,
        network: scenario.asset === "USDT" ? "trc20" : undefined,
      ...
      if (scenario.expectNeedsVerification) {
        ...
        create = (await must(tools.create_exchange_order, "create_exchange_order").execute({
          asset: scenario.asset,
          amount: scenario.amount,
          amountMode: scenario.amountMode,
          network: scenario.asset === "USDT" ? "trc20" : undefined,
      ...
      if (scenario.paymentRail === "binance_id") expect(requisites.exchangeId).toBe("66775963");
        ...
        const proof = (await must(tools.verify_exchange_payment, "verify_exchange_payment").execute(
          {
            ...
            receiptAmount:
              scenario.amountMode === "target_thb" ? Number(create.amountFrom) : scenario.amount,
      ...
      const payout = (await must(tools.issue_exchange_payout, "issue_exchange_payout").execute({
        payoutMethod: scenario.payoutMethod,
      ...
      const expired = (await must(tools.issue_exchange_payout, "issue_exchange_payout").execute({
        payoutMethod: scenario.payoutMethod,
        location: scenario.payoutLocation,
        destination: scenario.payoutDestination ?? {},
      })) as Record<string, unknown>;
      ...
      if (scenario.asset === "RUB") expect(order.proofJson).toContain("fiat_receipt");
    });
  }
  ...
  it("rejects replayed crypto tx hash on a second order", async () => {
    ...
    const rows = await withTenant(db, tenantId, (tx) =>
      tx
        ...
        .from(exchangeOrders)
        .where(and(eq(exchangeOrders.tenantId, tenantId), eq(exchangeOrders.id, second.orderId))),
    );
  ...
  it("allows only one paid order when two verifies race with the same crypto tx hash", async () => {
    ...
    const raced = orderRows.filter((row) => [first.orderId, second.orderId].includes(row.id));
  ...
  it("keeps old crypto tx hash out of paid status", async () => {
    ...
    const rows = await withTenant(db, tenantId, (tx) =>
      tx
        ...
        .from(exchangeOrders)
        .where(and(eq(exchangeOrders.tenantId, tenantId), eq(exchangeOrders.id, order.orderId))),
    );
  ...
  it("продвигает лида: order_created при заявке, requisites_sent при реквизитах", async () => {
    ...
    const { contactId, conversationId } = await makeConversation(
      { id: "stage-advance", title: "Stage advance" } as Parameters<typeof makeConversation>[0],
      true,
    );
    const now = Math.floor(Date.now() / 1000);
    await withTenant(db, tenantId, async (tx) => {
  ...
  it("повторная заявка после протухшей создаёт свежую (idempotency revive)", async () => {
    if (!sql) return;
    const { conversationId } = await makeConversation(
      { id: "idem-revive", title: "Idem revive" } as Parameters<typeof makeConversation>[0],
      true,
    );
    const tools = toTools(conversationId);
    const orderArgs = {
  ...
  it("продвигает лида: payment_proof_waiting на чеке, payout_or_completion на выдаче", async () => {
    if (!sql) return;
    const { contactId, conversationId } = await makeConversation(
      { id: "pay-advance", title: "Pay advance" } as Parameters<typeof makeConversation>[0],
      true,
    );
    const now = Math.floor(Date.now() / 1000);
    await withTenant(db, tenantId, async (tx) => {
    ...
    const vp = (await must(tools.verify_exchange_payment, "verify_exchange_payment").execute({
      proof: "чек",
      sourceBank: "Sber",
    })) as Record<string, unknown>;
    ...
    await must(tools.issue_exchange_payout, "issue_exchange_payout").execute({
      payoutMethod: "cardless_atm",
      location: "SCB ATM",
    });
  ...
  it("стейдж-гейт не блокирует инструмент после продвижения стадии в ходе", async () => {
    if (!sql) return;
    const { contactId, conversationId } = await makeConversation(
      { id: "gate-current", title: "Gate current" } as Parameters<typeof makeConversation>[0],
      true,
    );
    const now = Math.floor(Date.now() / 1000);
    await withTenant(db, tenantId, async (tx) => {
    ...
    const req = (await must(stale.fetch_exchange_requisites, "fetch_exchange_requisites").execute(
      {},
    )) as Record<string, unknown>;
  ...
  it("fetch_exchange_requisites оживляет протухшую заявку", async () => {
    if (!sql) return;
    const { conversationId } = await makeConversation(
      { id: "revive-req", title: "Revive req" } as Parameters<typeof makeConversation>[0],
      true,
    );
    const tools = toTools(conversationId);
    const created = (await must(tools.create_exchange_order, "create_exchange_order").execute({
  ...
  it("перекотировка с другой суммой отменяет заявку и откатывает на quote_calculated", async () => {
    if (!sql) return;
    const { contactId, conversationId } = await makeConversation(
      { id: "requote", title: "Requote" } as Parameters<typeof makeConversation>[0],
      true,
    );
    const now = Math.floor(Date.now() / 1000);
    await withTenant(db, tenantId, async (tx) => {
    ...
    const q = (await must(tools.compute_exchange_quote, "compute_exchange_quote").execute({
      asset: "USDT",
      amount: 300,
      amountMode: "source_amount",
      network: "trc20",
