...
export const EXCHANGE_SELF_PLAY_SCENARIOS: readonly ExchangeSelfPlayScenario[] =
	[
			...
			criticalReplyAssertions: [
				{
					id: "receipt-pending",
					description:
						"Receipt upload must be acknowledged as pending operator/payment review.",
		...
		},
		{
			...
			criticalReplyAssertions: [
				{
					id: "wallet-from-tool",
					description:
						"Wallet/requisites must come from configured tool state.",
			...
			debugHint:
				"Use this to verify crypto flow can progress without unnecessary operator handoff.",
		},
		{
			...
			criticalReplyAssertions: [
				{
					...
					description:
						"Bot must collect bank details before promising transfer.",
					mustIncludeAny: ["bangkok bank", "счет", "реквиз"],
					mustNotIncludeAny: ["перевод отправлен", "деньги ушли"],
				},
			],
			debugHint:
				"Use this to check non-cash fulfilment fields and no premature transfer promise.",
		},
		{
			...
			criticalReplyAssertions: [
				{
					...
					description:
						"Changed amount/rail must trigger a fresh quote/order context.",
			...
			debugHint:
				"Use this to catch stale quote reuse after the customer changes terms.",
		},
		{
			...
			criticalReplyAssertions: [
				{
					id: "ask-next-field",
					description:
						"Bot asks for missing required fields instead of guessing.",
			...
			criticalReplyAssertions: [
				{
					id: "kyc-pending",
					description:
						"Media/documents must become pending verification, not auto-approved.",
					mustIncludeAny: ["верификац", "провер", "оператор"],
					mustNotIncludeAny: ["kyc пройден", "верификация подтверждена"],
				},
			],
			debugHint:
				"Use this to catch media being treated as plain chat text or auto-approved KYC.",
		},
		{
			...
			debugHint:
				"Use this to verify RUB receipts do not move straight to paid without review.",
		},
		{
			...
			criticalReplyAssertions: [
				{
					id: "no-fake-eta",
					description:
						"Bot must not invent courier availability in unsupported city/out-of-hours.",
			...
			debugHint:
				"Use this to catch fabricated availability, ETA, and unsupported coverage promises.",
		},
		{
			...
			criticalReplyAssertions: [
				{
					...
					description:
						"Bot must not reuse stale rate or force flow after explicit cancellation.",
					mustIncludeAny: ["актуальн", "миним", "отмен"],
					mustNotIncludeAny: ["старый курс подтверждаю", "оплатите"],
				},
			],
			debugHint:
				"Use this to catch stale quote reuse and ignoring cancellation intent.",
		},
	] as const;

export function getExchangeSelfPlayScenario(
	id: string,
): ExchangeSelfPlayScenario | null {
	return (
		EXCHANGE_SELF_PLAY_SCENARIOS.find((scenario) => scenario.id === id) ?? null
	);
}
...
export function runExchangeSelfPlayScenarioCorpus(
	input: {
		scenarioId?: string | null;
		fixtureCases?: readonly ExchangeGoldenCase[];
	} = {},
	...
	for (const scenario of scenarios) {
		...
		if (
			scenario.expectedFields.filter((field) => field.required).length === 0
		) {
			failures.push({
				scenarioId: scenario.id,
	...
	return {
		...
		scenarios: scenarios.map((scenario) => ({
			...
			...(scenario.expectedOrder
				? { expectedOrderStatus: scenario.expectedOrder.status }
				: {}),
			criticalReplyAssertions: scenario.criticalReplyAssertions.map(
...
export function formatExchangeSelfPlayScenarioReport(
	report: ExchangeScenarioCorpusReport,
): string {
	const header = [
		...
		lines.push(
			[
				...
				scenario.expectedHandoffs.length > 0
					...
					: "handoffs=none",
				scenario.expectedOrderStatus
					? `order=${scenario.expectedOrderStatus}`
					: null,
			]
				.filter(Boolean)
				.join(" "),
