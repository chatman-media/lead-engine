 ...
 */
export async function getTenantQuoteCurrency(
	db: Db,
	tenantId: number,
): Promise<QuoteCurrency> {
...
export function applyRateDeviation(
	baseRate: number,
	deviationPct: number,
): number {
	return baseRate * (1 + deviationPct / 100);
}
		...
		for (const net of network ? [network, ""] : [""]) {
			...
			if (row) {
				return {
					...
					minAmountFrom:
						row.minAmountFrom === null ? null : Number(row.minAmountFrom),
					maxAmountFrom:
						row.maxAmountFrom === null ? null : Number(row.maxAmountFrom),
				};
			}
		}
		return null;
...
export async function computeQuote(
	...
	const amountMode =
		input.amountMode === "target_thb" ? "target_thb" : "source_amount";
	...
	const row = await findActiveRateRow(
		db,
		tenantId,
		asset,
		network,
	...
	if (
		...
		row.minAmountFrom !== null &&
		amount < row.minAmountFrom
	) {
		return {
			ok: false,
			error: `Минимальная сумма обмена — ${row.minAmountFrom} ${asset}.`,
		};
	}
	if (
		amountMode === "source_amount" &&
		row.maxAmountFrom !== null &&
		amount > row.maxAmountFrom
	) {
		return {
			ok: false,
			error: `Максимальная сумма обмена — ${row.maxAmountFrom} ${asset}.`,
		};
	}

	const mode = row.quoteMode === "divide" ? "divide" : "multiply";
	const tiers = await findActiveTierRows(
		db,
		tenantId,
		asset,
		network,
	...
	const eff = tier
		? applyRateDeviation(row.baseRate, tier.deviationPct)
		: mode === "divide"
			? row.baseRate * (1 + row.marginPct / 100)
			: row.baseRate * (1 - row.marginPct / 100);

	// Guardrail: блокируем неадекватную/убыточную котировку. Срабатывает и для
	// tier.deviationPct — он раньше обходил любые проверки (кейс «10 вместо 35»).
	const guard = checkRateGuard({
		eff,
