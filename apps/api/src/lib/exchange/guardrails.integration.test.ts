 ...
 * через ПОЛНЫЙ путь computeQuote (включая tier.displayRate, который раньше
...
beforeAll(async () => {
  ...
  await withTenant(db, tenantId, async (tx) => {
    await tx.insert(exchangeRates).values([
      ...
      { tenantId, asset: "USDT", quoteAsset: "THB", network: "trc20", baseRate: 31.5, quoteMode: "multiply", marginPct: 0, isActive: true, createdAt: now, updatedAt: now },
      // BTC — базовая формула с абсурдной маржой 80% → eff далеко от base.
      { tenantId, asset: "BTC", quoteAsset: "THB", network: "", baseRate: 1000, quoteMode: "multiply", marginPct: 80, isActive: true, createdAt: now, updatedAt: now },
      // RUB — нормальная маржа 2%.
      { tenantId, asset: "RUB", quoteAsset: "THB", network: "", baseRate: 2.55, quoteMode: "divide", marginPct: 2, isActive: true, createdAt: now, updatedAt: now },
    ]);
    // Битый тариф USDT: displayRate 10 при рынке 31.5 (кейс «10 вместо 35»).
    await tx.insert(exchangeRateTiers).values({
      tenantId, asset: "USDT", quoteAsset: "THB", network: "trc20", rangeBasis: "target_thb",
      minAmount: 0, maxAmount: null, marketRate: 31.5, displayRate: 10, deviationPct: -68, isActive: true,
...
describe("guardrail в computeQuote", () => {
  it("блокирует битый tier.displayRate (10 при рынке 31.5)", async () => {
    if (!enabled) return;
  ...
  it("после починки тарифа (displayRate 31) котировка проходит", async () => {
    if (!enabled) return;
    await withTenant(db, tenantId, async (tx) => {
      await tx
