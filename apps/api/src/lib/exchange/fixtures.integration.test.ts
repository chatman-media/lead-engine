...
import { computeQuote, getTenantQuoteCurrency, listActiveDirections } from "./rates.ts";
...
describe("exchange deterministic fixtures", () => {
  ...
  it("seeded fixtures drive exchange tools without direct secret reads", async () => {
    ...
    const info = (await tools.get_exchange_business_info?.execute({})) as Record<string, unknown>;
  ...
  it("lists only real configured directions and rejects unknown pair with alternatives", async () => {
    ...
    const list = (await tools.list_exchange_directions?.execute({})) as {
      quoteAsset: string;
      directions: Array<{ asset: string; quoteAsset: string; kind: string; networks: string[] }>;
      display: string;
    };
    expect(list.quoteAsset).toBe("PHP");
    // Реальные направления PHP-набора — прямые USDT/RUB/USD → PHP плюс обратное
    // PHP→RUB (#741). BTC/ETH/EUR из старого захардкоженного списка отсутствуют.
    expect(list.directions.map((d) => d.asset).sort()).toEqual(["PHP", "RUB", "USD", "USDT"]);
    // Обратное направление видно с собственной валютой выдачи RUB.
    const phpToRub = list.directions.find((d) => d.asset === "PHP");
    expect(phpToRub?.quoteAsset).toBe("RUB");
...
describe("exchange fixtures: RUB→PHP auto-quote out of the box (PH)", () => {
  it("сидит активную (RUB,PHP,'',divide), отдаёт её в listActiveDirections и считает котировку с попаданием в тир", async () => {
    ...
    // #741: обратное направление PHP→RUB видно и считается БЕЗ оператора.
    const phpToRub = dirs.find((d) => d.asset === "PHP" && d.quoteAsset === "RUB");
    expect(phpToRub).toBeDefined();
    const rev = await computeQuote(appDb as Db, phTenant.id, {
      asset: "PHP",
      amount: q.amountToThb, // отдаём ровно те песо, что получили в прямой сделке
      amountMode: "source_amount",
      payoutAsset: "RUB",
    });
    expect(rev.ok).toBe(true);
    if (!rev.ok) return;
    expect(rev.direction).toBe("PHP->RUB");
    expect(rev.quoteAsset).toBe("RUB");
    expect(rev.quoteMode).toBe("multiply");
    // Anti-1/eff: вернув полученные песо обратно, клиент получает МЕНЬШЕ рублей,
    // чем отдал (спред в пользу обменника на обеих сторонах — не убыток).
    expect(rev.amountToThb).toBeLessThan(100_000);
    // Регрессия: прямой RUB→PHP без payoutAsset по-прежнему работает (q выше).
  });
});
