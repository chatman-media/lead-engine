...
async function readRubCardDetails(deps: {
  db: Db;
  tenantId: number;
  masterKeyHex: string;
}): Promise<string | null> {
...
async function readWestWalletConfig(deps: {
  ...
  const ipnUrlBase =
    ipnUrlSecret || (publicUrl ? `${publicUrl}/webhook/westwallet/${deps.tenantId}` : null);
  // Embed the per-tenant IPN auth token so the webhook can reject forged
  // callbacks (see lib/exchange/westwallet-ipn.ts).
  const ipnUrl = ipnUrlBase ? withIpnKey(ipnUrlBase, deps.tenantId, deps.masterKeyHex) : null;
  return {
...
export class ConfigPaymentProvider implements PaymentProvider {
  ...
  constructor(private readonly deps: { db: Db; tenantId: number; masterKeyHex: string }) {}
        ...
        for (const currency of currencyCandidates) {
          try {
            const invoice = await westWallet.client.createInvoice({
              ...
              description:
                `${asset}${net ? ` ${net.toUpperCase()}` : ""} order ${input.orderId ?? ""}`.trim(),
...
export async function verifyWestWalletInvoicePayment(opts: {
  ...
  if (!tx)
    return { ok: false, transaction: null, note: "Платёж по WestWallet invoice пока не найден." };
