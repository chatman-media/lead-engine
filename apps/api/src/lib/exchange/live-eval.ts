...
function concreteFieldExpected(
	field: ExchangeScenarioFieldExpectation,
): boolean {
	return (
...
function fieldMatches(
	...
	if (field.value !== undefined)
		return normalized(actual) === normalized(field.value);
	if (field.oneOf) {
...
function scoreReplyAssertions(
	...
	for (const assertion of scenario.criticalReplyAssertions) {
		if (
			...
			!assertion.mustIncludeAny.some((item) =>
...
function scoreGuardFindings(
	actual: ExchangeLiveEvalActualState,
): ExchangeLiveEvalFailure[] {
	...
	return {
		...
		expected: {
			...
			...(scenario.expectedOrder
				? { orderStatus: scenario.expectedOrder.status }
				: {}),
...
export async function runExchangeLiveEval(opts: {
	...
	const score =
		results.length === 0
			...
			: Number(
					(
						results.reduce((sum, result) => sum + result.metrics.score, 0) /
						results.length
					).toFixed(4),
				);
	return {
...
export function formatExchangeLiveEvalSummary(
	report: ExchangeLiveEvalReport,
): string {
	const lines = [
