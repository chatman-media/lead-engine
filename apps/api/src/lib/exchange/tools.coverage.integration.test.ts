...
function mockWestWalletTransactions(
	...
	globalThis.fetch = (async (input: Parameters<typeof fetch>[0]) => {
		...
		return new Response(
			JSON.stringify({ error: "ok", count: result.length, result }),
			{ status: 200 },
		);
...
describe("exchange tools — stage policy wrapper", () => {
	it("denies a tool outside its stage and passes allowed tools through", async () => {
		...
		).execute({ payoutMethod: "office_cash", location: "офис" })) as Record<
			string,
			unknown
		>;
		...
		).execute({ asset: "USDT", amount: 100, network: "trc20" })) as Record<
			string,
			unknown
		>;
...
describe("exchange tools — create order branches", () => {
	it("rate-guard trip blocks order creation and alerts the owner", async () => {
		...
		).execute({ asset: "ETH", amount: 1, network: "erc20" })) as Record<
			string,
			unknown
		>;
...
describe("exchange tools — requisites and payout operator branches", () => {
	it("unconfigured wallet → requisites needsOperator", async () => {
		...
		const created = (await must(
			tools.create_exchange_order,
			"create_exchange_order",
		).execute({ asset: "BTC", amount: 0.01 })) as Record<string, unknown>;
	...
	it("payout before paid → error; paid without code → needsOperator", async () => {
		...
		const created = (await must(
			tools.create_exchange_order,
			"create_exchange_order",
		).execute({ asset: "USDT", amount: 200, network: "trc20" })) as Record<
			string,
			unknown
		>;
...
describe("exchange tools — crypto payment verification branches", () => {
	it("crypto proof without tx hash → asks for hash; broken requisites json → operator", async () => {
		...
		const created = (await must(
			tools.create_exchange_order,
			"create_exchange_order",
		).execute({ asset: "USDT", amount: 75, network: "trc20" })) as Record<
			string,
			unknown
		>;
		const orderId = created.orderId as number;

		const noHash = (await must(
			tools.verify_exchange_payment,
			"verify_exchange_payment",
		).execute({ proof: "перевёл, чек пришлю позже" })) as Record<string, unknown>;
		...
		const noAddress = (await must(
			tools.verify_exchange_payment,
			"verify_exchange_payment",
		).execute({ proof: "a".repeat(64) })) as Record<string, unknown>;
	...
	it("non-TRC20 network → auto-check unavailable, operator takes over", async () => {
		...
		const res = (await must(
			tools.verify_exchange_payment,
			"verify_exchange_payment",
		).execute({ proof: "b".repeat(64) })) as Record<string, unknown>;
...
describe("computeQuote — validation branches", () => {
	it("rejects non-positive amounts, unknown rates and min/max limits", async () => {
		...
		if (!missing.ok) expect(missing.error).toContain("не настроен");
...
describe("westwallet invoice verification", () => {
	it("invoice requisites without keys → operator; with keys and completed tx → paid", async () => {
		...
		const created = (await must(
			tools.create_exchange_order,
			"create_exchange_order",
		).execute({ asset: "USDT", amount: 120, network: "trc20" })) as Record<
			string,
			unknown
		>;
		const orderId = created.orderId as number;
	...
	it("verifyWestWalletInvoicePayment: not found / pending / insufficient / NaN amount", async () => {
		...
		try {
			...
			mockWestWalletTransactions([
				{ id: 2, amount: "120", address: "T", currency: "USDTTRC", status: "pending", type: "receive" },
			]);
			...
			mockWestWalletTransactions([
				{ id: 3, amount: "50", address: "T", currency: "USDTTRC", status: "completed", type: "receive" },
			]);
			...
			mockWestWalletTransactions([
				{ id: 4, amount: "not-a-number", address: "T", currency: "USDTTRC", status: "completed", type: "send" },
			]);
