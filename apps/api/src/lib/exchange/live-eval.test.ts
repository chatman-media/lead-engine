...
import { EXCHANGE_SELF_PLAY_SCENARIOS, type ExchangeSelfPlayScenario } from "./scenario-corpus.ts";
...
describe("exchange live eval", () => {
  ...
  it("can run a single scenario", async () => {
    ...
    expect(report.results[0]?.scenarioId).toBe("rub-office-pickup-payment-proof");
  ...
  it("reports missing required concrete fields", () => {
    ...
    expect(result.failures.some((failure) => failure.kind === "field_missing")).toBe(true);
  ...
  it("reports missing stages and handoffs", () => {
    ...
    expect(result.passed).toBe(false);
    expect(result.failures.some((failure) => failure.kind === "stage_missing")).toBe(true);
    expect(result.failures.some((failure) => failure.kind === "handoff_missing")).toBe(true);
  });
  ...
  it("reports guard findings and forbidden reply text", () => {
    ...
    expect(result.failures.some((failure) => failure.kind === "guard_violation")).toBe(true);
    expect(result.failures.some((failure) => failure.kind === "reply_assertion")).toBe(true);
  });
});

// ── Ветки скоринга, недостижимые на «здоровом» корпусе: синтетический сценарий ──

function syntheticScenario(overrides: Partial<ExchangeSelfPlayScenario>): ExchangeSelfPlayScenario {
  return {
    ...
    expectedFields: [{ key: "asset", required: true, value: "USDT", source: "client" }],
...
describe("exchange live eval — failure branches", () => {
  it("reports field mismatch when value differs from expectation", () => {
    ...
    expect(result.passed).toBe(false);
    expect(result.failures.some((failure) => failure.kind === "field_mismatch")).toBe(true);
  ...
  it("reports unexpected order when scenario expects none", () => {
    ...
    expect(result.passed).toBe(false);
    expect(result.failures.some((failure) => failure.kind === "order_unexpected")).toBe(true);
  ...
  it("reports order field mismatches against expected order", () => {
    ...
    expect(result.passed).toBe(false);
    const mismatches = result.failures.filter((failure) => failure.kind === "order_mismatch");
    expect(mismatches.length).toBe(2); // status + payoutMethod
    expect(mismatches.map((failure) => failure.message)).toContain("Order mismatch: status");
  });
  ...
  it("reports missing mustIncludeAny tokens and replay errors", () => {
    ...
    expect(
      result.failures.some(
        (failure) =>
          failure.kind === "reply_assertion" && failure.message.includes("missing include token"),
      ),
    ).toBe(true);
    expect(result.failures.some((failure) => failure.kind === "replay_error")).toBe(true);
