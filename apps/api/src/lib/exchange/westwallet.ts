...
function sign(secretKey: string, timestamp: number, payload: string): string {
  return createHmac("sha256", secretKey).update(`${timestamp}${payload}`, "utf8").digest("hex");
}
...
export class WestWalletClient {
  ...
  async generateAddress(input: { currency: string; ipnUrl: string; label?: string }): Promise<{
    error: "ok";
    address: string;
    dest_tag?: string | null;
    currency: string;
