...
describe("exchange golden eval — failure recording branches", () => {
			...
			expect(
				result.failures.some((failure) =>
					failure.actual.includes(
						"compute_exchange_quote denied at exchange_request",
					),
				),
			).toBe(true);
			expect(result.trace).toContain(
				"rate_quote: exchange_request -> compute_exchange_quote",
			);
		} finally {
			mock.module("./tools.ts", () => ({
				...tools,
	...
	it("records draft failures when the reply guard allows or misclassifies drafts", () => {
		...
		try {
			...
			expect(result.passed).toBe(false);
			expect(
				result.failures.some((failure) => failure.actual === "draft was allowed"),
			).toBe(true);
			expect(
				result.failures.some(
					(failure) => failure.actual === "blocked with some_other_reason",
				),
			).toBe(true);
			expect(
				result.failures.some((failure) => failure.actual === "blocked with unknown"),
			).toBe(true);
		} finally {
			mock.module("@chatman-media/conversation-engine", () => ({
				...engine,
