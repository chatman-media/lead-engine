 ...
 */
export async function tenantSupportsMultiRequest(
	db: Db,
	tenantId: number,
): Promise<boolean> {
	...
	return withTenant(opts.db, opts.tenantId, async (tx) => {
		const requestRows = await tx
			...
			.where(
				and(
					...
					eq(customerRequests.contactId, opts.contactId),
		...
		if (requestRows.length > 0) {
			return requestRows.map((r) => ({
				type: REQUEST_TYPE_LABEL[r.requestType] ?? r.title ?? r.requestType,
				stage: r.stageName ?? r.status,
			}));
		}
