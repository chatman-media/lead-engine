...
describe("partner-ping token", () => {
  ...
  it("rejects tampered token", async () => {
    const token = await makeCallbackToken({ tenantId: 1, leadId: 1, stageId: 2, ts: 100 }, SECRET);
  ...
  it("rejects legacy payload without tenantId", async () => {
    const header = Buffer.from(JSON.stringify({ leadId: 1, stageId: 2, ts: 100 }), "utf8").toString(
      "base64url",
    );
  ...
  it("валидная подпись, но битый JSON в header → null (line 90)", async () => {
    const { signWebhookPayload } = await import("./webhook-sign.ts");
    // header декодируется в НЕ-JSON; HMAC корректный → дойдём до JSON.parse → catch.
...
describe("firePartnerPing", () => {
  it("HTTP transport posts signed callback payload", async () => {
    ...
    expect(body.callbackUrl).toBe(`https://app.test/api/partner/cb/${result.token}`);
    expect(body.actions.confirm).toBe(`https://app.test/api/partner/cb/${result.token}?a=confirm`);
    expect(body.actions.cancel).toBe(`https://app.test/api/partner/cb/${result.token}?a=cancel`);
  ...
  it("Telegram transport sends formatted message with action buttons", async () => {
    ...
    const result = await firePartnerPing(basePing({ webhookUrl: "tg://-100123456" }));
  ...
  it("Telegram transport validates chat id and bot token", async () => {
    await expect(firePartnerPing(basePing({ webhookUrl: "tg:// " }))).rejects.toThrow(
      "tg:// has no chat_id",
    );
    await expect(
      firePartnerPing(basePing({ webhookUrl: "tg://42", operatorBotToken: "" })),
  ...
  it("Telegram transport throws with Telegram description on non-ok response", async () => {
    globalThis.fetch = (async () =>
      Response.json({ description: "chat not found" }, { status: 400 })) as unknown as typeof fetch;

    await expect(firePartnerPing(basePing({ webhookUrl: "tg://42" }))).rejects.toThrow(
