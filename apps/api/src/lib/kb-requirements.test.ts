...
describe("buildKbRequirementDrafts", () => {
  it("обычная воронка без стадий → 3 базовых funnel-требования", () => {
    ...
    expect(
      drafts.every(
        (d) => d.scopeType === "funnel" && d.funnelId === 7 && d.stageSlug === null && d.required,
      ),
    ).toBe(true);
  });
  ...
  it("awaiting_operator/external_approval → необязательное handoff-требование", () => {
    const drafts = buildKbRequirementDrafts({
      funnel: baseFunnel,
      stages: [stage("operator", "awaiting_operator"), stage("approval", "external_approval")],
    });
...
describe("coverKbRequirements", () => {
  ...
  function doc(overrides: Partial<KbRequirementCoverageDocument>): KbRequirementCoverageDocument {
    return {
      topic: null,
      scopeType: "global",
      funnelId: null,
