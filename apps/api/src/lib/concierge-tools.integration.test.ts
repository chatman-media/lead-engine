...
beforeAll(async () => {
  ...
  await applyFunnelStages(db as never, tenantId, conciergeTemplate, "concierge");
...
async function makeConversationFor(requestType: string, stageSlug: string): Promise<number> {
  const [c] = await db
    .insert(contacts)
...
function toolByName(tools: ReturnType<typeof makeConciergeDomainTools>, name: string) {
...
describe("concierge domain tools (Фаза 3 — каталог и квоты)", () => {
  ...
  it("transfer options фильтрует capacity и confirm возвращает summary", async () => {
    ...
    const confirmed = (await toolByName(tools, "confirm_transfer_booking").execute({
      classId: "sedan",
      pickup: "BKK",
      dropoff: "Pattaya",
      datetime: "tomorrow 10:00",
  ...
  it("domain tools отказывают при несовпадении request_type", async () => {
    if (!sql) return;
    const foodConversationId = await makeConversationFor("food", "food_request");
    const tools = makeConciergeDomainTools({
      db: db as never,
      tenantId,
    ...
    const out = (await toolByName(tools, "get_transfer_options").execute({})) as { error?: string };
  ...
  it("housekeeping options даёт estimate, confirm применяет minHours", async () => {
    if (!sql) return;
    const hkConversationId = await makeConversationFor("housekeeping", "housekeeping_request");
    const tools = makeConciergeDomainTools({
      db: db as never,
      tenantId,
    ...
    const options = (await toolByName(tools, "get_housekeeping_options").execute({
      date: "2026-06-10",
      hours: 3,
    ...
    const booking = (await toolByName(tools, "confirm_housekeeping_booking").execute({
      date: "2026-06-10",
      timeSlot: "10:00",
      hours: 1,
  ...
  it("tour options фильтрует participants, confirm проверяет min/max", async () => {
    if (!sql) return;
    const tourConversationId = await makeConversationFor("tour", "tour_request");
    const tools = makeConciergeDomainTools({
      db: db as never,
      tenantId,
    ...
    expect(options.options.find((o) => o.id === "island")?.totalPrice).toBe(2700);
...
describe("concierge domain tools — fallback-ветки", () => {
  async function signupTenant(email: string): Promise<number> {
    ...
    return ((await sa.json()) as { admin: { tenantId: number } }).admin.tenantId;
  }
  ...
  it("каталог не настроен → все 8 инструментов отдают needsOperator", async () => {
    ...
    const calls: Array<[string, Record<string, unknown>]> = [
      ...
      ["get_tour_options", {}],
      ["confirm_tour_booking", { tourId: "island", date: "2026-06-12", participants: 2 }],
    ];
  ...
  it("confirm_tour_booking: превышение maxParticipants и неизвестный тур", async () => {
    ...
    const unknownTour = (await toolByName(tools, "confirm_tour_booking").execute({
      tourId: "ghost-tour",
      date: "2026-06-12",
      participants: 2,
