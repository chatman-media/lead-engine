...
export function makeAdminProviderMarketplaceRoutes(opts: { db: Db }): Hono {
	...
	app.get("/api/admin/provider-marketplace", async (c) => {
		...
		const customProviders = rows
			.map((row) => {
				...
				if (meta.source !== MARKETPLACE_CUSTOM_SOURCE || !meta.providerKey)
					return null;
		...
		async (c) => {
			...
			if (result.created) {
				await recordAudit(opts.db, {
					...
					targetId: String(
						result.provider.installed?.serviceCatalogItemId ?? "",
					),
					details: { providerKey },
				});
			}
	...
	app.post("/api/admin/provider-marketplace/custom", async (c) => {
		...
		const body = await c.req
			.json<CustomProviderBody>()
			.catch(() => ({}) as CustomProviderBody);
...
async function loadCatalogInstallRows(
	db: Db,
	tenantId: number,
...
async function installCuratedProvider(
	...
	const existing = buildInstalledByKey(rows, MARKETPLACE_SOURCE).get(
		provider.key,
	);
	if (existing)
		return { provider: { ...provider, installed: existing }, created: false };
...
async function installCustomProvider(
	...
	const rows = await loadCatalogInstallRowsFromTx(tx, tenantId);
	const existing = buildInstalledByKey(rows, MARKETPLACE_CUSTOM_SOURCE).get(
		key,
	);
	if (existing) {
		return {
...
function installedRef(
	...
	return {
		...
		installedAt: Number.isFinite(meta.installedAt)
			? Number(meta.installedAt)
			: null,
	};
}
...
async function createCatalogItem(
	...
	const nextSortOrder =
		existingRows.reduce((max, row) => Math.max(max, row.sortOrder), 0) + 10;
	const [row] = await tx
