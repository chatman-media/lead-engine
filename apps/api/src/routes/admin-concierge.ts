...
import { CONCIERGE_CATALOG_KEY, type ConciergeCatalog } from "../lib/concierge-domain-tools.ts";
...
export function makeAdminConciergeRoutes(opts: AdminConciergeRoutesOpts) {
  ...
  app.put("/api/admin/concierge/catalog", async (c) => {
    ...
    if (typeof body !== "object" || body === null || !knownKeys.some((k) => k in body)) {
      return c.json(
        {
  ...
  app.delete("/api/admin/concierge/catalog", async (c) => {
    ...
    await withTenant(db, tenantId, (tx) =>
      tx
        ...
        .where(
          and(eq(tenantSecrets.tenantId, tenantId), eq(tenantSecrets.key, CONCIERGE_CATALOG_KEY)),
        ),
    );
