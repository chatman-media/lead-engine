...
describe("partner callback route", () => {
  it("renders landing page and confirm advances through workflow runtime", async () => {
    ...
    const confirm = await app.request(`/api/partner/cb/${fixture.token}?a=confirm`);
    ...
    const [event] = await db
      .select({
        fromState: leadEvents.fromState,
        toState: leadEvents.toState,
        notes: leadEvents.notes,
      })
      .from(leadEvents)
    ...
    const repeat = await app.request(`/api/partner/cb/${fixture.token}?a=confirm`);
  ...
  it("cancel clears token, rejects deal, and records callback event without advancing", async () => {
    ...
    const cancel = await app.request(`/api/partner/cb/${fixture.token}?a=cancel`);
...
async function createCallbackFixture(label: string): Promise<{
  ...
  await db.update(leads).set({ awaitingToken: token, updatedAt: now }).where(eq(leads.id, lead.id));
