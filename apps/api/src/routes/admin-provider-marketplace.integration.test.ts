...
describe("admin provider marketplace", () => {
	it("lists curated providers and installs one idempotently into the service catalog", async () => {
		...
		expect(
			list.items.find((item) => item.key === "phuket_transfer_network")
				?.installed,
		).toBeNull();
		...
		expect(installed.provider.installed.serviceCatalogSlug).toBe(
			"transfer_provider",
		);
		...
		expect(catalog.items[0]?.partnerServiceId).toBe(
			installed.provider.installed.partnerServiceId,
		);
		...
		const tenantBListRes = await authReq(
			tokenB,
		...
		expect(
			tenantBList.items.find((item) => item.key === "phuket_transfer_network")
				?.installed,
		).toBeNull();
	});
	...
	it("creates custom providers as tenant-owned partner services", async () => {
		...
		expect(
			list.customProviders.some(
				(item) => item.serviceName === "Private dinner",
			),
		).toBe(true);
	});
});
