...
import { type WebChannelEntry, WebChannelRegistry } from "../lib/web-channel-registry.ts";
...
function wsAvailable(): boolean {
  if (!runWsIntegration) {
    console.warn("[ws-web.test] skipping websocket assertions: set RUN_WS_INTEGRATION=1 to enable");
    return false;
  }
