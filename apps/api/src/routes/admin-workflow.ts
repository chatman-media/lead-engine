...
function normalizeOptions(fieldType: string, f: FieldDraft): string | undefined {
  ...
  if (typeof f.optionsJson === "string" && f.optionsJson.trim()) return f.optionsJson;
...
export function normalizeStages(draft: StageDraft[]): SeedStage[] {
  ...
  for (let i = 0; i < draft.length; i++) {
    const d = draft[i];
    if (!d || typeof d.slug !== "string" || typeof d.displayName !== "string") continue;
    const slug = sanitizeSlug(d.slug);
    if (!slug) continue;
    const kind = validKind.has(d.kind) ? (d.kind as SeedStage["kind"]) : "active";
    const stageType = validStageType.has(d.stageType) ? d.stageType : "form_fill";
    ...
    for (let j = 0; j < draftFields.length; j++) {
      ...
      if (!f || typeof f.slug !== "string" || typeof f.displayName !== "string") continue;
    ...
    const hasExplicitCondition =
      typeof d.autoAdvanceCondition === "string" && d.autoAdvanceCondition.trim().length > 0;
    ...
    stages.push({
      ...
      ...(typeof d.goal === "string" && d.goal.trim() ? { goal: d.goal.slice(0, 500) } : {}),
...
export function makeAdminWorkflowRoutes(opts: AdminWorkflowRoutesOpts): Hono {
  ...
  app.post("/api/admin/workflows/ai-chat", async (c) => {
    ...
    let unwrapGuard = 0;
    while (parsed.readyToGenerate !== true && typeof parsed.reply === "string" && unwrapGuard < 3) {
  ...
  app.post("/api/admin/workflows/apply", async (c) => {
    ...
    const result = await applyFunnelStages(opts.db, tenantId, stages, "ai_workflow", {
      snapshot: {
        source: "ai_apply",
        adminId,
  ...
  app.post("/api/admin/workflows/recommend-skills", async (c) => {
    ...
    const catalogue = SKILLS_CATALOGUE.map((s) => `- ${s.slug}: ${s.description}`).join("\n");
...
Верни ТОЛЬКО JSON: {"slugs": ["slug", ...]} — 5-10 наиболее уместных техник ИМЕННО для этого бизнеса. Только slug'и из каталога, без выдумок.`;
    ...
    const recommended = Array.isArray(parsed.slugs)
      ? [...new Set(parsed.slugs.filter((s): s is string => typeof s === "string" && known.has(s)))]
      : [];
    ...
    await withTenant(opts.db, tenantId, async (tx) => {
      ...
      await tx
        ...
        .where(and(eq(skills.tenantId, tenantId), inArray(skills.slug, recommended)));
    });
