...
const OPERATOR_TARGETS = new Set<OperatorOutreachTarget>([
	"all",
	"role",
	"admins",
]);
const OPERATOR_ROLES = new Set<OperatorOutreachRole>(["manager", "superadmin"]);
const OPERATOR_CHANNELS = new Set<OperatorOutreachChannel>([
	"in_app",
	"telegram",
]);
...
function isOperatorTarget(value: unknown): value is OperatorOutreachTarget {
	return (
		typeof value === "string" &&
		OPERATOR_TARGETS.has(value as OperatorOutreachTarget)
	);
}
...
function isOperatorRole(value: unknown): value is OperatorOutreachRole {
	return (
		typeof value === "string" &&
		OPERATOR_ROLES.has(value as OperatorOutreachRole)
	);
}
...
function isOperatorPriority(value: unknown): value is OperatorOutreachPriority {
	return (
		typeof value === "string" &&
		OPERATOR_PRIORITIES.has(value as OperatorOutreachPriority)
	);
}
...
function normalizeOperatorChannels(value: unknown): OperatorOutreachChannel[] {
	...
	const channels = value.filter(
		(item): item is OperatorOutreachChannel =>
			typeof item === "string" &&
			OPERATOR_CHANNELS.has(item as OperatorOutreachChannel),
	);
	return Array.from(new Set(channels));
}

function operatorSeverity(
...
export function makeAdminOutreachRoutes(opts: {
	...
	app.post("/api/admin/outreach", async (c) => {
		...
		const scheduledAtRaw =
			typeof body.scheduledAt === "number" ? body.scheduledAt : null;
		const scheduledAt =
		...
		const hasStageSlug =
			typeof body.stageSlug === "string" && body.stageSlug.trim().length > 0;
		...
		const result = await withTenant(opts.db, tenantId, async (tx) => {
			...
			for (const lead of targetLeads) {
				...
				const source =
					identity.channelKind === "telegram_userbot" ? "userbot" : "bot";
	...
	app.post("/api/admin/outreach/operators", async (c) => {
		...
		const priority = isOperatorPriority(body.priority)
			? body.priority
			: "normal";
		const selectedChannels = normalizeOperatorChannels(body.channels);
		if (selectedChannels.length === 0)
			return c.json({ error: "channels required" }, 400);
		...
		const targetOperators = await withTenant(opts.db, tenantId, async (tx) => {
			...
			if (target === "role" && targetRole)
				filters.push(eq(admins.role, targetRole));
		...
		for (const operator of targetOperators) {
			...
			const sendResult = opts.notificationService
				? await opts.notificationService.sendDirectMessage(
