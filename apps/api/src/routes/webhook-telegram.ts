...
export function makeTelegramWebhookRoutes(opts: {
	 ...
	 * STT, дробление ответа, debounce (replyDelaySeconds). Резолвер собирается
	 * в apps/api index.ts.
	 */
	...
	app.post("/webhook/telegram/:slug", async (c) => {
		...
		if (got !== opts.webhookSecret) {
			opts.metrics?.webhookRequests.inc(1, {
		...
		if (entries.length === 0) {
			...
			return c.json(
				{ error: "no active telegram_bot channel for tenant" },
				404,
			);
		}
		...
		const botSettings: BotSettings = opts.resolveBotSettings
			? await opts
					.resolveBotSettings(entry.tenantId)
					.catch(() => DEFAULT_BOT_SETTINGS)
			: DEFAULT_BOT_SETTINGS;
		...
		if (botSettings.voiceStt) {
			await transcribeInboundVoice(inbound, {
				...
				resolveTranscriber: opts.resolveTranscriber
					...
					: null,
				downloadVoice: (mediaRef: MediaRef) =>
					adapter.rawClient.downloadFile(mediaRef.externalRef),
		...
		let result = await withTenant(opts.db, entry.tenantId, async (tx) => {
			...
			return processInbound(inbound, {
				...
				deferPostProcessing: Boolean(
					opts.memoryExtractor || opts.stageClassifier,
				),
				...(template ? { template } : {}),
				...(opts.sink ? { sink: opts.sink } : {}),
			});
		...
		if (!skipReply && result.replyDeferred && opts.replyStrategy) {
			// Debounce: если тенант задал паузу (botSettings.replyDelaySeconds) и это
			// «обычный» reply-ход (не media, не эскалация, есть текст) — не отвечаем
			// сразу, а ставим reply_due_at = now+delay. Новое сообщение/правка в окне
			// перезапишут → таймер сбрасывается; поллер replyDebounceTick сгенерит
			// один ответ по тишине.
			const delaySec = botSettings.replyDelaySeconds ?? 0;
			const canDebounce =
				delaySec > 0 &&
				!result.mediaOnly &&
			...
			if (canDebounce) {
				...
				await withTenant(opts.db, entry.tenantId, async (tx) => {
					await new ConversationsRepo({
						db: tx,
						tenantId: entry.tenantId,
					}).setReplyDueAt(result.conversationId, dueAt);
				});
				...
				const gen = await generateReplyAndEnqueue({
					...
					fallbackText: botSettings.fallbackText,
					handoffAfterFallbacks: botSettings.handoffAfterFallbacks,
					...(opts.sink ? { sink: opts.sink } : {}),
				});
		...
		opts.metrics?.webhookLatency.observe(elapsedSec, {
			channel: "telegram_bot",
		});
