...
import { expandCallbackQuery, wrapWithConciergeButtons } from "../lib/concierge-reply-markup.ts";
  ...
  app.post("/webhook/telegram/:slug", async (c) => {
    ...
    let result = await withTenant(opts.db, entry.tenantId, async (tx) => {
      ...
      return processInbound(inbound, {
        ...
        reply: opts.replyStrategy ? wrapWithConciergeButtons(opts.replyStrategy, opts.db) : null,
        deferReply: true,
