...
describe("admin provider orders", () => {
  it("exposes ops settings and metrics and blocks writes when disabled", async () => {
    ...
    const disableRes = await authReq("/api/admin/provider-orders/ops/settings", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
    ...
    const blockedRes = await authReq(`/api/admin/provider-orders/${orderId}/assign-provider`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("persists provider order audit rows under non-bypass RLS role", async () => {
    ...
    const assignRes = await rlsAuthReq(`/api/admin/provider-orders/${orderId}/assign-provider`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ providerId }),
    });
  ...
  it("runs operator actions for provider workflow", async () => {
    ...
    const providersRes = await authReq("/api/admin/provider-orders/providers?requestType=massage");
    expect(providersRes.status).toBe(200);
    const providers = (await providersRes.json()) as {
      items: Array<{ id: number; services: Array<{ serviceType: string }> }>;
    };
    expect(providers.items.some((provider) => provider.id === providerId)).toBe(true);

    const assignRes = await authReq(`/api/admin/provider-orders/${orderId}/assign-provider`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ providerId }),
    });
    ...
    const approveRes = await authReq(`/api/admin/provider-orders/${orderId}/approve-quote`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const offerRes = await authReq(`/api/admin/provider-orders/${orderId}/send-customer-offer`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
