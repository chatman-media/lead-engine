...
export function makeAdminTenantRoutes(opts: AdminTenantRoutesOpts): Hono {
	...
	app.get("/api/admin/tenant", async (c) => {
		...
		return c.json({
			tenant: { ...rest, botSettings: parseBotSettings(botSettingsJson) },
		});
	});
	...
	app.put("/api/admin/tenant/bot-settings", async (c) => {
		...
		const merged = await withTenant(opts.db, tenantId, async (tx) => {
			const [r] = await tx
				...
				.from(tenants)
				.where(eq(tenants.id, tenantId));
			const next = mergeBotSettings(
				parseBotSettings(r?.botSettingsJson ?? null),
				patch,
			);
			await tx
				.update(tenants)
				.set({
					botSettingsJson: serializeBotSettings(next),
					updatedAt: nowEpoch,
				})
				.where(eq(tenants.id, tenantId));
			return next;
	...
	app.put("/api/admin/tenant/reply-history-limit", async (c) => {
		...
		if (body.limit === null || body.limit === undefined || body.limit === "") {
			...
			if (!Number.isInteger(n) || n < 2 || n > 100) {
				return c.json(
					{ error: "limit must be an integer 2..100 or null" },
					400,
				);
			}
			limit = n;
	 ...
	 * null/0 → выключено. Диапазон 0..120. Сохраняется в bot_settings_json
	 * (поле replyDelaySeconds). Старая колонка tenants.reply_delay_seconds устарела.
	 */
	app.put("/api/admin/tenant/reply-delay-seconds", async (c) => {
		...
		if (
			...
			if (!Number.isInteger(n) || n < 0 || n > 120) {
				return c.json(
					{ error: "seconds must be an integer 0..120 or null" },
					400,
				);
			}
			seconds = n === 0 ? null : n;
		...
		return c.json({ ok: true, replyDelaySeconds: merged.replyDelaySeconds });
	});
