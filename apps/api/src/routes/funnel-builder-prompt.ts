import { ACTIVE_PHASES } from "@chatman-media/verticals";
import { FIELD_TYPES, STAGE_KINDS, STAGE_TYPES } from "./admin-funnel.ts";

export const FUNNEL_STAGE_SCHEMA_PROMPT = `СХЕМА stages (массив стадий по порядку):
{
  ...
  "fields": [
    {
      ...
      "options": ["вариант1", "вариант2"] (ТОЛЬКО для select/multiselect; можно [{ "value": "snake_case", "label": "Подпись" }])
    }
  ]
}`;

export const FUNNEL_GENERATION_RULES_PROMPT = `ПРАВИЛА:
...
export const FUNNEL_MULTI_REQUEST_PROMPT = `МУЛЬТИ-ЗАПРОС (один клиент ↔ несколько типов услуг):
...
export const FUNNEL_GENERATION_CONTRACT_PROMPT = [
	FUNNEL_STAGE_SCHEMA_PROMPT,
	FUNNEL_GENERATION_RULES_PROMPT,
	FUNNEL_MULTI_REQUEST_PROMPT,
].join("\n\n");

export const FUNNEL_FEWSHOT_EXAMPLES_PROMPT = `ПРИМЕРЫ (как из описания собрать качественную воронку — ориентируйся на них):
...
export function buildWorkflowFunnelSystemPrompt(): string {
...
${FUNNEL_FEWSHOT_EXAMPLES_PROMPT}`;
}

export function buildCopilotBuildFunnelActionPrompt(): string {
	return `2) Собрать кастомную воронку (когда собрал достаточно информации):
   { "type": "build_funnel", "stages": [ <стадии> ] }
   Используй тот же контракт стадий, что AI Workflow Builder:
