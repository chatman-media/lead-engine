...
async function activeFunnel(): Promise<{
	slug: string;
...
describe("admin-verticals install → vertical_template_id", () => {
	it("без auth → 401", async () => {
		if (!sql) return;
		const res = await app.request("/api/admin/verticals/exchange_v1/install", {
			method: "POST",
		});
	...
	it("install exchange_v1 ставит funnels.vertical_template_id = registry slug", async () => {
		if (!sql) return;
		const res = await authReq("/api/admin/verticals/exchange_v1/install", {
			method: "POST",
		});
	...
	it("install мульти-стилевого шаблона активирует ровно ОДИН стиль", async () => {
		...
		const res = await authReq("/api/admin/verticals/real_estate_v1/install", {
			method: "POST",
		});
		expect(res.status).toBe(200);
		...
		const seeded = rows.filter((r) =>
			REAL_ESTATE_STYLES.some((s) => s.slug === r.slug),
		);
		...
		expect(seeded.find((r) => r.isActive)?.slug).toBe(
			REAL_ESTATE_STYLES[0]!.slug,
		);
	});
});
...
describe("admin-verticals — GET list + неизвестный slug", () => {
	...
	it("POST install несуществующий slug → 404", async () => {
		if (!sql) return;
		const res = await authReq("/api/admin/verticals/nonexistent_v999/install", {
			method: "POST",
		});
