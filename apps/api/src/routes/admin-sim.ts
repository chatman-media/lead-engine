...
import type { QuoteCurrency, VerticalTemplate } from "@chatman-media/verticals";
...
// …), а не от платформенного дефолта: иначе THB-тенант получает персон, которые
// просят «песо». Брифы строятся из quote_asset тенанта (см. ensureBuiltinPersonas).
function buildPersonas(qc: QuoteCurrency): SimPersona[] {
  const QW_PL = qc.tabloWord.toLowerCase();
  const QW_GEN = qc.wordGen;
  const QC = qc.code;
  const bankLabel = qc.bankLabel;
  return [
    {
      id: "exchange_usdt",
    ...
    },
    {
      ...
      name: `Обменник — перевод на ${bankLabel}`,
      displayName: "Игорь Лапин",
      brief:
        `Ты — клиент, меняешь 60 000 рублей и хочешь получить переводом на ${bankLabel}${QC === "PHP" ? " (BDO)" : " (Bangkok Bank)"}. ` +
 ...
 * Брифы строятся под котируемую валюту ТЕНАНТА (THB/PHP/…), а не платформенный
 * дефолт — иначе THB-тенант получает персон, просящих «песо».
 *
 * Новые ключи вставляются. Уже существующие встроенные строки лечатся под валюту
 * тенанта (чинит старый сид, засеянный на платформенном PHP), НО только если их
 * НЕ редактировал оператор (`updated_at <= created_at`) и текст реально изменился.
 * `updated_at` при лечении не трогаем — строка остаётся «неотредактированной» и
 * самоисцеляется при будущих сменах валюты; кастомные персоны не затрагиваются.
 */
async function ensureBuiltinPersonas(db: Db, tenantId: number): Promise<void> {
  const qc = await getTenantQuoteCurrency(db, tenantId);
  const personas = buildPersonas(qc);
  await withTenant(db, tenantId, async (tx) => {
    await tx
      ...
      .onConflictDoUpdate({
        ...
        setWhere: sql`${simPersonas.isBuiltin} = true and ${simPersonas.updatedAt} <= ${simPersonas.createdAt} and (${simPersonas.brief} is distinct from excluded.brief or ${simPersonas.name} is distinct from excluded.name or ${simPersonas.displayName} is distinct from excluded.display_name)`,
      });
  });
}
