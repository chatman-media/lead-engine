  ...
  exchange: [
    {
      ...
      fields: [
        ...
        { slug: "amount_from", displayName: "Сумма (в активе-источнике)", fieldType: "number", required: true, aiExtractable: true, hint: "Например 335 для 335 USDT", position: 2 },
        { slug: "payout_method", displayName: "Способ получения THB", fieldType: "select", required: false, aiExtractable: true, hint: "Офис (код сотруднику) или банкомат (cardless)", position: 3,
          optionsJson: '[{"value":"office","label":"Офис (код)"},{"value":"atm","label":"Банкомат (cardless)"}]' },
        { slug: "arrival_date", displayName: "Дата прилёта", fieldType: "text", required: false, aiExtractable: true, hint: "Если клиент прилетает — предложи трансфер/Зелёный коридор", position: 4 },
      ],
    },
    {
      ...
      color: "#f59e0b",
      staleTimeoutDays: 1,
      // KYC обязателен если нет в системе.
      nextStages: ["kyc_collection", "awaiting_payment", "cancelled"],
      fields: [
    ...
    },
    {
      ...
      fields: [
        { slug: "customer_name", displayName: "Имя клиента", fieldType: "text", required: false, aiExtractable: true, position: 0 },
        { slug: "verification_video", displayName: "Видео-кружок", fieldType: "video", required: false, aiExtractable: false, hint: "Кружок «я такой-то, перевожу то-то» — уходит оператору на визуальную проверку", position: 1 },
        { slug: "verification_crm_id", displayName: "ID в CRM верификации", fieldType: "text", required: false, aiExtractable: false, hint: "Внутренний номер клиента в системе проверки личности", position: 2 },
      ],
    },
    {
      ...
      staleTimeoutDays: 1,
      supportMode: true,
      nextStages: ["payout", "cancelled"],
      fields: [
        { slug: "payment_proof_text", displayName: "Пруф оплаты (tx hash)", fieldType: "text", required: false, aiExtractable: true, hint: "Хеш транзакции / ссылка на explorer для крипты", position: 0 },
        { slug: "payment_proof_image", displayName: "Чек / скрин перевода", fieldType: "photo", required: false, aiExtractable: true, hint: "Развёрнутый чек для RUB/фиата", position: 1 },
        { slug: "payment_confirmed", displayName: "Оплата подтверждена", fieldType: "boolean", required: true, aiExtractable: false, position: 2 },
      ],
    },
    {
      slug: "payout",
      ...
      fields: [
        { slug: "payout_method", displayName: "Способ получения", fieldType: "select", required: false, aiExtractable: true, position: 0,
          optionsJson: '[{"value":"office","label":"Офис (код)"},{"value":"atm","label":"Банкомат (cardless)"}]' },
        { slug: "payout_location", displayName: "Офис / банк", fieldType: "text", required: false, aiExtractable: true, hint: "Какой офис (Бангтао) или банк банкомата (Kbank/Bangkok Bank/SCB)", position: 1 },
        { slug: "payout_code", displayName: "Код получения", fieldType: "text", required: false, aiExtractable: false, hint: "Код для офиса (Amount+Code) или cardless (телефон+код) — от провайдера/оператора", position: 2 },
      ],
    },
    {
      slug: "completed",
