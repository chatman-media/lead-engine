...
export type { SeedStage } from "../lib/funnel-validation.ts";

type FunnelVersionSource = "ai_apply" | "template_apply" | "manual_edit" | "rollback";
...
export const SEED_TEMPLATES: Record<string, SeedStage[]> = {
  ...
  exchange: [
    ...
    },
    {
      ...
      position: 7,
      color: "#34d399",
      // 3 дня до автозакрытия: checkin-напоминание клиенту (день 1) успевает
      // сработать раньше, чем stale-sweep закроет незаконченный обмен.
      staleTimeoutDays: 3,
      checkinIntervalDays: 1,
      supportMode: true,
    ...
    },
    {
      ...
      goal: `Зафиксировать факт поступления оплаты и фактическую сумму перед выдачей ${QUOTE_CURRENCY.code}.`,
      guidance:
  ...
  real_estate: [
    ...
    },
    {
      ...
      nextStages: ["noc_application", "mortgage_approval", "handover_support", "dld_transfer"],
      autoAdvanceCondition: '{"type":"all_required_fields_filled"}',
      fields: [
        {
          slug: "mou_date",
  ...
  // Аэропорт-трансфер: компактная воронка под мульти-сервисный обменный
  // тенант (обмен + трансфер + зелёный коридор). Цены называет ТОЛЬКО
  // оператор; интент-роутер заводит лид по слову «трансфер» и т.п.
  transfer: [
    {
      slug: "transfer_request",
      ...
      position: 0,
      color: "#3b82f6",
      goal: "Собрать маршрут поездки: откуда, куда, дата, время и число пассажиров.",
      guidance:
        "Тепло поприветствуйте клиента. Уточняйте по одному: откуда забрать (аэропорт/отель/адрес), куда ехать, дата и время подачи или номер рейса, сколько пассажиров и багажа. Цену не называйте — её подтвердит оператор на следующем шаге.",
      nextStages: ["transfer_offer", "transfer_lost"],
      autoAdvanceCondition: '{"type":"all_required_fields_filled"}',
      fields: [
        {
          slug: "pickup",
        ...
        },
        {
          ...
          required: true,
          aiExtractable: true,
          hint: "Дата поездки или прилёта",
          position: 2,
        },
        {
          slug: "time",
          displayName: "Время / рейс",
          fieldType: "text",
          required: false,
          aiExtractable: true,
          hint: "Время подачи или номер рейса",
          position: 3,
        },
        {
          slug: "passengers",
    ...
    },
    {
      ...
      staleTimeoutDays: 3,
      checkinIntervalDays: 1,
      goal: "Сообщить цену и класс авто от оператора и получить подтверждение брони.",
      guidance:
        "Передайте цену и класс автомобиля, как их указал оператор; цену не придумывайте. Ответьте на вопросы по поездке и дождитесь чёткого «да» перед подтверждением.",
      nextStages: ["transfer_fulfill", "transfer_lost"],
      autoAdvanceCondition: '{"type":"all_required_fields_filled"}',
      fields: [
        {
          slug: "price",
    ...
    },
    {
      ...
      color: "#06b6d4",
      staleTimeoutDays: 3,
      goal: "Подтвердить назначение водителя, держать клиента в курсе подачи и довести поездку до конца.",
      guidance:
        "Сообщите, что водитель назначен, передайте контакт и детали машины от оператора. Отвечайте оперативно на вопросы о статусе подачи; после поездки уточните, всё ли прошло хорошо.",
      nextStages: ["transfer_done", "transfer_lost"],
      fields: [
        {
          slug: "driver_contact",
          displayName: "Контакт водителя",
          fieldType: "text",
          required: false,
          aiExtractable: false,
          position: 0,
  ...
  // «Зелёный коридор» — VIP-встреча в аэропорту: встреча у выхода,
  // сопровождение через паспортный контроль без очереди, помощь с багажом.
  green_corridor: [
    {
      slug: "gc_request",
      displayName: "Заявка на встречу",
      kind: "intake",
      stageType: "form_fill",
      position: 0,
      color: "#3b82f6",
      goal: "Собрать данные для VIP-встречи в аэропорту: рейс, дата прилёта, число пассажиров, терминал.",
      guidance:
        "Поприветствуйте клиента и коротко объясните, что такое зелёный коридор: встреча у выхода, сопровождение через паспортный контроль без очереди, помощь с багажом. Уточняйте по одному: номер рейса, дата прилёта, сколько человек, терминал, если известен. Цену не называйте — её подтвердит оператор.",
      nextStages: ["gc_confirm", "gc_lost"],
      autoAdvanceCondition: '{"type":"all_required_fields_filled"}',
      fields: [
        {
          slug: "flight_number",
    ...
    },
    {
      slug: "gc_confirm",
      phase: "offer",
      displayName: "Подтверждение и цена",
      kind: "active",
      stageType: "awaiting_operator",
      position: 1,
      color: "#f59e0b",
      staleTimeoutDays: 3,
      checkinIntervalDays: 1,
      goal: "Озвучить стоимость встречи от оператора и получить подтверждение клиента.",
      guidance:
        "Передайте цену и условия, как их указал оператор; не придумывайте стоимость и не обещайте слот, который не подтверждён. Объясните, где и как встретит сопровождающий. Дождитесь явного согласия клиента.",
      nextStages: ["gc_meet", "gc_lost"],
      autoAdvanceCondition: '{"type":"all_required_fields_filled"}',
      fields: [
        {
          slug: "price",
    ...
    },
    {
      slug: "gc_meet",
      phase: "fulfill",
      displayName: "Встреча в аэропорту",
      kind: "active",
      stageType: "milestone",
      position: 2,
      color: "#06b6d4",
      staleTimeoutDays: 3,
      goal: "Подтвердить назначение встречающего и провести встречу по прилёте.",
      guidance:
        "Сообщите имя и контакт встречающего и точное место встречи (у выхода, с табличкой). Напомните клиенту в день прилёта. Будьте на связи, если рейс задерживается.",
      nextStages: ["gc_done", "gc_lost"],
      fields: [
        {
          slug: "greeter_contact",
          displayName: "Контакт встречающего",
          fieldType: "text",
          required: false,
          aiExtractable: false,
          position: 0,
    ...
    {
      slug: "gc_done",
      displayName: "Встреча состоялась",
      kind: "terminal_won",
      stageType: "milestone",
      position: 3,
      color: "#10b981",
...
> = {
  ...
  transfer: {
    displayName: "Трансфер из аэропорта",
    description: "Маршрут, цена от оператора, водитель и поездка.",
    verticalTemplateId: null,
  },
  green_corridor: {
    displayName: "Зелёный коридор",
    description: "VIP-встреча в аэропорту: рейс, цена, встречающий.",
    verticalTemplateId: null,
  },
  concierge: {
...
function listFunnelTemplateInfos() {
  ...
  return keys.map((key) => {
    ...
    const fieldsCount = stages.reduce((sum, stage) => sum + stage.fields.length, 0);
...
async function buildFunnelSnapshot(
  ...
  const stages = await tx
    ...
    .where(and(eq(stageDefinitions.tenantId, tenantId), eq(stageDefinitions.funnelId, funnel.id)))
  ...
  const fields =
    stages.length > 0
      ...
      : [];
  const fieldsByStage = fields.reduce<Record<number, typeof fields>>((acc, field) => {
  ...
  return {
    ...
    stages: stages.map((stage) => ({
      ...
      ...(stage.staleTimeoutDays !== null ? { staleTimeoutDays: stage.staleTimeoutDays } : {}),
      ...(stage.checkinIntervalDays !== null
        ? { checkinIntervalDays: stage.checkinIntervalDays }
        : {}),
      ...(stage.supportMode ? { supportMode: true } : {}),
      nextStages: stage.nextStages,
      ...(stage.autoAdvanceCondition ? { autoAdvanceCondition: stage.autoAdvanceCondition } : {}),
      ...(stage.configJson && stage.configJson !== "{}" ? { configJson: stage.configJson } : {}),
      ...(stage.goal ? { goal: stage.goal } : {}),
      ...(stage.guidance ? { guidance: stage.guidance } : {}),
      ...(stage.partnerWebhookUrl ? { partnerWebhookUrl: stage.partnerWebhookUrl } : {}),
...
async function snapshotFunnelById(
  ...
  const [funnel] = await tx
    .select()
    .from(funnels)
    .where(and(eq(funnels.tenantId, opts.tenantId), eq(funnels.id, opts.funnelId)))
    .limit(1);
...
async function snapshotFunnelForStageId(
  ...
  const [stage] = await tx
    ...
    .from(stageDefinitions)
    .where(and(eq(stageDefinitions.tenantId, opts.tenantId), eq(stageDefinitions.id, opts.stageId)))
    .limit(1);
...
async function snapshotFunnelForFieldId(
  ...
  const [field] = await tx
    ...
    .where(and(eq(stageFields.tenantId, opts.tenantId), eq(stageFields.id, opts.fieldId)))
    .limit(1);
...
export async function applyFunnelStages(
  ...
  return withTenant(db, tenantId, async (tx) => {
    ...
    let [funnel] = await tx
      ...
      .where(
        opts.targetFunnelId
          ? and(eq(funnels.tenantId, tenantId), eq(funnels.id, opts.targetFunnelId))
          : targetSlug
    ...
    const leadsOnFunnel =
      existingStageIds.length > 0
        ? await tx
            ...
            .where(
              and(eq(leads.tenantId, tenantId), inArray(leads.stageDefinitionId, existingStageIds)),
            )
        : [];
    ...
    for (const stageTpl of stages) {
      ...
      for (const fieldTpl of fields) {
        await tx.insert(stageFields).values({
          ...
          ...(fieldTpl.validationJson ? { validationJson: fieldTpl.validationJson } : {}),
...
async function loadFunnelData(tx: Db, tenantId: number, funnel: FunnelRow) {
  ...
  const fieldsByStage = fields.reduce<Record<number, typeof fields>>((acc, f) => {
...
export function makeAdminFunnelRoutes(opts: AdminFunnelRoutesOpts): Hono {
    ...
    const result = await withTenant(opts.db, tenantId, async (tx) => {
      ...
      const stageCounts =
        rows.length > 0
          ...
          : [];
      const stageCountMap = new Map(stageCounts.map((r) => [r.funnelId, Number(r.stagesCount)]));
      ...
      const leadCounts =
        rows.length > 0
          ? await tx
              ...
              .groupBy(stageDefinitions.funnelId)
          : [];
      const leadCountMap = new Map(leadCounts.map((r) => [r.funnelId, Number(r.leadsCount)]));
  ...
  app.get("/api/admin/funnels/:id/versions", async (c) => {
    ...
    const result = await withTenant(opts.db, tenantId, async (tx) => {
      ...
      const rows = await tx
        ...
        .from(funnelVersions)
        .where(and(eq(funnelVersions.tenantId, tenantId), eq(funnelVersions.funnelId, id)))
  ...
  app.get("/api/admin/funnels/:id/versions/:versionId", async (c) => {
    ...
    if (!Number.isFinite(id)) return c.json({ error: "bad id" }, 400);
    if (!Number.isFinite(versionId)) return c.json({ error: "bad versionId" }, 400);
  ...
  app.post("/api/admin/funnels/:id/versions/:versionId/rollback", async (c) => {
    ...
    const snapshot = parseFunnelSnapshot(row.snapshotJson);
    if (!snapshot) {
      return c.json({ error: "snapshot invalid", violations: ["invalid snapshot"] }, 422);
    }
    const validation = validateFunnel(snapshot.stages);
  ...
  app.post("/api/admin/funnels", async (c) => {
    ...
    const slug =
      typeof body.slug === "string" ? body.slug.trim().toLowerCase().replace(/\s+/g, "_") : "";
        ...
        tx
          ...
          .where(
            and(
              ...
              verticalTemplateId
                ? or(eq(funnels.verticalTemplateId, verticalTemplateId), eq(funnels.slug, template))
    ...
    const result = await applyFunnelStages(opts.db, tenantId, SEED_TEMPLATES[template]!, slug, {
      targetSlug: slug,
    });
  ...
  app.patch("/api/admin/funnels/:id", async (c) => {
    ...
    if (body.verticalTemplateId !== undefined) patch.verticalTemplateId = body.verticalTemplateId;
  ...
  app.post("/api/admin/funnel/seed", async (c) => {
    ...
    const result = await applyFunnelStages(opts.db, tenantId, stages, body.template, {
  ...
  app.get("/api/admin/funnel/analytics", async (c) => {
    ...
    const primary = c.req.query("primary") === "1" || c.req.query("primary") === "true";
    if (funnelIdRaw && !Number.isFinite(funnelId)) return c.json({ error: "bad funnelId" }, 400);

    const stages = await withTenant(opts.db, tenantId, async (tx) => {
      const resolvedFunnelId =
        funnelId ?? (primary ? ((await loadPrimaryActiveFunnel(tx, tenantId))?.id ?? null) : null);
      return tx
        .select({
    ...
    const currentCounts = await withTenant(opts.db, tenantId, async (tx) =>
      tx
        ...
        .from(leads)
        .where(and(eq(leads.tenantId, tenantId), inArray(leads.stageDefinitionId, stageIds)))
    ...
    const entryCounts = await withTenant(opts.db, tenantId, async (tx) =>
      tx
        ...
        .from(leadEvents)
        .where(and(eq(leadEvents.tenantId, tenantId), inArray(leadEvents.toState, stageSlugs)))
  ...
  app.get("/api/admin/funnel/phase-stats", async (c) => {
    ...
    for (const r of counts) {
      const n = Number(r.n);
      const p = r.stageDefinitionId != null ? phaseByStage.get(r.stageDefinitionId) : null;
