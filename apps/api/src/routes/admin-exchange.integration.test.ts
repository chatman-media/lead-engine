...
async function createRubOrderThroughTools() {
  ...
  const proof = (await must(tools.verify_exchange_payment, "verify_exchange_payment").execute({
...
describe("admin-exchange routes", () => {
  ...
  it("approves mocked rate-card and reloads exchange tools", async () => {
    if (!sql) return;
    reloadCalls = [];
    const res = await postJson(tokenA, "/api/admin/exchange/rate-card/approve", {
    ...
    expect(ratesBody.rates.map((rate) => rate.asset).sort()).toEqual(["RUB", "USDT"]);
    expect(ratesBody.rates.find((rate) => rate.asset === "RUB")?.quoteMode).toBe("divide");
  });
  ...
  it("patches operator fields and counts completed turnover", async () => {
    ...
    const patchRes = await patchJson(tokenA, `/api/admin/exchange/orders/${orderId}`, {
      status: "completed",
  ...
  it("issue-payout-code generates a code, sets TTL and delivers to client", async () => {
    ...
    const res = await postJson(tokenA, `/api/admin/exchange/orders/${orderId}/issue-payout-code`, {
      generate: true,
      ttlMinutes: 30,
      payoutLocation: "Bangtao office",
    });
    expect(res.status).toBe(200);
    ...
    expect(ob.some((r) => String(r.key).startsWith(`exch-payout-code-${orderId}`))).toBe(true);
  ...
  it("confirm-payment moves order to paid and notifies client (A2)", async () => {
    ...
    const res = await postJson(tokenA, `/api/admin/exchange/orders/${orderId}/confirm-payment`, {});
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
      ok: boolean;
      delivered: boolean;
  ...
  it("keeps exchange CRM isolated by tenant", async () => {
    ...
    const orderId = must(row, "exchange order").id;
    const getFromB = await authReq(tokenB, `/api/admin/exchange/orders/${orderId}`);
  ...
  it("rejects invalid order status patch", async () => {
    ...
    const orderId = must(row, "exchange order").id;
    const res = await patchJson(tokenA, `/api/admin/exchange/orders/${orderId}`, {
  ...
  it("kyc-contacts lists verified clients with masked passport and aggregates", async () => {
    ...
    const search = await authReq(tokenA, "/api/admin/exchange/kyc-contacts?q=ivanov");
    const searchBody = (await search.json()) as {
      contacts: Array<Record<string, unknown>>;
    };
    expect(searchBody.contacts.some((c) => c.contactId === contactId)).toBe(true);
    const miss = await authReq(tokenA, "/api/admin/exchange/kyc-contacts?q=nope-nobody");
  ...
  it("kyc-contacts includes contacts with passport docs but no operator decision", async () => {
    ...
    const search = await authReq(tokenA, "/api/admin/exchange/kyc-contacts?q=petrov&limit=1");
    const searchBody = (await search.json()) as {
      contacts: Array<Record<string, unknown>>;
      nextOffset: number | null;
    };
...
describe("admin-exchange settings (per-tenant частота/порог)", () => {
  ...
  it("PUT валидные → сохраняет; GET и планировщик видят", async () => {
    ...
    const got = (await (await authReq(tokenA, "/api/admin/exchange/settings")).json()) as {
      rateRefreshSec: number;
      feedStaleSec: number | null;
      handoffCustomerNotice: boolean;
    };
...
describe("admin-exchange rate-proposals (подтверждение курса #732)", () => {
  async function rubRate(tenantId: number) {
    const [row] = await withTenant(db, tenantId, (tx) =>
      tx
        .select({
          id: schema.exchangeRates.id,
          baseRate: schema.exchangeRates.baseRate,
        })
        .from(schema.exchangeRates)
        .where(
          and(
            eq(schema.exchangeRates.tenantId, tenantId),
            eq(schema.exchangeRates.asset, "RUB"),
            eq(schema.exchangeRates.network, ""),
          ),
        )
        .limit(1),
    );
    return must(row, "RUB rate");
  }
  ...
  async function insertPending(
    ...
    const asset = opts.asset ?? "RUB";
    const network = opts.network ?? "";
    const quoteMode = opts.quoteMode ?? "divide";
    const deviationPct = ((opts.next - opts.prev) / opts.prev) * 100;
    const [row] = await withTenant(db, tenantId, (tx) =>
      tx
        .insert(schema.exchangeRateProposals)
        .values({
          tenantId,
          asset,
          quoteAsset: "PHP",
          network,
          quoteMode,
          prevBaseRate: opts.prev,
          nextBaseRate: opts.next,
          deviationPct,
          severity: Math.abs(deviationPct) > 50 ? "hard" : "soft",
          status: "pending",
          source: "feed",
        })
        .returning(),
    );
    return must(row, "inserted proposal");
  }

  it("confirm применяет next к base + пересчитывает тиры, pending исчезает", async () => {
    if (!sql) return;
    const before = await rubRate(tenantA);
    const prev = Number(before.baseRate);
    const next = Number((prev * 1.06).toFixed(6));
    const proposal = await insertPending(tenantA, { prev, next });

    const listed = (await (await authReq(tokenA, "/api/admin/exchange/rate-proposals")).json()) as {
      proposals: Array<{ id: number }>;
    };
    expect(listed.proposals.some((p) => p.id === proposal.id)).toBe(true);

    const res = await postJson(
      tokenA,
      `/api/admin/exchange/rate-proposals/${proposal.id}/confirm`,
      {},
    );
    expect(res.status).toBe(200);

    const after = await rubRate(tenantA);
    expect(Number(after.baseRate)).toBeCloseTo(next, 6);

    const [tier] = await withTenant(db, tenantA, (tx) =>
      tx
        .select({ displayRate: schema.exchangeRateTiers.displayRate })
        .from(schema.exchangeRateTiers)
        .where(
          and(
            eq(schema.exchangeRateTiers.tenantId, tenantA),
    ...
    // тир пересчитан от нового base (deviationPct сохранён) → вырос ~ на 6%
    expect(Number(tier?.displayRate)).toBeCloseTo(next * 1.035294, 2);

    const empty = (await (await authReq(tokenA, "/api/admin/exchange/rate-proposals")).json()) as {
      proposals: Array<{ id: number }>;
    };
    expect(empty.proposals.some((p) => p.id === proposal.id)).toBe(false);
  });

  it("reject оставляет base прежним, pending исчезает", async () => {
    if (!sql) return;
    const before = await rubRate(tenantA);
    const prev = Number(before.baseRate);
    const proposal = await insertPending(tenantA, {
      prev,
      next: Number((prev * 1.1).toFixed(6)),
    });

    const res = await postJson(
      tokenA,
      `/api/admin/exchange/rate-proposals/${proposal.id}/reject`,
      {},
    );
    expect(res.status).toBe(200);

    const after = await rubRate(tenantA);
    expect(Number(after.baseRate)).toBeCloseTo(prev, 6);

    const list = (await (await authReq(tokenA, "/api/admin/exchange/rate-proposals")).json()) as {
      proposals: Array<{ id: number }>;
    };
    expect(list.proposals.some((p) => p.id === proposal.id)).toBe(false);
  });

  it("повторный confirm уже решённого → 409 (идемпотентность)", async () => {
    if (!sql) return;
    const before = await rubRate(tenantA);
    const prev = Number(before.baseRate);
    const proposal = await insertPending(tenantA, {
      prev,
      next: Number((prev * 1.04).toFixed(6)),
    });
  ...
  it("confirm при сдвинувшемся активном base → 409 stale", async () => {
    if (!sql) return;
    const before = await rubRate(tenantA);
    const current = Number(before.baseRate);
    // снапшот prev намеренно не совпадает с текущим base → гонка фид↔confirm
    const proposal = await insertPending(tenantA, {
      prev: Number((current * 0.5).toFixed(6)),
      next: Number((current * 1.05).toFixed(6)),
    });
    const res = await postJson(
      tokenA,
      `/api/admin/exchange/rate-proposals/${proposal.id}/confirm`,
    ...
    // base не тронут
    const after = await rubRate(tenantA);
    expect(Number(after.baseRate)).toBeCloseTo(current, 6);
    // cleanup — освобождаем pending-слот направления
    await postJson(tokenA, `/api/admin/exchange/rate-proposals/${proposal.id}/reject`, {});
  });

  it("RLS: pending тенанта A невидим и неподтверждаем тенантом B", async () => {
    if (!sql) return;
    const proposal = await insertPending(tenantA, {
      asset: "USDT",
      network: "trc20",
    ...
    // B не видит чужой pending
    const listB = (await (await authReq(tokenB, "/api/admin/exchange/rate-proposals")).json()) as {
      proposals: Array<{ id: number }>;
    };
    expect(listB.proposals.some((p) => p.id === proposal.id)).toBe(false);
    // B не может подтвердить чужой id → 404 (tenant-scope)
    const confB = await postJson(
      tokenB,
      `/api/admin/exchange/rate-proposals/${proposal.id}/confirm`,
      {},
    );
    ...
    // A видит свой
    const listA = (await (await authReq(tokenA, "/api/admin/exchange/rate-proposals")).json()) as {
      proposals: Array<{ id: number }>;
    };
    expect(listA.proposals.some((p) => p.id === proposal.id)).toBe(true);
    // cleanup
    await postJson(tokenA, `/api/admin/exchange/rate-proposals/${proposal.id}/reject`, {});
  });

  it("тумблер requireRateConfirmation сохраняется (PUT→GET)", async () => {
    if (!sql) return;
    // PUT — полная замена настроек; rateRefreshSec обязателен.
    const put = await authReq(tokenA, "/api/admin/exchange/settings", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ rateRefreshSec: 180, requireRateConfirmation: true }),
    });
    expect(put.status).toBe(200);
    const got = (await (await authReq(tokenA, "/api/admin/exchange/settings")).json()) as {
      requireRateConfirmation: boolean;
    };
    expect(got.requireRateConfirmation).toBe(true);
    // вернуть дефолт, чтобы не влиять на другие тесты
    await authReq(tokenA, "/api/admin/exchange/settings", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ rateRefreshSec: 180, requireRateConfirmation: false }),
    });
  });
});
