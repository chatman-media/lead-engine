...
export function makeAdminLeadsRoutes(opts: AdminLeadsRoutesOpts): Hono {
		...
		const result = await withTenant(opts.db, tenantId, async (tx) => {
			...
			const fields = stageDef
				? await tx
						.select()
						.from(stageFields)
						.where(eq(stageFields.stageId, stageDef.id))
				: [];
	...
	app.patch("/api/admin/leads/:id/stage", async (c) => {
		...
		if (!stageDefinitionId)
			return c.json({ error: "stageDefinitionId required" }, 400);

		const now = Math.floor(Date.now() / 1000);
		let transitionBlocked = false;
		let webhookPayload: StageChangedPayload | null =
			null as StageChangedPayload | null;
		...
		await withTenant(opts.db, tenantId, async (tx) => {
			...
			const [newStage] = await tx
				...
				.where(
					and(
						eq(stageDefinitions.id, stageDefinitionId),
			...
			let prevStage: { id: number; slug: string; displayName: string } | null =
				null;
			if (
				lead.stageDefinitionId &&
				lead.stageDefinitionId !== stageDefinitionId &&
				!force
			) {
			...
			webhookPayload = {
				...
				from: prevStage
					? {
							...
							slug: prevStage.slug,
							displayName: prevStage.displayName,
						}
					: null,
				to: {
	...
	app.post("/api/admin/leads/:id/send-photo", async (c) => {
		...
		const photoRef =
			typeof body.photoRef === "string" ? body.photoRef.trim() : "";
		const caption =
			typeof body.caption === "string"
				? body.caption.trim().slice(0, 1024)
				: "";
		if (!photoRef)
		...
		const outcome = await withTenant(opts.db, tenantId, async (tx) => {
			...
			// Активный канал клиента (приоритет: telegram — sendPhoto by file_id/url).
			...
			// Самая свежая беседа контакта (для привязки outbound-строки).
			const [conv] = await tx
			...
			// messages.conversation_id NOT NULL — пишем строку в транскрипт только
			// если беседа есть. Иначе просто ставим outbound (его conv_id nullable).
			if (conv?.id) {
