...
beforeAll(async () => {
	...
	const sb = await app.request("/api/auth/signup", {
		...
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({
			email: "conv-b@demo.io",
			password: "strong-pwd-12345",
		}),
	});
	const sbb = (await sb.json()) as {
		token: string;
		admin: { tenantId: number };
	};
	tokenB = sbb.token;
...
describe("admin-conversations", () => {
	...
	it("GET list → 3 conversations отсортированы DESC по lastMessageAt", async () => {
		...
		expect(body.items[0]!.lastMessageAt).toBeGreaterThan(
			body.items[1]!.lastMessageAt,
		);
		expect(body.items[1]!.lastMessageAt).toBeGreaterThan(
			body.items[2]!.lastMessageAt,
		);
	...
	it("pagination: cursor возвращает следующую страницу", async () => {
		...
		const page2 = (await (
			await authReq(
				tokenA,
				`/api/admin/conversations?limit=2&cursor=${page1.nextCursor}`,
			)
	...
	it("GET /:id → возвращает conversation + messages в chronological order", async () => {
		...
		expect(body.messages[0]!.createdAt).toBeLessThan(
			body.messages[4]!.createdAt,
		);
	...
	it("GET /:id → возвращает operator-bot side-effect metadata for activity", async () => {
		if (!sql) return;
		const id = conversationIdsA[0]!;
		const now = Math.floor(Date.now() / 1000);
		await db.insert(messages).values({
		...
		expect(res.status).toBe(200);
		const body = (await res.json()) as {
			messages: Array<{ text: string; metaJson: string | null }>;
		};
		const operatorBotMessage = body.messages.find((m) =>
			m.metaJson?.includes("operator-bot-preview"),
		);
		expect(operatorBotMessage?.text).toBe("✅ Верификация пройдена.");
		const meta = JSON.parse(String(operatorBotMessage?.metaJson)) as Record<
			string,
			unknown
		>;
	...
	it("GET /:id для чужого conversation → 404 (cross-tenant)", async () => {
		...
		const res = await authReq(
			tokenB,
			`/api/admin/conversations/${conversationIdsA[0]}`,
		);
...
describe("GET /api/admin/conversations/:id/operator-handoffs", () => {
	it("returns only operator handoffs for this tenant conversation", async () => {
		if (!sql) return;
		const id = conversationIdsA[0]!;
		const otherId = conversationIdsA[1]!;
		const now = Math.floor(Date.now() / 1000);
		...
		await db.insert(adminNotifications).values([
			{
				...
				body: "Клиент прислал видео. Проверьте документы.",
				dedupKey: `operator_handoff_required:${id}:kyc_review`,
				createdAt: now + 1,
			},
			{
				tenantId: tenantA,
				...
				body: "Клиент прислал чек.",
				dedupKey: `operator_handoff_required:${otherId}:payment_review`,
				createdAt: now + 2,
			},
			{
				tenantId: tenantA,
			...
			},
			{
				...
				kind: "operator_handoff_required",
				title: "Чужой tenant",
				body: "Не должен быть виден tenant A.",
				dedupKey: `operator_handoff_required:${id}:kyc_review`,
				createdAt: now + 4,
			},
		]);
		...
		expect(body.items[0]!.body).toContain("видео");
	});

	it("returns 404 for another tenant conversation", async () => {
		if (!sql) return;
		const res = await authReq(
			tokenB,
			`/api/admin/conversations/${conversationIdsA[0]}/operator-handoffs`,
		);
		expect(res.status).toBe(404);
	});
});
...
describe("POST /api/admin/conversations/:id/reply", () => {
	...
	it("invalid id → 400", async () => {
		...
		const res = await authReq(
			tokenA,
			"/api/admin/conversations/not-num/reply",
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ text: "hi" }),
