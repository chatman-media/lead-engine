...
describe("admin-director-hooks CRUD", () => {
  ...
  it("POST создаёт хук → 201, виден в GET", async () => {
    if (!sql) return;
    const res = await req("POST", BASE, { name: "Срочность", body: "Курс фиксируется на 15 минут", triggerHint: "когда торгуется" });
    expect(res.status).toBe(201);
    const hook = (await res.json()) as { id: number; name: string; triggerHint: string | null; isActive: boolean };
  ...
  it("PATCH /:id обновляет; bad id → 400; not found → 404", async () => {
    ...
    expect((await req("PATCH", `${BASE}/${id}`, { name: "Срочность 2", isActive: false, triggerHint: "" })).status).toBe(200);
    expect((await req("PATCH", `${BASE}/0`, { name: "x" })).status).toBe(400);
    expect((await req("PATCH", `${BASE}/999999`, { name: "x" })).status).toBe(404);

    const item = ((await (await req("GET", BASE)).json()) as { items: Array<{ name: string; isActive: boolean; triggerHint: string | null }> }).items[0]!;
  ...
  it("PATCH /reorder: массив id → ok; не-массив → 400", async () => {
    ...
    const ids = ((await (await req("GET", BASE)).json()) as { items: Array<{ id: number }> }).items.map((h) => h.id);
