...
import { SCOOTER_V1 } from "@chatman-media/vertical-scooter";
import { VISA_V1 } from "@chatman-media/vertical-visa";
...
import { makeAdminRbac } from "./middleware/admin-rbac.ts";
...
import { makeAdminProviderOrdersRoutes } from "./routes/admin-provider-orders.ts";
...
async function main() {
	...
	app.use("/api/admin/*", makeAdminRbac());
	app.use(
		"/api/superadmin/*",
		makeRequireAuth({ db: db as never, secret: cfg.authSecret }),
	);
	...
	if (embedderResolver) {
		app.route(
			"/",
			makeAdminKbRoutes({ db, resolveEmbedder: embedderResolver }),
		);
		log.info("admin-kb routes enabled (KB upload + list + delete)");
	} else {
		log.info(
			"admin-kb routes disabled — no tenant has embed config (DB or env)",
		);
	}
