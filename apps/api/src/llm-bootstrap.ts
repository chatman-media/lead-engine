...
function makeSupportModeResolver(db: Db) {
  return async (input: { tenantId: number; contactId: number }): Promise<boolean> => {
    ...
    const open = rows.filter((r) => r.kind !== "terminal_won" && r.kind !== "terminal_lost");
...
export function makeRequestContextResolver(db: Db) {
  return async (input: { tenantId: number; contactId: number }): Promise<string | null> => {
    const rows = await db
      .select({
      ...
      .from(leads)
      .leftJoin(stageDefinitions, eq(leads.stageDefinitionId, stageDefinitions.id))
      .where(and(eq(leads.tenantId, input.tenantId), eq(leads.userId, input.contactId)))
      .orderBy(desc(leads.updatedAt));
    const open = rows.filter((r) => r.kind !== "terminal_won" && r.kind !== "terminal_lost");
...
export function makeLeadFunnelResolver(db: Db) {
  return async (input: {
    ...
    const open = rows.find(
      (r) => r.kind !== "terminal_won" && r.kind !== "terminal_lost" && r.slug !== null,
    );
    return open?.slug
...
function makeKbScopeResolver(db: Db) {
  return async (input: { tenantId: number; contactId: number }): Promise<KbScope | null> => {
...
export function exchangeOrderPolicyStateFromOrder(order: OrderRow): ExchangeOrderPolicyState {
  return {
    id: order.id,
    status: order.status,
...
export function makeExchangeCollectedResolver(db: Db) {
  return async (input: { tenantId: number; conversationId: number; contactId: number }) => {
...
export function makeExchangeQuoteCurrencyResolver(db: Db) {
  return (input: { tenantId: number }) => getTenantQuoteCurrency(db, input.tenantId);
}
...
export function makeToolsResolver(opts: {
  ...
  async function resolveTools(input: {
    ...
    if (multiRequestEnabled === undefined) {
      multiRequestEnabled = await tenantSupportsMultiRequest(db, input.tenantId).catch(() => false);
...
function makeToolCallRecorder(db: Db, source: AgentToolCallSource) {
  return async (input: {
    ...
    const calls =
      input.toolCalls ?? (input.telemetry ? toolCallsFromTelemetry(input.telemetry) : []);
...
function makeExchangeResponseGuardFlagResolver(db: Db) {
  return async (input: { tenantId: number }): Promise<boolean> => {
    try {
        ...
        const [row] = await tx
          ...
          .where(
            and(
              ...
              eq(tenantFeatureFlags.featureKey, EXCHANGE_RESPONSE_GUARD_FEATURE_KEY),
            ),
          )
          .limit(1);
      ...
      console.warn("[llm-bootstrap] failed to resolve exchange response guard flag:", err);
      return true;
    }
  };
}
...
function makeExchangeCustomerNoticeResolver(db: Db) {
  return async (input: { tenantId: number }): Promise<boolean> => {
    try {
      ...
      console.warn("[llm-bootstrap] failed to resolve exchange handoff notice setting:", err);
      return true;
    }
  };
}
...
// #730 — язык ответа клиенту: каскад conversations.detected_lang (#735) →
// tenant defaultLang (bot_settings_json) → DEFAULT_LANG. Дешёвый (два
// индексированных select), параллелится в loadTurnContext. Прокидывается в
// RagTurnContext.lang → answerWithRag → composeSystemPrompt (директива языка).
function makeLangResolver(db: Db) {
  return async (input: { tenantId: number; conversationId: number }) => {
    try {
      return await withTenant(db, input.tenantId, async (tx) => {
        const [conv] = await tx
        ...
        const [tenantRow] = await tx
          ...
          .where(eq(tenants.id, input.tenantId))
          .limit(1);
        const settings = parseBotSettings(tenantRow?.json ?? null);
        return effectiveLang({
          detectedLang: conv?.detectedLang ?? null,
          tenantDefaultLang: settings.defaultLang,
        });
      });
    } catch (err) {
      console.warn("[llm-bootstrap] failed to resolve dialog lang:", err);
      return DEFAULT_LANG;
    }
  };
}
...
export function makeReplyStrategy(
  ...
  const resolveExchangeResponseGuardEnabled = makeExchangeResponseGuardFlagResolver(db);
  const resolveExchangeCustomerNoticeEnabled = makeExchangeCustomerNoticeResolver(db);
  const resolveHistoryLimit = makeHistoryLimitResolver(db);
  const resolveGenerationParams = makeGenerationParamsResolver(db);
  const resolveLang = makeLangResolver(db);
  ...
  const loadTurnContext = async (input: RagTurnInput): Promise<RagTurnContext> => {
    ...
    const [
      ...
      isExchange
        ? resolveExchangePolicyState({
            ...
            console.warn("[llm-bootstrap] failed to resolve exchange policy state:", err);
            return null;
          })
      ...
      isExchange
        ...
        : Promise.resolve(null),
      isExchange ? resolveExchangeResponseGuardEnabled({ tenantId }) : Promise.resolve(true),
      isExchange
        ...
        : Promise.resolve(null),
      isExchange ? resolveExchangeCustomerNoticeEnabled({ tenantId }) : Promise.resolve(true),
      resolveLang({ tenantId, conversationId }),
    ]);
    ...
    return {
      ...
      exchangePolicyState,
      exchangeCollected,
      exchangeResponseGuardEnabled: exchangeResponseGuardEnabled && leadFunnelIsExchange,
      exchangeCustomerNoticeEnabled,
      lang,
    };
  };
...
export function makeTranscriberResolver(
  ...
  return (tenantId: number) => {
    ...
    return buildTranscriber(cfg.provider, apiKey, cfg.baseUrl, dedicated ? cfg.model : undefined);
  };
}
