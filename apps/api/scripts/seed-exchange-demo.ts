...
interface DemoOrder {
  ...
  rate: number;
  amountToThb: number;
  paymentMethod: "crypto_transfer" | "sbp_qr" | "card_transfer" | "bank_transfer" | "cash";
  paymentRail: string;
  payoutMethod: "office_cash" | "cardless_atm" | "courier_cash" | "thai_bank_transfer" | "atm";
...
async function freshenSeededRates(input: {
  ...
  try {
    const usdtMarket = await fetchMarketBaseRate("USDT", "multiply", input.currency.code);
    const rubMarket = await fetchMarketBaseRate("RUB", "divide", input.currency.code);
    const usdMarket = await fetchMarketBaseRate("USD", "multiply", input.currency.code);
      ...
      for (const tier of tiers) {
        ...
        await tx
          ...
          .set({
            ...
            displayRate: roundTo(market * (1 + Number(tier.deviationPct) / 100), 2),
            updatedAt: input.now,
          })
          .where(
            and(eq(exchangeRateTiers.tenantId, input.tenantId), eq(exchangeRateTiers.id, tier.id)),
          );
      }
    });
...
function buildSampleLeads(currency: QuoteCurrency, live: LiveDemoRates | null): DemoLead[] {
...
function parseArgs(): Args {
  const out: Partial<Args> = {
    ...
    operatorEmail: process.env.EXCHANGE_DEMO_OPERATOR_EMAIL ?? DEFAULT_OPERATOR_EMAIL,
    ...
    telegramBotUsername: process.env.EXCHANGE_DEMO_TELEGRAM_BOT_USERNAME ?? null,
    llmProvider:
      process.env.EXCHANGE_DEMO_LLM_PROVIDER ?? process.env.LLM_PROVIDER ?? DEFAULT_LLM_PROVIDER,
    llmModel: process.env.EXCHANGE_DEMO_LLM_MODEL ?? process.env.LLM_MODEL ?? DEFAULT_LLM_MODEL,
    llmApiKey: process.env.EXCHANGE_DEMO_LLM_API_KEY ?? process.env.LLM_API_KEY ?? null,
    llmBaseUrl: process.env.EXCHANGE_DEMO_LLM_BASE_URL ?? process.env.LLM_BASE_URL ?? null,
  ...
  for (const arg of process.argv.slice(2)) {
    ...
    else if (rawKey === "telegram-bot-username") out.telegramBotUsername = value;
  ...
  const llmProvider = normalizeLlmProvider(out.llmProvider ?? DEFAULT_LLM_PROVIDER);
  ...
  return {
    ...
    telegramBotUsername: normalizeTelegramUsername(out.telegramBotUsername ?? null),
    llmProvider,
    llmModel,
...
function normalizeLlmProvider(value: string): LlmProvider {
  ...
  throw new Error("LLM provider must be one of: openai, openrouter, ollama, anthropic");
}
...
async function seedChannelsAndLlm(input: {
  ...
  return withTenant(input.db, input.tenantId, async (tx) => {
    ...
    if (input.args.telegramBotToken) {
      telegramUsername =
        input.args.telegramBotUsername ?? `${input.args.slug.replace(/-/g, "_")}_bot`;
    ...
    return {
      ...
      telegramChannelId,
      telegramUsername,
      llmHasSecret: input.args.llmProvider === "ollama" || chatSecretRef != null,
    };
  });
}
...
async function seedDemoFunnels(input: {
  ...
  for (const item of DEMO_FUNNELS) {
    const seeded = await seedFunnelByKey(input.db, input.tenantId, item.key, input.ownerAdminId);
    if ("error" in seeded) throw new Error(`seedFunnel(${item.key}): ${seeded.error}`);
    await withTenant(input.db, input.tenantId, async (tx) => {
      await tx
        .update(funnels)
        .set({
        ...
        .where(and(eq(funnels.tenantId, input.tenantId), eq(funnels.id, seeded.funnelId)));
    });
...
async function seedServiceCatalog(input: {
  ...
  const items = [
    ...
    },
    {
      ...
      category: "transfer",
      description: "трансфер, аэропорт, NAIA, встретить, довезти, минивэн, такси",
      funnelKey: "transfer",
      sortOrder: 1,
    },
    {
  ...
  await withTenant(input.db, input.tenantId, async (tx) => {
    for (const item of items) {
      ...
      if (!funnelId) throw new Error(`catalog: no funnel for ${item.funnelKey}`);
      await tx
  ...
  await withTenant(input.db, input.tenantId, async (tx) => {
    for (const hook of hooks) {
      await tx
        ...
        .where(and(eq(directorHooks.tenantId, input.tenantId), eq(directorHooks.name, hook.name)));
...
async function seedSalesStyle(input: {
  ...
  return withTenant(input.db, input.tenantId, async (tx) => {
    const [existing] = await tx
      ...
      .from(styles)
      .where(
        and(eq(styles.tenantId, input.tenantId), eq(styles.slug, slug), isNull(styles.deletedAt)),
      )
      .limit(1);
    let styleId: number;
    if (existing) {
    ...
    await tx
      ...
      .where(and(eq(funnels.tenantId, input.tenantId), eq(funnels.id, input.exchangeFunnelId)));
    return { styleId, slug };
  });
}
...
async function seedSampleKb(input: { db: Db; tenantId: number; now: number }): Promise<number> {
  let count = 0;
  await withTenant(input.db, input.tenantId, async (tx) => {
...
async function cleanDemoRows(input: { db: Db; tenantId: number; slug: string }): Promise<void> {
  await withTenant(input.db, input.tenantId, async (tx) => {
    await tx
...
async function loadStageContext(input: { db: Db; tenantId: number; funnelIds: number[] }) {
  return withTenant(input.db, input.tenantId, async (tx) => {
    ...
    for (const field of fieldRows) {
      const fields = fieldIdByStage.get(field.stageSlug) ?? new Map<string, number>();
...
async function seedDemoWorkflow(input: {
  ...
  await withTenant(input.db, input.tenantId, async (tx) => {
    for (let i = 0; i < SAMPLE_LEADS.length; i++) {
      ...
      if (!stageId) throw new Error(`exchange stage not found: ${sample.stage}`);
      ...
      const [conversation] = await tx
        ...
        .values({
          ...
          status: ["payout_or_completion", "transfer_done", "gc_done"].includes(sample.stage)
            ? "resolved"
            : "open",
      ...
      if (!conversation) throw new Error(`conversation insert failed: ${sample.name}`);
      ...
      if (sample.order) {
        const rateExpiresAt =
          sample.order.status === "completed" || sample.order.status === "cancelled"
            ? null
            : input.now + 30 * 60;
        await tx.insert(exchangeOrders).values({
          ...
          payoutCodeExpiresAt: sample.order.payoutCode ? input.now + 3600 : null,
          status: sample.order.status,
          requisitesJson: sample.order.requisitesJson
            ? JSON.stringify(sample.order.requisitesJson)
            : null,
          proofJson: sample.order.proofJson ? JSON.stringify(sample.order.proofJson) : null,
          riskJson: sample.order.riskJson ? JSON.stringify(sample.order.riskJson) : null,
          rateExpiresAt,
          idempotencyKey: `${DEMO_SEED}:${input.slug}:${sample.key}`,
          completedAt: sample.order.status === "completed" ? input.now - 900 : null,
          createdAt,
          updatedAt: input.now,
        });
        orderCount++;
...
async function setTelegramWebhook(args: Args): Promise<boolean> {
  if (!args.telegramBotToken || !args.publicUrl || !args.setTelegramWebhook) return false;
  ...
  const response = await fetch(`https://api.telegram.org/bot${args.telegramBotToken}/setWebhook`, {
    method: "POST",
    body: form,
  });
  if (!response.ok) {
    const body = await response.text();
    throw new Error(`Telegram setWebhook failed ${response.status}: ${body.slice(0, 300)}`);
  }
  const body = (await response.json()) as {
    ok?: boolean;
    description?: string;
  };
  if (!body.ok) throw new Error(`Telegram setWebhook rejected: ${body.description ?? "unknown"}`);
  return true;
}
...
async function readinessCounts(input: { db: Db; tenantId: number }) {
  return withTenant(input.db, input.tenantId, async (tx) => {
    const rates = await tx
      ...
      .from(exchangeRates)
      .where(and(eq(exchangeRates.tenantId, input.tenantId), eq(exchangeRates.isActive, true)));
    const tiers = await tx
      .select({ id: exchangeRateTiers.id })
      .from(exchangeRateTiers)
      .where(
        and(eq(exchangeRateTiers.tenantId, input.tenantId), eq(exchangeRateTiers.isActive, true)),
      );
    ...
    const channelRows = await tx
      ...
      .from(channels)
      .where(and(eq(channels.tenantId, input.tenantId), eq(channels.status, "active")));
...
async function main() {
  ...
  if (masterKeyHex === MASTER_KEY_PLACEHOLDER) {
    throw new Error("PLATFORM_MASTER_KEY must be the real 64-hex key, not the placeholder");
  }
  ...
  try {
      ...
      JSON.stringify(
        {
          ...
          funnels: Object.fromEntries(
            [...funnelsByKey.entries()].map(([key, value]) => [
              key,
              {
                ...
                verticalTemplateId:
                  DEMO_FUNNELS.find((f) => f.key === key)?.verticalTemplateId ?? null,
              },
            ]),
          ),
          ...
          quote: {
            asset: currency.code,
            liveRates: liveRates ?? "feed unavailable — static fallback rates used",
          },
          sampleKbDocs,
    ...
    console.log("  - Diagnostics: GET /api/admin/diagnostics as the owner admin");
  } finally {
...
main().catch((err) => {
  console.error("[seed-exchange-demo] FAILED:", err instanceof Error ? err.message : err);
  process.exit(1);
});
