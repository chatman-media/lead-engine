...
function parseArgs(): Args {
  const out: Args = {
    ...
    maxTurns: Number(process.env.EXCHANGE_DEMO_REHEARSAL_MAX_TURNS ?? DEFAULT_MAX_TURNS),
    personaIds: (process.env.EXCHANGE_DEMO_REHEARSAL_PERSONAS?.split(",") ?? DEFAULT_PERSONA_IDS)
      .map((item) => item.trim())
      .filter(Boolean),
  ...
  for (const arg of process.argv.slice(2)) {
    ...
    else if (rawKey === "max-turns") out.maxTurns = parsePositiveInt(value, "--max-turns");
  ...
  if (out.personaIds.length === 0) throw new Error("at least one persona id is required");
  return out;
}
...
function printRunResult(run: number, body: ExchangeEvalResponse): boolean {
  ...
  for (const row of body.report) {
    ...
    const reasons = row.reasons && row.reasons.length > 0 ? ` - ${row.reasons.join("; ")}` : "";
...
main().catch((err) => {
  console.error("[rehearse-exchange-demo] FAILED:", err instanceof Error ? err.message : err);
  process.exit(1);
});
