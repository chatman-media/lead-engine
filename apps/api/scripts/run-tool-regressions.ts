...
export function parseArgs(
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (raw === "--help" || raw === "-h") args.help = true;
      ...
      const next = argv[index + 1];
      if (!next) return { error: "--token-env requires an environment variable name" };
      tokenEnv = next;
      index += 1;
      ...
      if (!parsed) return { error: "--status must be active, archived, or all" };
      status = parsed;
      index += 1;
  ...
  if (args.help) return { ...args, ...outputArgs, source: { kind: "file", filePath } };
  if (!apiBase) return { ...args, ...outputArgs, source: { kind: "file", filePath } };
  if (fileExplicit) return { error: "--file and --api-base cannot be used together" };
  if (!isValidUrl(apiBase)) return { error: "--api-base must be a valid URL" };

  const resolvedToken = token ?? env[tokenEnv];
  if (!resolvedToken) return { error: `--api-base requires --token or ${tokenEnv}` };
...
export async function loadRegressionCorpus(args: CliArgs): Promise<LoadedRegressionCorpus> {
...
export async function main() {
  ...
  const report = runToolCallRegressionCases({
    raw: corpus.raw,
    includeArchived:
      args.includeArchived || (args.source.kind === "api" && args.source.status === "archived"),
...
export async function writeReportOutputs(report: ToolCallRegressionReport, args: CliArgs) {
  if (args.outJsonPath) {
    await writeTextFile(args.outJsonPath, `${JSON.stringify(report, null, 2)}\n`);
  }
...
export function renderToolRegressionJunit(report: ToolCallRegressionReport): string {
...
export function renderToolRegressionMarkdown(report: ToolCallRegressionReport): string {
  const lines = [
  ...
  for (const result of failures.slice(0, 20)) {
    ...
    for (const failure of result.failures) {
      lines.push(`  - ${failure.path}: expected ${failure.expected}; actual ${failure.actual}`);
    }
  }
  if (failures.length > 20) {
    lines.push(`- ${failures.length - 20} more failed cases omitted from summary.`);
  }
...
function printSummary(report: ToolCallRegressionReport) {
  ...
  if (skipReasons.length > 0) {
    console.log(`Skipped: ${skipReasons.map(([reason, count]) => `${reason}=${count}`).join(" ")}`);
  }
}
...
function formatCaseFailures(failures: ToolCallRegressionReport["results"][number]["failures"]) {
  return failures
    .map((failure) =>
      [
        `line=${failure.line}`,
