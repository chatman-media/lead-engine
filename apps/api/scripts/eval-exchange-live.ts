...
async function loadSeedSnapshot(args: Args): Promise<{
	...
	if (!databaseUrl) {
		throw new Error(
			"DATABASE_URL is required when --tenant/--tenant-id is used",
		);
	}
	...
	const [tenant] = await db
		...
		.where(
			args.tenantId
				? eq(tenants.id, args.tenantId)
				: eq(tenants.slug, args.tenantSlug ?? ""),
		)
		.limit(1);
	if (!tenant) {
		await close();
	...
	const seed = await withTenant(db, tenant.id, async (tx) => {
		const [channelCount] = await tx
			...
			.from(channels)
			.where(
				and(eq(channels.tenantId, tenant.id), eq(channels.status, "active")),
			);
		const [rateCount] = await tx
			...
			.where(
				and(
					eq(exchangeRates.tenantId, tenant.id),
...
async function main() {
	...
	try {
		if (args.requireSeed && seed) {
			if (seed.activeChannels === 0)
				throw new Error("Seed check failed: no active channels");
			if (seed.activeRates === 0)
				throw new Error("Seed check failed: no active exchange rates");
		}
