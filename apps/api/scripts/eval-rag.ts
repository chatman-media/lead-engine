...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (const raw of argv) {
    if (raw === "--help" || raw === "-h") args.help = true;
    ...
    else if (raw.startsWith("--golden=")) args.goldenPath = resolve(raw.slice("--golden=".length));
      ...
      args.updateBaselinePath =
        args.baselinePath ?? resolve("packages/kb/evals/generic-rag.baseline.json");
...
class FixtureKbStore implements IKbStore {
  ...
  async search(_embedding: number[], k: number, topic?: string | null): Promise<KbSearchHit[]> {
  ...
  async hybridSearch(input: {
    ...
    return this.filtered(input.topic)
      .map((chunk) => {
        const score = keywordScore(input.query, `${chunk.title} ${chunk.source} ${chunk.text}`);
        return {
          chunk,
  ...
  async getDocumentBySource(_source: string): Promise<{ id: number; content_hash: string } | null> {
    return null;
  }
...
async function main() {
  ...
  if (args.updateBaselinePath) {
    await Bun.write(args.updateBaselinePath, `${JSON.stringify(report, null, 2)}\n`);
  }
...
function printSummary(
  ...
  for (const variant of report.variants) {
    ...
    const deltaText = delta ? ` delta pass=${formatDelta(delta.passRateDelta)}` : "";
    console.log(
      [
        `- ${variant.variantId}`,
...
function buildMockAnswer(item: FixtureRagGoldenCase): string {
  const facts = item.expectedFacts?.join(" ") ?? "I will use the available knowledge base context.";
