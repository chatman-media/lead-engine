...
function addCandidate(out: Candidate[], seen: Set<string>, candidate: Candidate): void {
...
function formatTonValue(text: string, address: string): string {
  ...
  const taggedMemo = /(?:memo|tag|teg|мемо|тег)[\s\S]{0,50}?([A-Fa-f0-9]{10,}|[0-9]{7,})/i.exec(
    after,
  )?.[1];
...
function extractCandidates(blocks: string[]): Candidate[] {
  ...
  blocks.forEach((text, idx) => {
    ...
    for (const match of text.matchAll(TRON_RE)) {
      const asset = includesAny(lower, ["trx", "tron"]) && !lower.includes("usdt") ? "trx" : "usdt";
      if (
        asset === "trx" ||
        includesAny(lower, ["trc20", "trc-20", "tron", "usdt", "биржа", "панель"])
      ) {
        addCandidate(candidates, seen, {
          ...base,
    ...
    const binance = /binance[\s\S]*?\bID\s*:?\s*(\d{5,})|\bID\s*:?\s*(\d{5,})[\s\S]*?binance/i.exec(
      text,
    );
    ...
    const bybit =
      /(?:by\s*bit|bybit)[\s\S]*?(?:UID|ID)\s*:?\s*(\d{5,})|(?:UID|ID)\s*:?\s*(\d{5,})[\s\S]*?(?:by\s*bit|bybit)/i.exec(
        text,
      );
    ...
    const htx = /htx[\s\S]*?(?:UID|ID)?\s*:?\s*(\d{5,})|(\d{5,})\s*(?:UID|ID)?[\s\S]*?htx/i.exec(
      text,
    );
...
function printPreview(
  ...
  console.log(
    `[import-requisites] selected (${args.apply ? "apply" : "dry-run"}): ${selected.length}`,
  );
  console.log("");
