...
describe("run-tool-regressions CLI source handling", () => {
  test("parses API mode with token from env and filter options", () => {
    ...
    expect(args.supportedTools).toEqual(["offer_booking_link", "create_exchange_order"]);
  ...
  test("each option missing its value or invalid → error", () => {
    const cases: Array<[string[], string]> = [
      ...
      [["--status", "weird"], "--status must be active, archived, or all"],
  ...
  test("rejects ambiguous and invalid API arguments", () => {
    ...
    expect(parseArgs(["--file=cases.jsonl", "--api-base=http://api.test", "--token=x"])).toEqual({
      error: "--file and --api-base cannot be used together",
    });
    expect(parseArgs(["--api-base=http://api.test", "--token=x", "--status=bad"])).toEqual({
      error: "--status must be active, archived, or all",
    });
    expect(parseArgs(["--api-base=http://api.test", "--token=x", "--limit=0"])).toEqual({
  ...
  test("reports API HTTP failures with status and body", async () => {
    ...
    const args = parseArgs(["--api-base", server.url.toString(), "--token", "bad"]);
    expect("error" in args).toBe(false);
    if ("error" in args) return;

    await expect(loadRegressionCorpus(args)).rejects.toThrow("Quality API export failed: 401");
  ...
  test("writes JSON, JUnit, and Markdown report outputs", async () => {
    ...
    const json = JSON.parse(await Bun.file(`${outputDir}/report.json`).text()) as {
      summary: { passed: number; failed: number; skipped: number };
    };
    const junit = await Bun.file(`${outputDir}/report.xml`).text();
    const md = await Bun.file(`${outputDir}/report.md`).text();

    expect(json.summary).toEqual(expect.objectContaining({ passed: 1, failed: 1, skipped: 1 }));
...
describe("main()", () => {
  ...
  test("--json with passing corpus → machine-readable report, exitCode 0", async () => {
    ...
    const { logs, errors, exitCode } = await runMain(["--file", corpusFile, "--json"]);
    ...
    expect(report.summary).toEqual(expect.objectContaining({ total: 1, passed: 1 }));
  });
});
