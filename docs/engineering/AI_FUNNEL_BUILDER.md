...
**Поведенческий слой — Phase 2 (в работе):**
- ✅ **Стиль:** `POST /api/admin/styles/generate-full` — AI собирает полный `StyleSchema`
  (персона/тон/фреймворк/хуки/goal+guidance по sales-стадиям/few-shot) из описания
  бизнеса, валидирует (`StyleSchema.safeParse`) и сохраняет активным (`StylesRepo`).
  Дополняет лёгкий `/styles/generate` (тот лишь извлекает метаданные из примеров).
  ⚠️ **Активация per-tenant:** выбор стиля для ответов сейчас идёт по глобальному
  `STYLE_SLUG` (env) или эксперименту (`experiment-router`), НЕ по «активному стилю
  тенанта». Чтобы сгенерированный стиль автоматически вёл бота каждого тенанта, нужен
  per-tenant резолв дефолтного стиля в `llm-bootstrap` (следующий слайс).
- ⏳ **Пер-стадийные `goal`/`guidance` на структурных стадиях** (`stage_definitions`):
  колонок ещё нет; нужна миграция + проброс в reply-пайплайн (маппинг
  funnel-стадия↔sales-стадия). Отдельный слайс.
- ⏳ **Подбор навыков** (`skills-catalogue.ts`) — следом.

То есть структуру воронки и теперь черновик стиля AI собирает; «живая» per-tenant
активация стиля и инструкции на структурных стадиях — следующие слайсы Phase 2.
